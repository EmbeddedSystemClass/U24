...
Program: QMSL
   ...
   $Id: //depot/HTE/QDART/QMSL6/inc/QMSL_SWDownload/QMSL_QPHONEMS_SWDL.h#23 $
  ...
  *******************************************************************************/
  ...
  QLIB_API void QLIB_QPHONEMS_SaharaConfigureCallback(HANDLE hResourceContext, swdlQPHONEMSCB pSWDL_QPHONEMS_CB);
   ...
   /**
	...
	\param MaxPayloadSizeToTargetInBytes The amount data sent by the host to the device.
	...
	\param bUseAckRawDataAttr This parameter is used if the host wants an ACK for every raw data packet it sends to the device.
   ...
   /**
	...
	\param imageSizeInMB returns the acturall downloading size in MB
   ...
   /**
	...
	\param pPartitionList	this strusture holds a list of the Rawprogram and Patch files which contains the details about the images to download an Patches to apply.
	...
	*******************************************************************************/
	...
	QLIB_API unsigned char QLIB_QPHONEMS_EnableReadDataValidation_FireHose(HANDLE hResourceContext, unsigned char enabled);
