...
$Id: //depot/HTE/QDART/QMSL6/QLib_Defines.h#329 $
...
#define MAX_CAL_RESP_BUFFER_SIZE 20000
#define MAX_NUM_CAL_PKT_QUERY_RETRY 30
...
typedef enum
{
   ...
   GNSS_Start_IQ_TEST_COMMAND_REVISION_GENC  = 52
...
typedef enum
{
   ...
   GNSS_GAL_MEASUREMENT_REPORT_LOG  =  0x1481   // this is wrong, put in-place to help development.
} gps_logs_enum_type;
...
typedef struct
{
   ...
   dword    UNFILT_MS;
   float    UNFILT_SM;
   float	OFFSET_UNFILT_SM;
   float    UNFILT_TIME_UNC;
   float    UNFILT_SPEED;
   float	OFFSET_UNFILT_SPEED;
   float    UNFILT_SPEED_UNC;
   dword    MEAS_STATUS;
   byte     MISC_STATUS;
   dword    MULTIPATH_EST;
...
const int MAX_NUM_ALT_PATH = 8;
typedef struct nvgenparam
{
   nv_mode_id_type nvmode;
   BAND_CLASS_ENUM band;
   unsigned short version;
   unsigned short nvcontainer[MAX_NUM_DEVICES][MAX_NUM_ALT_PATH];
}calnvgenparam;
...
typedef enum
{
   RX_CAL_DATA_INFO_V1 = 118,
   RX_CAL_DATA_INFO_V2 = 119,
...
const size_t RX_CAL_DATA_SIZE_MAX = sizeof(RFNV_DATA_TYPE_RX_CAL_DATA) * MAX_NUM_DEVICES * MAX_NUM_ALT_PATH * MAX_NUM_TYPES_PER_RX_NV_CONTAINER;
typedef struct
{
   QMSL_Variant_Marker_Type aVariantMarker;
