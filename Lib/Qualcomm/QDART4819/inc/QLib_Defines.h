...
$Id: //depot/HTE/QDART/QMSL6/QLib_Defines.h#259 $
...
#define DIAG_MAX_PACKET_SIZE 4096    /*Increased the size due to Sahara Protocol requirments */ 
...
#define MAX_NUM_CAL_PACKETS 5
#define MAX_CAL_PACKET_SIZE 4096
#define MAX_NUM_CAL_PKT_QUERY_RETRY 10
...
//3 seconds.
#define MMGSDIDIAG_TIME_OUT 3000
...
typedef struct
{
   SWDL_eMMC_MetaBuild_SearchFileEntry pFileList[128];
...
typedef enum
{
    ...
    GNSS_ENGINE_REVISION_MAX          = GNSS_ENGINE_REVISION_GEN8C
...
typedef enum
{
   ...
   /*IQ Capture commands owns 900-915*/
...
typedef enum
{
   ...
   FTM_MODE_WCDMA_RX3          = 56,    //!<' WCDMA Chain 3 
   FTM_MODE_WCDMA_RX4          = 57,    //!<' WCDMA Chain 4
...
typedef struct
{
   ...
   word BestASPSINRBuffer[16];  //!<' SINR for the BEST ASP for previous frame last 16 odd
...
*******************************************************************************/
/**
The enum represents a list of all the FTM_WP commands.
*/
typedef enum
{
...
typedef struct
{
   ...
   */
   ...
   //! Data decoded from SCH channel. Data is only valid if SCCH_Found?is 1.
   dword iSchData;
...
typedef struct
{
   ...
   word ftm_cmd;                                           //<' FTM command id
   word status;
   word mfg_id[MAX_DEVICE_INSTANCES];
   word prd_id[MAX_DEVICE_INSTANCES];
   word rev_id[MAX_DEVICE_INSTANCES];
...
#define MAX_NUM_MEAS_ACTIONS 16
...
typedef enum
{
   ...
   FTM_Tx_Purp_Lin_Num_Waveform_MLV3      = 4,
...
// size = 3ch*((4pa * 2mod *69 pmeas) + (2*260dpd) + (2mod * 36PCL)) + (4pa*2mod*37rgi) = 3512 bytes
//Adding Header Total NV Size = 3728 + 4 + 10 = 3742
#define GSM_TX_CAL_NV_REV0_MAX_PAYLOAD_SIZE            3728
...
*/
#define MAX_EFS_RFNV_DATA_SZ 3800

/**
The maximum size of RFNV data
*/
#define MAX_RFNV_DATA_SZ 16384
/**
...
typedef enum
{
   ...
   ESC_FEEDBACK_SETUP_ACTION_CONFIGURE_FBRX_FOR_IQRMS = 0x8,
...
typedef struct QMSL_RFCAL_Unified_Rx_Struct
{
   ...
   //3*2*sizeof(QMSL_RFNV_DATA_TYPE_RX_CAL_OFFSET_V1_TYPE)+16*2*sizeof(QMSL_RFNV_DATA_TYPE_RX_OFFSET_VS_FREQ_V1)
...
typedef struct
{
   ...
   QMSL_COMMON_RFCAL_RX_NV rxNV[4];
...
#define GSM_V4_MAX_NUM_OF_FRAMES          200     // max number of frames that can be sent in FTM command
...
#define GSM_V4_MAX_NUM_RESULTS            200     // the maximum number of results returned by the FTM command
...
#define GSM_V4_TEMP_CMD_BUFF_SIZE         15500   // size of the temporary buffer for holding the GSM V4 cal command, which is variable in length    
// Max # of slots = 8 Tx + 2 Rx = 10
// Bytes per Frame = (10 slots) * (7 bytes per slot) + ( 6 bytes for frame header) = 76 bytes
// Max Command Size = (76 bytes per frame) * (200 frames) + (10 bytes for header) = 15210 bytes
...
#define GSM_V4_Result_Size  (GSM_V4_Rx_RSSI_Result_Size + 1)   // currently 10 bytes
typedef struct
{
   unsigned char opID;
   union
...
#define GSM_V4_Result_Payload_Size  (GSM_V4_MAX_NUM_RESULTS_PER_SLOT*GSM_V4_Result_Size + 5)   // currently 25 bytes
typedef struct
{
...
#define GSM_V4_Result_Response_Size  (GSM_V4_MAX_NUM_RESULTS*GSM_V4_Result_Payload_Size*GSM_V4_MAX_NUM_OF_DEVICES + 2)   // currently 10002 bytes
typedef struct
{
...
//RSB packet structure not currently implemented
/*typedef struct
{
unsigned char rsb_type;
unsigned char rsb_cal_alg; // 0 for using callbox, 1 for using stg, 2 for using Tx
int offset; // offset from the ref chan, for using internal stg only
unsigned char num_of_carrier; //Number of Carrier, 1 for PCell only, 2 for PCell and SCell, Extendable to other number for future increasing of number of WTR, 
...
typedef struct
{
   ...
   unsigned char ftm_cmd;                                         //<' FTM command id
   unsigned short req_len;
   unsigned short resp_len;
