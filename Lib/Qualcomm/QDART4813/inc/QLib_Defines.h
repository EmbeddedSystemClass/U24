...
$Id: //depot/HTE/QDART/QMSL6/QLib_Defines.h#132 $
...
#define MAX_CAL_PACKET_SIZE 4000
...
/******************************************************************************
UIM Diagnostic - MM GSDI Subsystem
*******************************************************************************/
...
typedef enum
{
   ...
   _FTM_ANT_OPEN_LOGICAL_CHANNEL		=  7,	//!<' Open logical channel
...
// size = (3ch*(4pa * 2mod *69 pmeas) + (2*260dpd)) + (4pa*2mod*37rgi) = 3512 bytes
//Adding Header Total NV Size = 3512 + 4 + 10 = 3526
#define GSM_TX_CAL_NV_REV0_MAX_PAYLOAD_SIZE            3512
...
typedef enum
{
   ...
   ESC_FEEDBACK_SETUP_ACTION_CONFIG_RXLM  = 0x2,
...
#define QMSL_TXLIN_NV_V3_SIZE_MAX  ( 0                           \
           + (1  *  sizeof( QMSL_RFNV_CAL_CHANS_DATA_TYPE ) )     \
           + (32 *  sizeof( QMSL_RFNV_FREQ_OFFSETS_DATA_TYPE ) )  \
           + (96 *  sizeof( QMSL_RFNV_LIN_TABLE_V3_DATA_TYPE ) )  \
           + (96 *  sizeof( QMSL_RFNV_ET_DPD_DATA_TYPE ) * 64 ) )
...
typedef struct QMSL_RFCAL_ESC_Calibration_NV_Container
{
   ...
   //Add HDET and Rx Items over Time
...
// slot payload
// 7 bytes
#define GSM_V4_BYTES_PER_SLOT  7
typedef struct
{
...
// frame payload
// 16*7 + 4 = 116 bytes
#define GSM_V4_BYTES_PER_FRAME   ((GSM_V4_MAX_NUM_OF_SLOTS*GSM_V4_MAX_NUM_OF_DEVICES*GSM_V4_BYTES_PER_SLOT) + 4)
typedef struct
{
   unsigned char  band;
   unsigned short channel;
   unsigned char  numSlotsInPayload;
   GSM_V4_Slot_Payload a_slotPayload[GSM_V4_MAX_NUM_OF_SLOTS * GSM_V4_MAX_NUM_OF_DEVICES];

} GSM_V4_Frame_Payload;

// main packet structure
// (116 bytes per frame)*(200 max # of frames) + 5 = 23,205 bytes max size
typedef struct
{
   ...
   GSM_V4_Frame_Payload a_framePayload[GSM_V4_MAX_NUM_OF_FRAMES];

} GSM_V4_Cal_Params_Type;
...
typedef struct
{
   ...
   unsigned short rxPwr_dBm16;      // Rx power result, measured by the phone, in dBm16 units
...
#define GSM_V4_Result_Response_Size  (GSM_V4_MAX_NUM_RESULTS*GSM_V4_Result_Payload_Size + 2)   // currently 5002 bytes
typedef struct
{
   unsigned short numPayload;
   GSM_V4_Result_Payload_Type payload[GSM_V4_MAX_NUM_RESULTS];
