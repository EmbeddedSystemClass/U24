...
extern "C" {
   ...
   *******************************************************************************/
   ...
   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()
   \param iLength = Number of packets to run the test over
   \param iThreshold = Threshold for determining if the packet is considered ood?
   \param iPid = Only packets with this PID will be tested (this is done to filter any nwanted?PIDs)
