...
bool Clog::OpenFile (TCHAR* sz_ErrCode, TCHAR* sz_ErrMsg)
{
	if(m_str_LogName.IsEmpty())
	{
		...
		CString str_ErrMsg = _T("AutoUpdateTool LOG文件路径D:\\LOG\\AutoUpdateTool_Log为空");
		_tcscpy_s(sz_ErrCode,512,str_ErrCode.GetBuffer());
		_tcscpy_s(sz_ErrMsg,512,str_ErrMsg.GetBuffer());
...
bool Clog::WriteLog (CString str_logContent, TCHAR* sz_ErrCode, TCHAR* sz_ErrMsg)
{
    ...
    //_T("D:\\LOG\\AutoUpdateTool_Log");
	CStdioFile flog;
	CFileException fileException;
    
    if (PathIsDirectory(_T("D:\\LOG\\AutoUpdateTool_Log")))
    {
	...
	{
		if   (CreateDirectory(_T("D:\\LOG\\AutoUpdateTool_Log"),NULL))   
		{   
...
void Clog::Setpath(CString& str_path)
{
	...
	str_tm.Format(_T("%d_%02d_%02d_%02d_%02d_%02d"),dt_currenttm.GetYear(),dt_currenttm.GetMonth(),dt_currenttm.GetDay(),dt_currenttm.GetHour(),dt_currenttm.GetMinute(),dt_currenttm.GetSecond());
	str_tm = str_tm + _T("_");
	str_tm = str_tm + _T("AutoUpdateTool.log");
	str_path = _T("D:\\LOG\\AutoUpdateTool_Log");
...
CString Clog::GetcurrentTime()
{
	...
	CString str_tm;
	dt_currenttm = CTime::GetCurrentTime();
	str_tm.Format(_T("%d-%02d-%02d %02d:%02d:%02d"),dt_currenttm.GetYear(),dt_currenttm.GetMonth(),dt_currenttm.GetDay(),dt_currenttm.GetHour(),dt_currenttm.GetMinute(),dt_currenttm.GetSecond());
	return str_tm;
}
