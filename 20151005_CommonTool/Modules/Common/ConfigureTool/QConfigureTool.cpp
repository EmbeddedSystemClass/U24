...
namespace QSYNC_LIB
{
	...
	CONFIGURETOOL_DLL_DECLSPEC IConfigure* CreateIConfigure(LPCTSTR szConfigureTitle, LPCTSTR szFolderName)
	{
		...
		else
		{
			if(!szFolderName)
				szFolderName = _T("");
			pICfgTool = new(szConfigureTitle, szFolderName) CQConfigureTool(szConfigureTitle, szFolderName);
			g_pCfgToolMap[szConfigureTitle] = pICfgTool;
			g_pCfgToolMap[szConfigureTitle]->AddRef();
		}
		return pICfgTool;
	}
	...
	CQConfigureTool::CQConfigureTool(LPCTSTR szConfigureTitle, LPCTSTR szFolderName)
		:m_strCfgTitle(szConfigureTitle)
		,m_strCfgFolder(szFolderName)
		,m_bIsSecurity(false)
		,m_nRefCount(0)
	{
	...
	void CQConfigureTool::AddRef()
	{
		m_nRefCount++;
	}

	bool CQConfigureTool::Release()
	{
		bool bRes = false;
		m_nRefCount--;
		if(m_nRefCount == 0)
	...
	bool GetFileContent(const TSTRING& strFilePath,string& strContent)
	{
		strContent.clear();
		bool bRes = false;
		FILE *stream = ::_tfopen(strFilePath.c_str(),_T("rb"));
		if(stream)
		{
			char cTemp;
			while(fread((void*)&cTemp,1,1,stream) != 0)
				strContent += cTemp;
			::fclose(stream);
		}
		bRes = !strContent.empty();
		return bRes;
	}

	bool GetMD5Content(const string& strContent,string& strDigest)
	{
		strDigest.clear();
		bool bRes = false;

		unsigned char szDigest[2048] = {0};
		MD5((unsigned char *)strContent.c_str(),strContent.size(),szDigest);

		int nDigestSize = strlen((const char *)szDigest);
		if(nDigestSize > 0)
		{
			int nBDecLen = Base64EncodeGetRequiredLength(nDigestSize);
			char* szBuffer = new char[nBDecLen+2];
			memset(szBuffer,0,nBDecLen+2);
			BOOL bSuccess = Base64Encode((BYTE*)szDigest, nDigestSize, (LPSTR)szBuffer, &nBDecLen) == TRUE;
			if(bSuccess && nBDecLen >= 0)
			{
				szBuffer[nBDecLen] = 0;
				strDigest =  szBuffer;
			}
		...
		bRes = !strDigest.empty();
		return bRes;
	}

	bool checkHash(const TSTRING& strContentFilePath,const TSTRING& strMD5FilePath)
	{
		bool bRes = false;
		
		string strContent;
		GetFileContent(strContentFilePath,strContent);

		string strDigest;
		GetMD5Content(strContent,strDigest);

		string strMD5Content;
		GetFileContent(strMD5FilePath,strMD5Content);

		return strDigest == strMD5Content;
	}

	void* CQConfigureTool::operator new( size_t nSize,LPCTSTR szConfigureTitle, LPCTSTR szFolderName)
	{
		void* p = NULL;

		if(_tcscmp(szFolderName, _T("CheckSum")) == 0)
		{
			//check hash
			TCHAR szFolder[MAX_PATH] = {0};

			//Prepare user path
			bool bRes = Win32Utility::GetShellFolder(szFolder,CSIDL_PERSONAL,(LPTSTR)szFolderName);
			assert(bRes);
			TSTRING strFilePath = szFolder;
			strFilePath += szConfigureTitle;
			TSTRING strMD5FilePath = strFilePath + _TC(".md5");
			strFilePath += _TC(".ini");

			//Prepare system path
			bRes = Win32Utility::GetShellFolder(szFolder,CSIDL_COMMON_DOCUMENTS,(LPTSTR)szFolderName);
			TSTRING strSystemFilePath = szFolder;
			strSystemFilePath += szConfigureTitle;
			TSTRING strSystemMD5FilePath = strSystemFilePath + _TC(".md5");
			strSystemFilePath += _TC(".ini");			

			//Check file
			TSTRING strRealFilePath;
			if(::_taccess_s(strFilePath.c_str(),0) != -1 && ::_taccess_s(strMD5FilePath.c_str(),0) != -1 && checkHash(strFilePath,strMD5FilePath))
				strRealFilePath = strFilePath;
			else	if(::_taccess_s(strSystemFilePath.c_str(),0) != -1 && ::_taccess_s(strSystemMD5FilePath.c_str(),0) != -1 && checkHash(strSystemFilePath,strSystemMD5FilePath))
				strRealFilePath = strSystemFilePath;

			if(strRealFilePath.empty() == false)
				p = ::malloc( nSize );
		}
		else
		{//Folder is not _T("CheckSum")
			p = ::malloc( nSize );
		}
	...
	void* CQConfigureTool::operator new( size_t nSize)
	{
		void* p = ::malloc( nSize );
	...
	void CQConfigureTool::operator delete( void *p)
	{
		::free(p);
	}

	void CQConfigureTool::operator delete( void* p,const char*,int)
	{
		::free( p );
	}
	...
	bool CQConfigureTool::GetCfgVar(const TSTRING& strNameconst,const TSTRING& strDefault,TSTRING& strBufferValue)
	{
		bool bRes = false;
		CProtectMutexEvent auto_protect(m_hEvent);
		...
		{
			//assert(strBufferValue.capacity() > iter->second.size());
			if(strBufferValue.capacity() > iter->second.size())
			{
				strBufferValue = iter->second;
				bRes = true;
			}
		}
		else
		{
			//assert(strBufferValue.capacity() > strDefault.size());
			if(strBufferValue.capacity() > strDefault.size())
			{
				strBufferValue = strDefault;
				bRes = true;
			}
		}
		return bRes;
