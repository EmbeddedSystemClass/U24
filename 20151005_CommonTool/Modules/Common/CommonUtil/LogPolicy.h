...
public:	
	...
	{
		implementLog(true,szMessage,L"",strFileName);
	}
	static void Log(const CStringW& strMessage,const CString& strFileName = _T(""), bool bShowTime = true)
	{
...
protected:
	...
	{		
		...
		CString strLogFileName = strFileName.IsEmpty() ? _T("eventlog_") + strDate + _T(".txt") : strFileName;		
	...
	static void implementLog(bool bAnsi,const char* szaMessage,const CStringW& strMessage,const CString& strFileName, bool bShowTime = true)
	{
		...
		if(bEnableLog)
		{
...
#ifndef _UseFileOpen_ANSI_
			using namespace std;

			DWORD nReopenComPortStartTime = ::GetTickCount();
			ofstream * pStream = new ofstream(GetFile(strFileName), ios::app);
			if(pStream)
			{
				...
				if(bAnsi == false)
				{
					if (bShowTime)
					{
						*pStream << GetCurrentTime() << szCRLF;
						*pStream << GetModuleName() << szCRLF;
					}
					*pStream << strMessage << szCRLF;
				}
				else
				{
					if (bShowTime)
					{
						CStringA strTime = UnicodetoASCII(GetCurrentTime());							
						*pStream << strTime << szaCRLF;
						CStringA strModule = UnicodetoASCII(GetModuleName());
						if(strModule.IsEmpty() == FALSE)
							*pStream << strModule << szaCRLF;
					}
					*pStream << szaMessage << szaCRLF;
				}
			...
			DWORD nUseTime = ::GetTickCount() - nReopenComPortStartTime;
#else
			bool bOpened = false;;
			CFile logFile;
			CFileException ex;
