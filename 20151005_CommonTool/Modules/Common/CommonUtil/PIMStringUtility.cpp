...
namespace QSYNC_LIB 
{
	...
	//dave
	bool SplitA(const string& strSplitter, const string& strOrg, string& strFirst, string& strSecond )
	{
		bool bRes = false;
		string szOrg = strOrg;
		string szSplitter = strSplitter;
	...
	bool TokenizeA(const string& strSplitter, const string& strOrg, vector<string>& strvDes, bool bSpaceOn)
	{
		bool bSuccess = false;
		if(bSpaceOn)
		{
			string strSrc = strOrg;
			string strFirst, strSecond;
			while(SplitA(strSplitter, strSrc, strFirst, strSecond))
			{
				bSuccess = true;
		...
		else
		{
			...
			string szSplitter = strSplitter;
			CStringA strToken("");
			CStringA strCOrg = strSrc.c_str();

			strToken = strCOrg.Tokenize(szSplitter.c_str(), nPos);
			while( strToken != _T("") )
			{
				strvDes.push_back((string)strToken);
				strToken = strCOrg.Tokenize(szSplitter.c_str(), nPos);
			}
			bSuccess = true;
		}
	...
	bool Char2To1Byte(const char* szInput, int nInputSize, BYTE* szOutput/*its size= szInputSize/2 + szInputSize%2 */)
	{
		bool bRes = false;
		if(szInput && szOutput)
		{
			char* szLowerInput = new char[nInputSize];
			for(int i=0; i< nInputSize; i++)
				szLowerInput[i] = tolower(szInput[i]);

			int nOutputSize = nInputSize/2 + nInputSize%2;
			for(int i=0, j=0; i<nOutputSize; i++)
			{
				BYTE nA = szLowerInput[j] >= 'a' ? szLowerInput[j]-'a'+10 : szLowerInput[j]-'0';
				BYTE nB = szLowerInput[j+1] >= 'a' ? szLowerInput[j+1]-'a'+10 : szLowerInput[j+1]-'0';
				szOutput[i] = (nA<<4) + (nB);
				j+=2;
			}
			delete [] szLowerInput;
