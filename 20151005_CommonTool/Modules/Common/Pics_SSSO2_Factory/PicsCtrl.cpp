...
CPicsCtrl::~CPicsCtrl()
{
//	WinExec("regsvr32 /u /s PICSInterface.dll",SW_SHOWDEFAULT);
   ::CoUninitialize();
}
...
unsigned char CPicsCtrl::Initial_PICSInterfaceDLL()
{
	...
	if(FAILED(hresult))
	{
//		WinExec("tregsvr -q PICSInterface.dll",SW_SHOWDEFAULT);
		WinExec("regsvr32 /s PICSInterface.dll",SW_SHOWDEFAULT);
		...
		if(FAILED(hresult))
		{
			AfxMessageBox("Initial PICSinterface.dll fail");
            return FALSE;
		}		
	}
