...
#define _TC(x) "x"
	...
	typedef void (EventFunc_Char)(const char* strEvent,long nParam);
	...
	interface INotify_Char
	{
		virtual void Event(const char* strEvent,long nParam) = 0;
	};
	...
	interface INotifyBase_Char
	{
		virtual bool Register(INotify_Char* pNotify,const char* strEvent) = 0;
		virtual bool Register(EventFunc_Char* pEventFunc,const char* strEvent) = 0;
		virtual bool Unregister(INotify_Char* pNotify,const char* strEvent) = 0;
		virtual bool Unregister(EventFunc_Char* pEventFunc,const char* strEvent) = 0;
		virtual void DefineNotify(const char* strNewEvent) = 0;
		virtual void Fire(const char* strEvent,long nParam) = 0;
	};
	...
	typedef TNotifyFuncData<INotify_Char, EventFunc_Char> NotifyFuncData_Char;

	//typedef SmartPtr<NotifyFuncData> NotifyFuncDataPt;
	typedef std::vector<NotifyFuncData> INotifyPool;
	typedef std::vector<NotifyFuncData>::iterator INotifyPoolIter;
	typedef std::map<TSTRING,INotifyPool> NotifyMap;
	typedef std::map<TSTRING,INotifyPool>::iterator NotifyMapIter;
	typedef std::pair<TSTRING,INotifyPool> NotifyMapPair;
	...
	typedef std::vector<NotifyFuncData_ANSI> INotifyPoolA;
	typedef std::vector<NotifyFuncData_ANSI>::iterator INotifyPoolAIter;
	typedef std::map<std::string,INotifyPoolA> NotifyMapA;
	typedef std::map<std::string,INotifyPoolA>::iterator NotifyMapAIter;
	typedef std::pair<std::string,INotifyPoolA> NotifyMapAPair;
	...
	/************************************************************************/
	/************************************************************************/
	/* For char* NotifyBase Instance                                         */
	/************************************************************************/

	typedef std::vector<NotifyFuncData_Char> INotifyPoolCh;
	typedef std::vector<NotifyFuncData_Char>::iterator INotifyPoolChIter;
	typedef std::map<std::string,INotifyPoolCh> NotifyMapCh;
	typedef std::map<std::string,INotifyPoolCh>::iterator NotifyMapChIter;
	typedef std::pair<std::string,INotifyPoolCh> NotifyMapChPair;

	class CNotifyBase_Char: public INotifyBase_Char
	{
	public:
		CNotifyBase_Char(){}
		virtual ~CNotifyBase_Char(){}
	private:
		NotifyMapChIter exist(const char* strEvent)
		{
			CSingleLock singleLock(&m_CritSection,TRUE);
			return m_NotifyMap.find(strEvent);
		}
	protected:
		...
		{
			CSingleLock singleLock(&m_CritSection,TRUE);
			NotifyMapChIter Iter = exist(strNewEvent);
			if(Iter == m_NotifyMap.end())
			{
				INotifyPoolCh newPool;
				m_NotifyMap.insert ( NotifyMapChPair ( strNewEvent,newPool ) );
			}
		}
		virtual void Fire(const char* strEvent,long nParam = 0)
		{
			CSingleLock singleLock(&m_CritSection,TRUE);
			bool bRes = false;
			NotifyMapChIter Iter = exist(strEvent);
			assert(Iter != m_NotifyMap.end());
			if(Iter != m_NotifyMap.end())
			{
				for(INotifyPoolChIter INotifyPoolIterIter = Iter->second.begin();INotifyPoolIterIter != Iter->second.end();INotifyPoolIterIter++)
				{
					if(NULL != INotifyPoolIterIter->m_pINotify)
						INotifyPoolIterIter->m_pINotify->Event(strEvent,nParam);
					else if(NULL != INotifyPoolIterIter->m_pEventFunc)
	...
	private:
		virtual bool Register(const NotifyFuncData_Char& notifyData,const char* strEvent)
		{
			CSingleLock singleLock(&m_CritSection,TRUE);
			...
			if(Iter != m_NotifyMap.end())
			{
				INotifyPoolChIter INotifyPoolIterIter = find(Iter->second.begin(),Iter->second.end(),notifyData);
				if(INotifyPoolIterIter == Iter->second.end())
					Iter->second.push_back(notifyData);
				bRes = true;
			}
		...
		virtual bool Unregister(const NotifyFuncData_Char& notifyData,const char* strEvent)
		{
			CSingleLock singleLock(&m_CritSection,TRUE);
	...
	public:
		virtual bool Register(INotify_Char* pINotify,const char* strEvent)
		{
			return Register(NotifyFuncData_Char(pINotify),strEvent);
		}
		virtual bool Register(EventFunc_Char* pEventFunc,const char* strEvent)
		{
			return Register(NotifyFuncData_Char(pEventFunc),strEvent);
		}
		virtual bool Unregister(INotify_Char* pINotify,const char* strEvent)
		{
			return Unregister(NotifyFuncData_Char(pINotify),strEvent);
		}
		virtual bool Unregister(EventFunc_Char* pEventFunc,const char* strEvent)
		{
			return Unregister(NotifyFuncData_Char(pEventFunc),strEvent);
		}
	private:
		NotifyMapCh		m_NotifyMap;
		CCriticalSection m_CritSection;
	};
