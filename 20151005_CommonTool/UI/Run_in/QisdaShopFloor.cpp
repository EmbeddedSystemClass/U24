...
#include "..\..\Modules\Common\CommonUtil\XML/MyMSXML.h"
...
const bool CQisdaShopFloor::ParsingXML(char *szErrCode, char *szErrMsg)
{
	...
	cstrXMLPath.Format("%s%s%s%s.xml", this->m_ModulePath.GetString(),
									   XMLPATHFILE,
...
const bool CQisdaShopFloor::CheckRoute(char *szID, char *szErrCode, char *szErrMsg)
{
	...
	if("10" != this->m_cstrDBInfo[DBI_EMPLOYEELV])
	{
		...
		return this->m_CheckRouting(szID,
...
const bool CQisdaShopFloor::LoadDBAccess(char *szErrorMsg)
{
	...
	this->m_CheckRouting = (pCheckRouting)GetProcAddress(this->m_hDBAccess, "CheckRouting_V2");
	if(!this->m_CheckRouting)
	{
		::strcpy_s(szErrorMsg, DefaultStringLength, "Load CheckRouting_V2 from dll fail");
		return false;
	}
