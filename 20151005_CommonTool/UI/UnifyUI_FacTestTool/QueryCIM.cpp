...
bool CQueryCIM::InsertTestRecordCIM( bool b_pass )
{
	...
	/**
	* @brief L12 recorded tool was opened by EntranceTool or BAT, 20140822 
	*/
	if (m_st_idParameter.b_OpenbyEntranceTool)
		m_map_picsData[_T("EntranceTool_BAT_Flag")] = _T("EntranceTool");
	else
		m_map_picsData[_T("EntranceTool_BAT_Flag")] = _T("BAT");
					...
					if(m_st_idParameter.str_CRType != _T("C"))
					{
						...
						if (b_ret)
						{
							...
							if (b_ret)
							{
								...
								Log(_T("Image version list read from vrcm is: ") + str_vrcmVersionList);
...
bool CQueryCIM::InsertAndCheckAccessory(bool &bNeedChange)
{
	CSingleLock singleLock(&m_obj_DBLock, TRUE);
	if(ConnectDataBase())
	{
		if (InsertAndCheckAccessoryCIM(bNeedChange))
		{
			return CloseDataBase();
		}
	}
...
bool CQueryCIM::InsertAndCheckAccessoryCIM(bool &bNeedChange)
{
	USES_CONVERSION;
	bNeedChange = false;
	...
	char sz_model[15] = {0};
	char sz_process[15] = {0};
	char sz_consumeListMax[1024] = {0};
	strncpy_s(sz_model, sizeof(sz_model), T2A(m_st_idParameter.str_modelName), sizeof(sz_model));
	strncpy_s(sz_process, sizeof(sz_process), T2A(m_st_idParameter.str_station), sizeof(sz_process));

	if (!QueryConsumeType(sz_model, sz_process, sz_consumeListMax,sz_ErrCode, sz_ErrMsg))
	{
		m_str_errorCode = ConvertString(sz_ErrCode);
		m_str_errorMsg = ConvertString(sz_ErrMsg);
		return false;
	...
	//for loop here SNType:SNID,SNType:SNID,
	CString str_accessoryList = m_st_idParameter.str_accessoryList;
	while (str_accessoryList.GetLength() != 0)
	{
		strTmp.Format(_T("str_accessoryList:%s"), str_accessoryList);
		Log(strTmp);

		CString str_consumeMaxTimes(sz_consumeListMax);

		int iIndex = str_accessoryList.Find(_T(','));
		if (iIndex == -1)
			break;

		CString str_accessory = str_accessoryList.Left(iIndex);
		str_accessoryList = str_accessoryList.Mid(iIndex+1);

		char sz_accessoryID[64] = {0};
		char sz_accessoryType[64] = {0};
		char sz_accessoryUsedtimes[64] = {0};
		int iSubIndex = str_accessory.Find(_T(':'));
		CString str_Type = str_accessory.Left(iSubIndex);
		CString str_ID = str_accessory.Mid(iSubIndex+1);
		if (str_ID.GetLength() == 0)
		{
			strTmp.Format(_T("Accessory(%s) ID is NULL, do not insert record"), str_Type);
			Log(strTmp);
			continue;
		}
		strncpy_s(sz_accessoryType, sizeof(sz_accessoryType), T2A(str_Type), sizeof(sz_accessoryType));
		strncpy_s(sz_accessoryID, sizeof(sz_accessoryID),  T2A(str_ID), sizeof(sz_accessoryID));

		char sz_ErrCode[15]={0};
		char sz_ErrMsg[MAX_BUFFER_SIZE]={0};

		if (!InsertConsumeUsedTimes(sz_accessoryID, sz_accessoryType, sz_accessoryUsedtimes,sz_ErrCode, sz_ErrMsg))
		{
			m_str_errorCode = ConvertString(sz_ErrCode);
			m_str_errorMsg = ConvertString(sz_ErrMsg);
			Log(m_str_errorMsg);
		...
		int iUsedTimes = atoi (sz_accessoryUsedtimes);
		strTmp.Format(_T("QueryConsumeInfo: ID(%s), Type(%s), Usedtimes(%d)"), str_ID, str_Type, iUsedTimes);
		Log(strTmp);

		// compare usedtimes > maxtims?
		iIndex = str_consumeMaxTimes.Find(str_Type);

		if (iIndex != -1)//usb:5000,
		{
			iIndex = str_consumeMaxTimes.Find(_T(':'), iIndex+1);
			iSubIndex = str_consumeMaxTimes.Find(_T(','), iIndex+1);
			int iMaxTimes = _wtoi(str_consumeMaxTimes.Mid(iIndex+1, (iSubIndex-iIndex)-1));
			strTmp.Format(_T("QueryConsumeInfo: ID(%s), Type(%s), Usedtimes(%d), maxtimes(%d)"), str_ID, str_Type, iUsedTimes, iMaxTimes);
			Log(strTmp);
			if (iUsedTimes > (iMaxTimes-50))
			{
				bNeedChange = true;
				m_str_errorCode = CommErr_UI_Accessory_UsedTimes_Check_Fail;
				m_str_errorMsg.Format(_T("Accessory(%s) usedtimes(%d) > maxtimes(%d+-50),please change new accessory!"), str_Type, iUsedTimes, iMaxTimes);
				return true;
			}
		}
		else
		{
			m_str_errorCode = CommErr_UI_Accessory_No_Define_MaxTimes;
			m_str_errorMsg.Format(_T("Accessory(%s) is not define maxtimes, please check ConsumePCS table!"), str_Type);
			return false;
		}
	} // while
