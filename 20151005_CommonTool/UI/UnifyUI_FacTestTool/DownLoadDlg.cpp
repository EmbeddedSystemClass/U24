...
bool CDownLoadDlg::GetSWVersion(void)
{    
     bool bReturn = false;
...
void CDownLoadDlg::InitUIShow(void)
{
	...
	for(int i = 0; i < DEVICE_NUMBER; i++)
	{
		...
		if ((m_st_uiParameter.str_station == STATION_PREDL)||(m_st_uiParameter.str_station == STATION_DL))
		{
//#ifndef RD_MODE
...
void CDownLoadDlg::ChangeGB()
{

	m_brush.CreateSolidBrush(BKGCOLOUR_FACTORY);
	/* Image path static frame*/
	m_label_swPath.SetBkColor(BKGCOLOUR_FACTORY);
	...
	else 
	{
		...
		for(int i = 0; i < DEVICE_NUMBER; i++)
		{
			if (m_map_COMPort[i] != _T("0"))
			{
				/* Set Slot */
				char sz_slot[8] = {0};
				sprintf_s(sz_slot, sizeof(sz_slot), "%d", i);

				if (!m_p_DLInterface->SetParameterValue(INIT_SLOT, sz_slot))
				{
					st_UIResult st_result;
					st_result.i_slot = i;
					CStringToChar(_T("Set InitSlot Fail"), st_result.sz_msg);
					CStringToChar(CommErr_UI_Interface_Setparameter_Fail, st_result.sz_errCode);
					m_p_DLInterface->Fire(UI_RESULT, (long)&st_result);
				}
...
{	
	...
	if (! p_this->m_p_st_uiControl->b_ReadFA)
	{
		/* Set Slot */
		char sz_slot[8] = {0};
		sprintf_s(sz_slot, sizeof(sz_slot), "%d", p_this->m_i_slot);

		if (!p_this->m_p_handsetDLInterface->SetParameterValue(INIT_SLOT, sz_slot))
		{
			p_this->FireUIMessage("Set InitSlot Fail.");
