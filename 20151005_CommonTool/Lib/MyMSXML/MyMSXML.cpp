...
INT CMyMSXML::Load(LPCTSTR filePath)
{
	...
	OleInitialize(NULL);
	if (m_DomDoc == NULL) {
		HRESULT hr = m_DomDoc.CreateInstance(MSXML2::CLSID_DOMDocument, NULL, CLSCTX_ALL);//CLSCTX_INPROC_SERVER| CLSCTX_LOCAL_SERVER);
	...
	if (!bResult) {
		...
		MSXML2::IXMLDOMParseErrorPtr pParseError = m_DomDoc->GetparseError();
...
BOOL CMyMSXML::CreateNode(LPCTSTR path, LPCTSTR nodeText)
{
	...
	// create attributes
	MSXML2::IXMLDOMNamedNodeMapPtr pAttrList = newElem->Getattributes();
	if (NULL == pAttrList)
		return FALSE;
	...
	for (UINT i = 0; i < attrName.size(); i++){
		...
		MSXML2::IXMLDOMAttributePtr pNewAttr = m_DomDoc->createAttribute(bstrAttrName.m_str);
...
BOOL CMyMSXML::GetDocNum(XMLNode node, long& i_num)
{
	if(node == NULL)
		return false;
...
BOOL CMyMSXML::DeleteNode(LPCTSTR path)
{
	...
	if (MSXML2::NODE_DOCUMENT == nodeType ||	MSXML2::NODE_DOCUMENT_FRAGMENT == nodeType)
...
void CMyMSXML::GetNodeAttrValue(LPCTSTR nodeName, LPCTSTR attrName, LPTSTR attrValue, size_t inLength)
{
	...
	MSXML2::IXMLDOMNamedNodeMapPtr attr = NULL;
...
BOOL CMyMSXML::GetNodeListItem(XMLNodeList nodeList, CString& str_name, CString& str_MD5)
{
	bool b_res = false;
	...
	for(int j = 0; j <= i_NodeNum; j++)
	{
		...
		// 获取name节点
		str_temp = GetNodeName(pItem);
		
		if (str_temp.CompareNoCase(_T("name")) == 0)
		{
			str_name = GetNodeText(pItem);
			str_name = str_name.Mid(str_name.ReverseFind('\\') + 1);
		}

		// 获取MD5节点

		if (str_temp.CompareNoCase(_T("MD5")) == 0)
		{
			str_MD5 = GetNodeText(pItem);
		}
	}
