...
BOOL CUnifyUI_FacTestToolDlg::OnInitDialog()
{
	...
	m_map_StationIndexMap[STATION_TRIGGER] = _T("01");
...
bool CUnifyUI_FacTestToolDlg::InitialUIControl()
{
	...
	//auto run only for OS_DL, 2G3GTEST
	::GetPrivateProfileString(m_st_idParameter.str_station, _T("AutoRun"), _T("0"), sz_temp, sizeof(sz_temp), sz_iniFolderName);
	if (m_st_idParameter.str_station == STATION_OS_DL || m_st_idParameter.str_station == STATION_2G3GTEST)
	{
...
{
	if ((m_st_idParameter.str_station == STATION_TRIGGER)||
...
void CUnifyUI_FacTestToolDlg::EnableDisableButton(bool b_flag)
{
	...
	if (m_st_idParameter.str_station == STATION_2G3GTEST && m_st_uiControl.b_AutoRun)
	{
		m_button_start.EnableWindow(false);                     //For auto Run to disable Go Button
		m_button_exit.EnableWindow(false);
	}
...
{  
	...
	if (m_b_NewErrCode)
	{
		if ((str_errorCode.Left(2) != _T("DB")) && (str_errorCode.Left(2) != _T("UI")))
		{
			...
			{		
				...
				strTmp.Format(_T("[%d] Error Not Found:\n%s"),p_st_result->i_slot,str_errorCode);
				str_errorCode = strTmp;
			}
			else
			{
	...
	else if (m_st_idParameter.str_station == STATION_2G3GTEST && m_st_uiControl.b_Multi && m_st_uiControl.b_AutoRun)
	{
...
UINT CUnifyUI_FacTestToolDlg::RunThread(LPVOID pParam)
{
	...
	if ( !((pThis->m_st_idParameter.str_station == STATION_LTE || pThis->m_st_idParameter.str_station == STATION_2G3GTEST) && pThis->m_st_uiControl.b_Multi))
	{
	...
	if (!(pThis->m_st_idParameter.b_tsFlag))
	{
		if (!pThis->CheckPicassoRule() 
			&& (pThis->m_st_idParameter.str_station != STATION_TRIGGER) 
	...
	case 1:
		{
			...
			else if(m_st_idParameter.str_station == STATION_2G3GTEST && m_st_uiControl.b_AutoRun)
			{
			}
			else
			{
...
#ifdef RD_MODE
	if ((m_st_idParameter.str_station == STATION_TRIGGER)||(m_st_idParameter.str_station == STATION_DL))
	{
		i_pageIdx = m_tab_stationPageCtrl.GetCurSel();
		if (m_pageArray[i_pageIdx] == NULL)
		{
