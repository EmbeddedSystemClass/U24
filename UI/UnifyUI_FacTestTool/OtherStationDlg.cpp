...
bool COtherStationDlg::LoadDLL(void)
{
	...
	{
		str_temp.Format(_T("DLL isn't defined for this station:%s in UnifyUI, use default FT.dll instead"), m_st_uiParameter.str_station);
		LogMsg(str_temp);
	...
	if (m_h_module == NULL)
	{ 
		LogMsg(_T("Load ") + str_dllName + _T(" Fail! Use") + m_st_uiParameter.str_station + _T(".dll, retry.."));
...
bool COtherStationDlg::GetPicasso(std::map<int, CString> &map_picasso)
{
	...
	if (m_st_uiControl.b_ScanPicasso)
	{
		m_p_facTestToolInterface->SetParameterValue("PICASSO", T2A(map_picasso[1]));//Set picasso for PreRun, nancy.yang 20130802
	}
...
bool COtherStationDlg::ReadFA()
{
	...
	if (!m_b_haveReadedFAFlag)
	{
		...
		if (m_st_uiControl.i_FAArea == OLDFA)
		{
			b_ret = m_p_facTestToolInterface->GetFAData(1,sz_faData,sizeof(sz_faData));
		}
		else if (m_st_uiControl.i_FAArea == NEWFA)
		{
			b_ret = m_p_facTestToolInterface->GetFASector(1,0,sz_faData,sizeof(sz_faData));
		}
