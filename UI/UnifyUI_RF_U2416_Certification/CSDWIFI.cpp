...
IMPLEMENT_DYNAMIC(CCSDWIFI, CDialog)

CCSDWIFI::CCSDWIFI(CWnd* pParent /*=NULL*/)
	: CDialog(CCSDWIFI::IDD, pParent)
	, bRxTestStop(false)
{
...
void CCSDWIFI::DoDataExchange(CDataExchange* pDX)
{
...
void CCSDWIFI::SetInit(dlltoolinfo_t& data)
{
	m_data = data;
}

int CCSDWIFI::MsgCall(const char* szMsg, const char* szTag)
{
	CString msg, tag;
	if (szMsg != NULL) {
...
void CCSDWIFI::PrintMsg(CString message, CString tag, bool isLog)
{
...
int CCSDWIFI::RxResultCall(const char* szMeasured)
{
	CString measured;
...
void CCSDWIFI::SetResult(int nIndex, char* szUnit, char* szLower, char* szUpper, char* szMeasured, char* szResult, char* szErrCode, char* szMsg)
{
...
void CCSDWIFI::UIControl(bool isEnable)
{
	GetDlgItem(IDC_BUTTON_RUN)->EnableWindow(isEnable);
}

void CCSDWIFI::UITotalTime(double secTime)
{
	CString csTime;
...
void CCSDWIFI::Timer(bool isEnable)
{
	if (isEnable) {
		if (m_thrdTimer == NULL) {
...
UINT CCSDWIFI::WorkerThreadTimerProc(LPVOID param)
{
	CCSDWIFI* pThis = reinterpret_cast<CCSDWIFI*>(param);
	ASSERT(pThis != NULL);
...
void CCSDWIFI::WorkerThreadTimerRun()
{
	clock_t start, now;
	start = clock();
	do {
...
void CCSDWIFI::OnBnClickedButtonRun()
{
...
UINT CCSDWIFI::WorkerThreadFuncProc(LPVOID param)
{
	CCSDWIFI* pThis = reinterpret_cast<CCSDWIFI*>(param);
	ASSERT(pThis != NULL);
...
void CCSDWIFI::WorkerThreadFuncRun()
{
	Timer(true);
	UIControl(false);
	bRxTestStop = false;
