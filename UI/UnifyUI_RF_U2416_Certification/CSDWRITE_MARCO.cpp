...
CCSDWRITE_MARCO* g_ui;

// CCSDWRITE_MARCO dialog

IMPLEMENT_DYNAMIC(CCSDWRITE_MARCO, CDialog)

CCSDWRITE_MARCO::CCSDWRITE_MARCO(CWnd* pParent /*=NULL*/)
	: CDialog(CCSDWRITE_MARCO::IDD, pParent)
	//, m_sChannel(_T(""))
	, m_sTag(_T(""))
	//, m_sRateBitIndex(_T(""))
{
...
CCSDWRITE_MARCO::~CCSDWRITE_MARCO()
{
}

void CCSDWRITE_MARCO::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CCSDWRITE_MARCO, CDialog)
	ON_BN_CLICKED(IDC_BUTTON_RUN, &CCSDWRITE_MARCO::OnBnClickedButtonRun)
END_MESSAGE_MAP()


// CCSDWRITE_MARCO message handlers
BOOL CCSDWRITE_MARCO::OnInitDialog()
{
	...
	((CEdit*)GetDlgItem(IDC_EDIT_TAG))->SetLimitText(SN_LENGTH);
...
BOOL CCSDWRITE_MARCO::DestroyWindow()
{
...
void CCSDWRITE_MARCO::SetInit(dlltoolinfo_t& data)
{
	m_data = data;
}

int CCSDWRITE_MARCO::MsgCall(const char* szMsg, const char* szTag)
{
	CString msg, tag;
	if (szMsg != NULL) {
...
void CCSDWRITE_MARCO::PrintMsg(CString message, CString tag, bool isLog)
{
...
int CCSDWRITE_MARCO::ResultCall(const int nIndex, const char* szUnit, const char* szLower, const char* szUpper, const char* szMeasured, const char* szResult, const char* szErrCode, const char* szMsg)
{
...
void CCSDWRITE_MARCO::SetResult(int nIndex, char* szUnit, char* szLower, char* szUpper, char* szMeasured, char* szResult, char* szErrCode, char* szMsg)
{
...
void CCSDWRITE_MARCO::UITotalTime(double secTime)
{
	CString csTime;
...
void CCSDWRITE_MARCO::Timer(bool isEnable)
{
	if (isEnable) {
		if (m_thrdTimer == NULL) {
...
UINT CCSDWRITE_MARCO::WorkerThreadTimerProc(LPVOID param)
{
	CCSDWRITE_MARCO* pThis = reinterpret_cast<CCSDWRITE_MARCO*>(param);
	ASSERT(pThis != NULL);
...
void CCSDWRITE_MARCO::WorkerThreadTimerRun()
{
	clock_t start, now;
	start = clock();
	do {
...
UINT CCSDWRITE_MARCO::WorkerThreadFuncProc(LPVOID param)
{
	CCSDWRITE_MARCO* pThis = reinterpret_cast<CCSDWRITE_MARCO*>(param);
	ASSERT(pThis != NULL);
...
void CCSDWRITE_MARCO::WorkerThreadFuncRun()
{
	Timer(true);
	//UIControl(false);
	CString csString;
	...
	if (csTag.GetLength() != SN_LENGTH){
		 csString  = _T("illegal  Sn,  ") + csTag  + _T("\n");
	...
	{
		csString = _T("start to write Sn,  ") + csTag  + _T("\n");
		PrintMsg(csString, _T(""));
		csTag.ReleaseBuffer();
...
void CCSDWRITE_MARCO::OnBnClickedButtonRun()
{
...
bool CCSDWRITE_MARCO::SearchPortUsbAdb(const UINT secTimeout)
{
	UINT timer = 0;
	bool hasPort = false;
	for (;;) {
...
bool CCSDWRITE_MARCO::FindUsbDevice()
{
...
bool CCSDWRITE_MARCO::FindDeviceId(CString sDeviceId)
{
	/*bool isFound =
...
bool CCSDWRITE_MARCO::SearchPortUsb(const UINT secTimeout)
{
	UINT timer = 0;
	bool hasPort = false;
	m_com.port.Empty();
...
CString CCSDWRITE_MARCO::GetDeviceName(HDEVINFO& hDevInfo, SP_DEVINFO_DATA& spDevInfoData)
{
	DWORD DataT;
...
bool CCSDWRITE_MARCO::GetComPort()
{
...
bool CCSDWRITE_MARCO::GetAdbDevice()
{
	return
