// RFCertification2G4Tx.cpp : implementation file
//
...
CRFCertification2G4Tx* g_ui;


// CRFCertification2G4Tx dialog

IMPLEMENT_DYNAMIC(CRFCertification2G4Tx, CDialog)

CRFCertification2G4Tx::CRFCertification2G4Tx(CWnd* pParent /*=NULL*/)
	: CDialog(CRFCertification2G4Tx::IDD, pParent)
	, m_sChannel(_T(""))
	, m_sDataRate(_T(""))
	, m_sPower(_T(""))
	, m_sChain(_T(""))
...
CRFCertification2G4Tx::~CRFCertification2G4Tx()
{
}

void CRFCertification2G4Tx::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_COMBO_PORT, m_combComport);
	DDX_Text(pDX, IDC_EDIT_CHANNEL, m_sChannel);
	DDX_Text(pDX, IDC_EDIT_DATA_RATE, m_sDataRate);
	DDX_Text(pDX, IDC_EDIT_POWER, m_sPower);
}


BEGIN_MESSAGE_MAP(CRFCertification2G4Tx, CDialog)
	ON_BN_CLICKED(IDC_BUTTON_RUN, &CRFCertification2G4Tx::OnBnClickedButtonRun)
	ON_CBN_DROPDOWN(IDC_COMBO_PORT, &CRFCertification2G4Tx::OnCbnDropdownComboPort)
END_MESSAGE_MAP()


// CRFCertification2G4Tx message handlers

BOOL CRFCertification2G4Tx::OnInitDialog()
{
...
BOOL CRFCertification2G4Tx::DestroyWindow()
{
...
void CRFCertification2G4Tx::SetInit(dlltoolinfo_t& data)
{
	m_data = data;
}

int CRFCertification2G4Tx::MsgCall(const char* szMsg, const char* szTag)
{
	CString msg, tag;
	if (szMsg != NULL) {
...
void CRFCertification2G4Tx::PrintMsg(CString message, CString tag, bool isLog)
{
...
int CRFCertification2G4Tx::UpdatePortCall(const char* szPort)
{
	CString port;
	if (szPort != NULL) {
...
void CRFCertification2G4Tx::SetPortUpdate(CString port)
{
...
int CRFCertification2G4Tx::ResultCall(const int nIndex, const char* szUnit, const char* szLower, const char* szUpper, const char* szMeasured, const char* szResult, const char* szErrCode, const char* szMsg)
{
...
int CRFCertification2G4Tx::RxResultCall(const char* szMeasured)
{
	CString measured;
...
void CRFCertification2G4Tx::SetResult(int nIndex, char* szUnit, char* szLower, char* szUpper, char* szMeasured, char* szResult, char* szErrCode, char* szMsg)
{
...
void CRFCertification2G4Tx::SetRxResult(char* szMeasured)
{
	CEdit* edit = (CEdit*)GetDlgItem(IDC_EDIT_PACKET);
	edit->SetWindowText(CA2T(szMeasured));
}

void CRFCertification2G4Tx::UIInit()
{
	LOGFONT lf;
...
void CRFCertification2G4Tx::UIWarning(CString message)
{
	CHARFORMAT cf;
	memset(&cf, 0, sizeof(CHARFORMAT));
	cf.cbSize = sizeof(CHARFORMAT);
...
void CRFCertification2G4Tx::UIControl(bool isEnable)
{
	GetDlgItem(IDC_BUTTON_RUN)->EnableWindow(isEnable);
}

void CRFCertification2G4Tx::UITotalTime(double secTime)
{
	CString csTime;
...
void CRFCertification2G4Tx::Timer(bool isEnable)
{
	if (isEnable) {
		if (m_thrdTimer == NULL) {
...
UINT CRFCertification2G4Tx::WorkerThreadTimerProc(LPVOID param)
{
	CRFCertification2G4Tx* pThis = reinterpret_cast<CRFCertification2G4Tx*>(param);
	ASSERT(pThis != NULL);
...
void CRFCertification2G4Tx::WorkerThreadTimerRun()
{
	clock_t start, now;
	start = clock();
	do {
...
void CRFCertification2G4Tx::OnBnClickedButtonRun()
{
	...
	{
		PrintMsg(_T("Please enter Channel in the input field.\n"), _T("WARN"));
		return;
	}
	if (m_sDataRate.IsEmpty())
	{
		PrintMsg(_T("Please enter Data Rate in the input field.\n"), _T("WARN"));
		return;
	}
	if (m_sPower.IsEmpty())
	{
		PrintMsg(_T("Please enter Power in the input field.\n"), _T("WARN"));
		return;
	}
	if (m_sChain.IsEmpty())
	{
		PrintMsg(_T("Please enter Chain in the input field.\n"), _T("WARN"));
		return;
	}
...
UINT CRFCertification2G4Tx::WorkerThreadFuncProc(LPVOID param)
{
	CRFCertification2G4Tx* pThis = reinterpret_cast<CRFCertification2G4Tx*>(param);
	ASSERT(pThis != NULL);
...
void CRFCertification2G4Tx::WorkerThreadFuncRun()
{
	Timer(true);
	UIControl(false);
	...
	char BtInput[128] = {0};
	CString sChannel;
	GetDlgItem(IDC_EDIT_CHANNEL)->GetWindowText(sChannel);
	strcat(BtInput, CT2A(sChannel.GetBuffer()));
	
	strcat(BtInput, ",");

	CString sDataRate;
	GetDlgItem(IDC_EDIT_DATA_RATE)->GetWindowText(sDataRate);
	strcat(BtInput, CT2A(sDataRate.GetBuffer()));

	strcat(BtInput, ",");

	CString sPower;
	GetDlgItem(IDC_EDIT_POWER)->GetWindowText(sPower);
	strcat(BtInput, CT2A(sPower.GetBuffer()));

	CString sChain;
	GetDlgItem(IDC_COMBO_CHAIN)->GetWindowText(sChain);
	strcat(BtInput, CT2A(sChain.GetBuffer()));


	m_dllCtrl.SetParameter("RF2G4INPUT", BtInput);
	
	sChannel.ReleaseBuffer();
	sDataRate.ReleaseBuffer();
	sPower.ReleaseBuffer();
	sChain.ReleaseBuffer();
...
void CRFCertification2G4Tx::OnCbnDropdownComboPort()
{
...
bool CRFCertification2G4Tx::SearchPortUsbAdb(const UINT secTimeout)
{
	UINT timer = 0;
	bool hasPort = false;
	for (;;) {
...
bool CRFCertification2G4Tx::FindUsbDevice()
{
...
bool CRFCertification2G4Tx::FindDeviceId(CString sDeviceId)
{
	/*bool isFound =
...
bool CRFCertification2G4Tx::SearchPortUsb(const UINT secTimeout)
{
	UINT timer = 0;
	bool hasPort = false;
	m_com.port.Empty();
...
CString CRFCertification2G4Tx::GetDeviceName(HDEVINFO& hDevInfo, SP_DEVINFO_DATA& spDevInfoData)
{
	DWORD DataT;
...
bool CRFCertification2G4Tx::GetComPort()
{
...
bool CRFCertification2G4Tx::GetAdbDevice()
{
	return
