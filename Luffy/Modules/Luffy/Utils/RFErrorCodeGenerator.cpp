...
bool GenRFErrorCode(std::string strTech, int iBand, std::string strChannelRange, std::string strMode, std::string strItem, std::string strBasicErrCode, std::string& strErrorCode, std::string& strErrMsg)
{
    ...
    int i = -1;
    int nMaxCompareLength = 8;
    char szBasicErrCode[1024] = {0};
    char szBasicErrCodeLower[1024] = {0};

    strErrorCode.clear() ;
    strErrMsg.clear() ;

    //Make lower
    sprintf_s(szBasicErrCode, 1024, "%s", strBasicErrCode.c_str());
    while(szBasicErrCode[i++])
    {
        if (i >= nMaxCompareLength)
            break;

        szBasicErrCodeLower[i] = tolower(szBasicErrCode[i]);
    }
    strBasicErrCode = szBasicErrCodeLower;

	// Generate Error Code
    if ( strBasicErrCode.compare(0, 7, "device_") == 0 )
    {
    ...
    else if ( strBasicErrCode.compare(0, 5, "qlib_") == 0 )
    {
        // Generate Error Code
        strErrorCode = strBasicErrCode ;
    }
    else if (( strBasicErrCode.compare(0, 3, "gsm") == 0 )
        || ( strBasicErrCode.compare(0, 4, "edge") == 0 )
        || ( strBasicErrCode.compare(0, 4, "cdma") == 0 )
        || ( strBasicErrCode.compare(0, 5, "wcdma") == 0 )
        || ( strBasicErrCode.compare(0, 3, "lte") == 0 ))
    {
...
{
    if ( (0 == _stricmp(strChannelRange.c_str(), "LOW")) || (0 == _stricmp(strChannelRange.c_str(), "L")) )
    {
        strChannelRange = "L" ;
        return true ;
    }
    else if ( (0 == _stricmp(strChannelRange.c_str(), "MID")) || (0 == _stricmp(strChannelRange.c_str(), "M")) )
    {
        strChannelRange = "M" ;
        return true ;
    }
    else if ( (0 == _stricmp(strChannelRange.c_str(), "HIGH")) || (0 == _stricmp(strChannelRange.c_str(), "H")) )
    {
        strChannelRange = "H" ;
        return true ;
    }
