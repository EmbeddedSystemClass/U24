...
size_t CTestBoxHdrStd::LoadTestBox()
{
	...
	XMLNode testerDevice = m_TestBoxXML.SearchNode(_T("//Configuration//TesterDevice"));
...
size_t CTestBoxHdrStd::SetToolTestBox()
{
	...
	for(	std::map<CString, CString>::iterator iter = this->m_mapTesteBox.begin();
			...
			iter++)
	{
		if(!this->m_pITI->SetTestPart(IToolInterfaceV2::TEST_BOX, L"", iter->first.GetString(), iter->second.GetString()))
		{
