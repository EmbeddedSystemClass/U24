...
size_t CPhoneHdrStd::LoadPhoneInTestItem()
{
	...
	XMLNodeList phoneList = m_TestItemXML.GetChildNodes(_T("//Configuration//Phone"));
...
size_t CPhoneHdrStd::SetToolPhone()
{
	...
	for(	std::map<CString, CString>::iterator iter = this->m_mapPhone.begin();
			...
			iter++)
	{
		//if(!this->m_pITI->SetTestPart(IToolInterfaceV2::PHONE, L"", iter->first.GetString(), iter->second.GetString()))
		//{
