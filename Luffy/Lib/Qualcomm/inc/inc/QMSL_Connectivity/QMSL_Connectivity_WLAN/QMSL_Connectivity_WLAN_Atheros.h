...
Program: QMSL_WLAN
	...
	$Id: //depot/HTE/QDART/QMSL6/inc/QMSL_Connectivity/QMSL_Connectivity_WLAN/QMSL_Connectivity_WLAN_Atheros.h#6 $
...
extern "C" {
   ...
   *******************************************************************************/
   ...
   \param   DevDLLname	= Device specific DLL name (less than 30 charactors, no path)
		 ...
		 boardDataFName = Board data file name (less than 50 charactors, no path)
						  Board data file must be in .\..\driver\BoardData\ from QMSL dll location

		   devInterFace	= device interface number
   ...
   *******************************************************************************/
   ...
   \param calGainMode =	Configuration options for 2 point calibration
		0: The second element of *piGain in QLIB_FTM_WLAN_Atheros_Tx_CALIBRATE() is an absolute gainIndex
		1: The second element of *piGain in QLIB_FTM_WLAN_Atheros_Tx_CALIBRATE() is a delta from the first 
   ...
   *******************************************************************************/
   ...
   \param 
         ...
         iDataRate =  calibration data rate
			   (Please refer iDataRate definition in QLIB_FTM_WLAN_Atheros_DevTx for the rate specifiers for both QC98xx, AR6004)
   ...
   *******************************************************************************/
   ...
   \param	iRegAddr  = register address
   			iHighBit = high bit for register value to be read
   			iLowBit  = low bit for register value to be read
   			piRegValue = pointer to the register value read from register
   ...
   \warning QLIB_FTM_WLAN_SetModuleType() must be called first to set the WLAN module type
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_WLAN_Atheros_DevReadReg
   (
      HANDLE hResourceContext,
      unsigned long iRegAddr,
      unsigned long iHighBit,
	  unsigned long iLowBit,
      unsigned long* piRegValue
   );
  ...
  \param	iRegAddr  = register address
   			iHighBit  = high bit for register value to be written
   			iLowBit   = low bit for register value to be written
   			iRegValue = register value to be written
   ...
   \warning QLIB_FTM_WLAN_SetModuleType() must be called first to set the WLAN module type
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_WLAN_Atheros_DevWriteReg
   (
      HANDLE hResourceContext,
      unsigned long iRegAddr,
      unsigned long iHighBit,
