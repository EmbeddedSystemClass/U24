...
CDownload8994::CDownload8994(const TSTRING& str_deviceName, const std::string& str_platform)
:CDLFormatCommand(str_deviceName, str_platform), m_b_reOpenAfterReset(false), m_b_resetMode(true), m_b_overridePartition(false), m_p_adbDevice(NULL)
{
	...
	//m_iCOMPort = 0;
	//m_iCmdTimeOut = 30000;
	//m_iFirehoseDLMode = FIREHOSE_DL_MODE_FASTBOOT_ONLY;
	m_iFileCount = 0;
	m_iFileIndex = 0;

}

CDownload8994::CDownload8994(int i_COMPort, CString str_multiDLFlag, const std::string& str_platform)
...
bool CDownload8994::FastbootEXE_Download(){
	...
	int nGetADBPort = -1;
	for ( int nTime = 0 ; nTime < 120 ; nTime ++){
		nGetADBPort = FindADBdevice( -1 );
		if (nGetADBPort != -1 ){
		//	bGetADB = true;
			break;
		}
	...
	if ( nGetADBPort == -1 ){ 
		AddMsg(false, "FindADBdevice Fail", FireHoseProtocol);
		return false;
	}
	...
	for (int i = 0 ; i < mapDL_PASSPORT_IMGS_TYPE.size() ; i++){
		...
		//csExePath.Format(_T("\"%s\" flash %s \"%s\" -s QPHONE%d"),str_FastbootPath, csImgType, csImgPath, nGetADBPort);
		csExePath.Format(_T("\"%s\" flash %s \"%s\" -s QPHONE%d"),str_FastbootPath, csImgType, csImgPath, nGetADBPort);
		//csExePath.Format(_T("%s flash %s %s -s QPHONE%d"),str_FastbootPath, csImgType, csImgPath, nGetADBPort);

		//MyExecCmd("fastboot", szCmd, szSerialNumber, szReturn, dwReturnLen, timeout);
		//csExePath.Format(_T("%s"),"D:\\05.Q_utility\\ADB_20131023\\fastboot.exe flash passport passport_FactoryDLTool");
		bFastbootDL(csExePath);


	//	bFastbootDL_3("fastboot", " flash passport passport_FactoryDLTool", "4", NULL, NULL, 10 *1000);
		//if (!MyExecAdbCmd(NULL, "", "kill-server"))
	///	bDLFlag = bFastbootDL_New2(NULL, csExePath, NULL);
	//	Sleep(10000);

		//csExePath.Format(_T(" flash %s %s -s QPHONE%d"),csImgType, csImgPath, nGetADBPort);
	//	csExePath.Format(_T(" flash %s passport_FactoryDLTool -s QPHONE%d"),csImgType, csImgPath, nGetADBPort);
	//	bDLFlag = bFastbootDL_New2(NULL, csExePath, NULL);
	//	Sleep(10000);	
	...
	bFastbootDL_New(csExePath, szOutPullCommand, szErrorCodePullCommand);
	*/
	...
	for (int i = 0 ; i < mapFORMAT_IMGS.size() ; i++){
		...
		csExePath.Format(_T("\"%s\" format \"%s\" -s QPHONE%d"),str_FastbootPath, csImgType, nGetADBPort);
		bDLFlag = bFastbootDL(csExePath);
	...
	for (int i = 0 ; i < mapERASE_ALL_IMGS.size() ; i++){
		...
		csExePath.Format(_T("\"%s\" erase \"%s\" -s QPHONE%d"),str_FastbootPath, csImgType, nGetADBPort);
		bDLFlag = bFastbootDL(csExePath);
...
bool CDownload8994::bFastbootDL(CString folderPath){
	...
	DWORD dw_timeout;
	dw_timeout = 5 * 1000;
	//dw_waitResult = ::WaitForMultipleObjects(_countof(pi_bFastbootDL.hProcess), &pi_bFastbootDL.hProcess, true, dw_timeout);
...
int CDownload8994::FindADBdevice(int nADBPort)
{
	...
	while (1)
	{
			...
			&&(0 == _strnicmp("\\\\?\\usb", pDeviceDetailData->DevicePath, 7))) // We only care about usb device
		{
			...
			{/*return specific port */
				if( nADBPort = atoi( sz_id)) return nADBPort;
			}
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8994::DMSSDL(void)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8994::StreamingDL(bool b_speedUp, DownloadProtocol nDLPROTOCOL)
{
	using namespace PLATFORM_TOOL;
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8994::SetFastBootFlag(void)
{
	bool b_res = false;
...
*****************************************************************************/
bool CDownload8994::DDRTest(void)
{
	using namespace PLATFORM_TOOL;
...
* 1.0          Alex.Chen     2011/09/28          First version             
*****************************************************************************/
bool CDownload8994::ReadDDRTestReslult(void)
{
	using namespace PLATFORM_TOOL;
...
*****************************************************************************/
bool CDownload8994::StreamingDLIRAMLog(void)
{
	bool b_res = true;
	bool b_speedUp = false;
	char * pEnd;
...
bool CDownload8994::StreamingDLeMMCLog(int nSector)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
int CDownload8994::GetComport(void)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
void CDownload8994::SetIsOveridePartition(bool b_overridePartition)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
void CDownload8994::SetResetMode(bool b_reset)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8994::FastbootDownload(void)
{
...
*****************************************************************************/
bool CDownload8994::GetOemMainVersion(CString& str_parameter)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8994::FastbootFlashCmd(const std::string& str_parameter)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8994::FastbootFormatCmd(const std::string& str_parameter)
{
	/* Check input parameter */
	if (str_parameter == _T("")) 
	{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8994::ADBFlash(const char* sz_parmeter, const void* p_data, unsigned long long l_fileLen)
{
	/* Check input parameter */
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
void* CDownload8994::LoadFile(const char* sz_fileName, unsigned long long* i_fileSize)
{
	/* 线程同步处理 */
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
ADBDevice* CDownload8994::GetADBDevice(const char* sz_comPort)
{
	AddMsg("CDownload8994 GetADBDevice Start ...", DownloadStep::None, 100);
...
*****************************************************************************/
bool CDownload8994::GetADB(void)
{
...
*****************************************************************************/
bool CDownload8994::SetFTDMode(void)
{
...
* 1.0          Alex.Chen     2011/09/19             First version             
*****************************************************************************/
bool CDownload8994::EraseSBL1(void)
{
...
bool CDownload8994::EraseUserFat(void)
{
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8994::WriteCheckSum(CString str_imageFilePathName)
{
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8994::ReadCheckSumFile(void)
{
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8994::BackupNVPartition(void)
{
...
bool CDownload8994::ADBFlashAll(const char* sz_parmeter, const char* sz_fileName)
{
	/* Check input parameter */
...
bool CDownload8994::ADBFlashAllLargeSize(const char* sz_parmeter, const char* sz_fileName)
{
	char sz_imgae[64] = {0};
	CString str_message;
	int64_t sz64;
...
bool CDownload8994::ExecuteFastboot(char* sz_command)
{
	/* Check input parameter */
	if ((sz_command == NULL)) 
	{
...
bool CDownload8994::ExecuteFastbootOut(char* sz_command, CString& out)
{
	/* Check input parameter */
	if ((sz_command == NULL)) 
	{
...
bool CDownload8994::LoadSparseLibDLL(){
