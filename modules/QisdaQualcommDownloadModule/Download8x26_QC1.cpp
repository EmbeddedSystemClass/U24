...
#include "StdAfx.h"
#include "Download8x26_QC1.h"
#include "../../lib/Qualcomm/QDART4813/inc/QLib.h"
...
CCriticalSection CDownload8x26_QC1::m_obj_loadcheckSumCritSection;

CDownload8x26_QC1::CDownload8x26_QC1(const TSTRING& str_deviceName, const std::string& str_platform)
...
CDownload8x26_QC1::CDownload8x26_QC1(int i_COMPort, CString str_multiDLFlag, const std::string& str_platform)
...
CDownload8x26_QC1::~CDownload8x26_QC1(void)
{
	if (m_p_adbDevice) 
	{
	...
	if (m_hQMSL_MSVC9R != NULL)
	{
		FreeLibrary (m_hQMSL_MSVC9R);
		m_hQMSL_MSVC9R = NULL;
	}
...
*****************************************************************************/
bool CDownload8x26_QC1::MultiDownload(bool b_speedUp, bool b_reOpenAfterReset, DownloadProtocol nDLPROTOCOL) 
{
	bool b_result = false;
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8x26_QC1::Download(bool b_speedUp, bool b_reOpenAfterReset, DownloadProtocol nDLPROTOCOL) 
{
	...
	if (m_i_DDRCheck == 1)
	{
		...
		b_reConnect = obj_ReOpenComport.ReOepnComport(g_i_8x26ReOpenComPortAfterReset, false, 15, 1000);
		...
		b_reConnect = obj_ReOpenComport.ReOepnComport(g_i_8x26ReOpenComPortAfterGo, false, 15, 1000);
		...
		if (!StreamingHello(b_speedUp, g_i_8x26HelloRetryTimes, g_i_8x26HelloTimeout, nDLPROTOCOL)) 
		{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8x26_QC1::DMSSDL(void)
{
	using namespace PLATFORM_TOOL;

	if (!DMSSNop(g_i_8x26NopNopRetryTimes, g_i_8x26NopTimeout)) 
	{
...
/*****************************************************************************
* Function name: QMSLCallBack_QC1
...
*****************************************************************************/
void QMSLCallBack_QC1(HANDLE hQMSLContext,
      char	*sMsg,
      unsigned short iMsgSize)
{
	...
	FILE *pFile = NULL;
	pFile = fopen("D:\\Log\\UI_RD\\QMSLCallBack_QC1.log", "wb");
...
* 1.0          Erin.Chang     2013/09/18          First version             
*****************************************************************************/
bool CDownload8x26_QC1::SAHARADL(void)
{
	using namespace PLATFORM_TOOL;

	m_hQMSL_MSVC9R = LoadLibrary("QMSL_MSVC9R.dll");
	if(m_hQMSL_MSVC9R == NULL)
	{
		return false;
	}

	pQLIB_QPHONEMS_ConnectServer_Sahara ConnectServer_Sahara = (pQLIB_QPHONEMS_ConnectServer_Sahara)GetProcAddress(m_hQMSL_MSVC9R, "QLIB_QPHONEMS_ConnectServer_Sahara");
	pQLIB_QPHONEMS_Sahara_FlashProgrammer Sahara_FlashProgrammer = (pQLIB_QPHONEMS_Sahara_FlashProgrammer)GetProcAddress(m_hQMSL_MSVC9R, "QLIB_QPHONEMS_Sahara_FlashProgrammer");
	pQLIB_DisconnectServer DisconnectServer = (pQLIB_DisconnectServer)GetProcAddress(m_hQMSL_MSVC9R, "QLIB_DisconnectServer");

	unsigned int iPort = m_i_COMPort;
	HANDLE m_hPhone = NULL;
	unsigned long saharaVer = 0;
	unsigned char cret;
	swdlQPHONEMSCB SWDL_QPHONEMS_CB = QMSLCallBack_QC1;

	if(ConnectServer_Sahara != NULL)
	{
		m_hPhone = ConnectServer_Sahara(iPort, &saharaVer, SWDL_QPHONEMS_CB);
...
bool CDownload8x26_QC1::IsPathExist(const CString& path)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8x26_QC1::StreamingDL(bool b_speedUp, DownloadProtocol nDLPROTOCOL)
{
	using namespace PLATFORM_TOOL;
	...
	if (!StreamingDLMultiImage(Q8K_OPEN_MULTI_MODE_EMMC_USER,  m_str_msimage,  b_speedUp, g_nRetryTimes, g_nTimeout, g_nTimeout, g_i_8x26CloseImageTimeout)) 
	{
	...
	if (!StreamingStrWrite(byteData, 1, 0x400000, b_speedUp, g_nRetryTimes, g_nTimeout))
	{
	...
	if (m_b_reOpenAfterReset) 
	{
		...
		b_res = obj_ReOpenComport.ReOepnComport(g_i_8x26ReOpenComPortAfterReset, false, 15, 1000);
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8x26_QC1::SetFastBootFlag(void)
{
	bool b_res = false;
...
*****************************************************************************/
bool CDownload8x26_QC1::DDRTest(void)
{
	using namespace PLATFORM_TOOL;
...
* 1.0          Alex.Chen     2011/09/28          First version             
*****************************************************************************/
bool CDownload8x26_QC1::ReadDDRTestReslult(void)
{
	using namespace PLATFORM_TOOL;
...
*****************************************************************************/
bool CDownload8x26_QC1::StreamingDLIRAMLog(void)
{
	bool b_res = true;
	bool b_speedUp = false;
	char * pEnd;
...
bool CDownload8x26_QC1::GetCOMPortDevByWDK(std::map<std::string, std::string>& map_strstrCOMDevice)
{
...
bool CDownload8x26_QC1::DetectDiagPort()
{
...
bool CDownload8x26_QC1::StreamingDLeMMCLog(int nSector)
{
...
* 1.0           Alex.Chen     2011/01/19          First version             
*****************************************************************************/
CString CDownload8x26_QC1::GetErrorCode(void) 
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
int CDownload8x26_QC1::GetComport(void)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
void CDownload8x26_QC1::SetIsOveridePartition(bool b_overridePartition)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
void CDownload8x26_QC1::SetResetMode(bool b_reset)
{
...
* 1.0           Alex.Chen     2011/01/19            First version             
*****************************************************************************/
bool CDownload8x26_QC1::SetImageFilePath(const int i_imageType, const std::string& str_filePath)
{
	bool b_res = false;
	...
	switch(i_imageType)
	{
	case IMAGE_TYPE_8x26::Q8K_HEX:
		m_str_hex = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_MSIMAGE:
		m_str_msimage = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_PARTITIONBIN:
		m_str_partitionBin = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_SBL1:
		m_str_sbl1 = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_SDI:
		m_str_sdi = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_RPM:
		m_str_rpm = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_TZ:
		m_str_tz = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_APPBOOT:
		m_str_appboot = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_SBL1BAK:
		m_str_sbl1bak = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_RPMBAK:
		m_str_rpmbak = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_TZBAK:
		m_str_tzbak = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_ABOOTBAK:
		m_str_abootbak = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_BOOT:
		m_str_boot = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_RECOVERY:
		m_str_recovery = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_SYSTEM:
		m_str_system = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_CACHE:
		m_str_cache = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_PERSIST:
		m_str_persist = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_QGLOGFILTER:
		m_str_qlogfilter = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_QGLOG:
		m_str_qglog = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_NONHLOS:
		m_str_nonhlos = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_USERDATA:
		m_str_userdata = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_DRM:
		m_str_drm = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_QCUST:
		m_str_qcust = str_filePath;
		break;

	case IMAGE_TYPE_8x26::Q8K_PASSPORT:
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8x26_QC1::FastbootDownload(void)
{
...
*****************************************************************************/
bool CDownload8x26_QC1::GetOemMainVersion(CString& str_parameter)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8x26_QC1::FastbootFlashCmd(const std::string& str_parameter)
{
	...
	else if (str_parameter == _T("sdi")) 
	{
		str_imageFilePath = m_str_sdi;
	}
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8x26_QC1::FastbootFormatCmd(const std::string& str_parameter)
{
	/* Check input parameter */
	if (str_parameter == _T("")) 
	{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8x26_QC1::ADBFlash(const char* sz_parmeter, const void* p_data, unsigned long long l_fileLen)
{
	/* Check input parameter */
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
void* CDownload8x26_QC1::LoadFile(const char* sz_fileName, unsigned long long* i_fileSize)
{
	/* 线程同步处理 */
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
ADBDevice* CDownload8x26_QC1::GetADBDevice(const char* sz_comPort)
{
	AddMsg("CDownload8x26_QC1 GetADBDevice Start ...", DownloadStep::None, 100);
...
*****************************************************************************/
bool CDownload8x26_QC1::GetADB(void)
{
...
*****************************************************************************/
bool CDownload8x26_QC1::SetFTDMode(void)
{
...
*****************************************************************************/
bool CDownload8x26_QC1::Reboot(void)
{
...
* 1.0          Alex.Chen     2011/09/19             First version             
*****************************************************************************/
bool CDownload8x26_QC1::EraseSBL1(void)
{
...
bool CDownload8x26_QC1::EraseUserFat(void)
{
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8x26_QC1::WriteCheckSum(CString str_imageFilePathName)
{
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8x26_QC1::ReadCheckSumFile(void)
{
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8x26_QC1::ReadFA(char* sz_FAData)
{
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8x26_QC1::WriteFA(char* sz_FAData)
{
...
*****************************************************************************/
bool CDownload8x26_QC1::ReadFASector(int i_sectorNum, char *sz_sectorData, int i_sectorSize)
{
	bool b_speedUp = false;
...
* 1.0          Alex.Chen        2011/11/23          First version             
*****************************************************************************/
bool CDownload8x26_QC1::WriteFASector(int i_sectorNum, char *sz_sectorData, int i_sectorSize)
{
	AddMsg("Write FA.", DownloadStep::None, 100);
...
bool CDownload8x26_QC1::runWriteFASector(char *sz_sectorData){
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8x26_QC1::BackupNVPartition(void)
{
...
bool CDownload8x26_QC1::SpiltString(CString str_sourceString, CString str_delimiter, CStringArray& stra_strArg)
{
	/* Check Input */
...
bool CDownload8x26_QC1::ADBFlashAll(const char* sz_parmeter, const char* sz_fileName)
{
	/* Check input parameter */
...
bool CDownload8x26_QC1::ADBFlashAllLargeSize(const char* sz_parmeter, const char* sz_fileName)
{
	char sz_imgae[64] = {0};
	CString str_message;
	int64_t sz64;
	...
	if (limit) {
		...
		if (s == NULL) {
			...
			while (*s) {
				...
				str_message.Format("sparse_file_len[%d]:: %d", nFileDivision++, ( sz64 / 1024) );
				AddMsg(str_message , DownloadStep::None, 100);
				sprintf(sz_imgae, "Fastboot download:%s", sz_parmeter);
...
bool CDownload8x26_QC1::ExecuteFastboot(char* sz_command)
{
	/* Check input parameter */
	if ((sz_command == NULL)) 
	{
...
bool CDownload8x26_QC1::ExecuteFastbootOut(char* sz_command, CString& out)
{
	/* Check input parameter */
	if ((sz_command == NULL)) 
	{
...
bool CDownload8x26_QC1::LoadSparseLibDLL(){
