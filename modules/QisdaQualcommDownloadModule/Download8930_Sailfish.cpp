...
#include "StdAfx.h"
#include "Download8930_Sailfish.h"

CCriticalSection CDownload8930_Sailfish::m_obj_loadcheckSumCritSection;

CDownload8930_Sailfish::CDownload8930_Sailfish(const TSTRING& str_deviceName, const std::string& str_platform)
...
CDownload8930_Sailfish::CDownload8930_Sailfish(int i_COMPort, CString str_multiDLFlag, CString str_toolMode, const std::string& str_platform)
:CDLFormatCommand(COMDeviceName(i_COMPort), str_platform), m_b_reOpenAfterReset(false), m_b_resetMode(true), m_b_overridePartition(false), m_p_adbDevice(NULL)
{
	...
	m_str_toolMode = str_toolMode;
}

CDownload8930_Sailfish::~CDownload8930_Sailfish(void)
{
	if (m_p_adbDevice) 
	{
...
*****************************************************************************/
bool CDownload8930_Sailfish::MultiDownload(bool b_speedUp, bool b_reOpenAfterReset, DownloadProtocol nDLPROTOCOL) 
{
	bool b_result = false;
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::Download(bool b_speedUp, bool b_reOpenAfterReset, DownloadProtocol nDLPROTOCOL) 
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::DMSSDL(void)
{
...
bool CDownload8930_Sailfish::IsPathExist(const CString& path)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::StreamingDL(bool b_speedUp, DownloadProtocol nDLPROTOCOL)
{
	using namespace PLATFORM_TOOL;
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::SetFastBootFlag(void)
{
	bool b_res = false;
...
*****************************************************************************/
bool CDownload8930_Sailfish::DDRTest(void)
{
	using namespace PLATFORM_TOOL;
...
* 1.0          Alex.Chen     2011/09/28          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::ReadDDRTestReslult(void)
{
	using namespace PLATFORM_TOOL;
...
*****************************************************************************/
bool CDownload8930_Sailfish::StreamingDLIRAMLog(void)
{
	bool b_res = true;
	bool b_speedUp = false;
	char * pEnd;
...
* 1.0           Alex.Chen     2011/01/19          First version             
*****************************************************************************/
CString CDownload8930_Sailfish::GetErrorCode(void) 
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
int CDownload8930_Sailfish::GetComport(void)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
void CDownload8930_Sailfish::SetIsOveridePartition(bool b_overridePartition)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
void CDownload8930_Sailfish::SetResetMode(bool b_reset)
{
...
* 1.0           Alex.Chen     2011/01/19            First version             
*****************************************************************************/
bool CDownload8930_Sailfish::SetImageFilePath(const int i_imageType, const std::string& str_filePath)
{
	bool b_res = false;
	...
	case IMAGE_TYPE_8960::Q8K_QGLOG:
		...
		break;
	case IMAGE_TYPE_8960::Q8K_SAILFISH001:
		m_str_sailfish001 = str_filePath;
		break;
	case IMAGE_TYPE_8960::Q8K_SAILFISH002:
		m_str_sailfish002 = str_filePath;
		break;
	case IMAGE_TYPE_8960::Q8K_SAILFISH003:
		m_str_sailfish003 = str_filePath;
		break;
	case IMAGE_TYPE_8960::Q8K_SAILFISH004:
		m_str_sailfish004 = str_filePath;
		break;
	case IMAGE_TYPE_8960::Q8K_SAILFISH005:
		m_str_sailfish005 = str_filePath;
		break;
	case IMAGE_TYPE_8960::Q8K_SAILFISH006:
		m_str_sailfish006 = str_filePath;
		break;
	case IMAGE_TYPE_8960::Q8K_SAILFISH007:
		m_str_sailfish007 = str_filePath;
		break;
	case IMAGE_TYPE_8960::Q8K_SAILFISH008:
		m_str_sailfish008 = str_filePath;
		break;
	case IMAGE_TYPE_8960::Q8K_SAILFISH009:
		m_str_sailfish009 = str_filePath;
		break;
	case IMAGE_TYPE_8960::Q8K_SAILFISH010:
		m_str_sailfish010 = str_filePath;
		break;
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::FastbootDownload(void)
{
	...
	for (int i = 0; i< m_vEraseList.size(); i++)
	{
		CString strCommand;
		strCommand.Format("erase:%s", m_vEraseList[i].c_str());

		if (!ExecuteFastboot(strCommand.GetBuffer()))
		{
			if(m_vEraseList[i].compare("modemst1") == 0)
			{
				m_str_errorCode = DLErr_Erase_Modemst1_Fail;
			}
			else if(m_vEraseList[i].compare("modemst2") == 0)
			{
				m_str_errorCode = DLErr_Erase_Modemst2_Fail;
			}
			else if(m_vEraseList[i].compare("recovery") == 0)
			{
				m_str_errorCode = DLErr_Erase_Recovery_Fail;
			}
			else if(m_vEraseList[i].compare("cache") == 0)
			{
				m_str_errorCode = DLErr_Erase_Cache_Fail;
			}
			else if(m_vEraseList[i].compare("swap") == 0)
			{
				m_str_errorCode = DLErr_Erase_Swap_Fail;
			}
	...
	for (int i = 0; i< m_vFormatList.size(); i++)
	{
		if (!FastbootFormatCmd(m_vFormatList[i].c_str()))
		{
			if(m_vFormatList[i].compare("Qglog") == 0)
			{
				m_str_errorCode = DLErr_Format_Qglog_Fail;
			}
			else if(m_vFormatList[i].compare("drm") == 0)
			{
...
*/
/* marked for Sailfish OS
	// Boston 璸购эノ format  flash
...
*/
	...
	//**** for sailfish OS image ****//
	if (!FastbootFlashCmd("Sailfish001")) 
	{
		m_str_errorCode = DLErr_DL_Sailfish_Fail;
		return false;
	}

	if (!FastbootFlashCmd("Sailfish002")) 
	{
		m_str_errorCode = DLErr_DL_Sailfish_Fail;
		return false;
	}

	if (!FastbootFlashCmd("Sailfish003")) 
	{
		m_str_errorCode = DLErr_DL_Sailfish_Fail;
		return false;
	}

	if (!FastbootFlashCmd("Sailfish004")) 
	{
		m_str_errorCode = DLErr_DL_Sailfish_Fail;
		return false;
	}

	if (!FastbootFlashCmd("Sailfish005")) 
	{
		m_str_errorCode = DLErr_DL_Sailfish_Fail;
		return false;
	}

	if (!FastbootFlashCmd("Sailfish006")) 
	{
		m_str_errorCode = DLErr_DL_Sailfish_Fail;
		return false;
	}

	if (!FastbootFlashCmd("Sailfish007")) 
	{
		m_str_errorCode = DLErr_DL_Sailfish_Fail;
		return false;
	}

	if (!FastbootFlashCmd("Sailfish008")) 
	{
		m_str_errorCode = DLErr_DL_Sailfish_Fail;
		return false;
	}

	if (!FastbootFlashCmd("Sailfish009")) 
	{
		m_str_errorCode = DLErr_DL_Sailfish_Fail;
		return false;
	}

	if (!FastbootFlashCmd("Sailfish010")) 
	{
		m_str_errorCode = DLErr_DL_Sailfish_Fail;
		return false;
	}
	//**** end of sailfish OS image ****//
	...
	if (m_str_DLMode == DOWNLOAD_MODE_PREDL)
	{
		...
		/* // Marked for the request from L1 team at 2013/8/end
	...
	{/* marked for Boston
		...
		{
			m_str_errorCode = DLErr_Erase_Inner_SD_Fail;
			return false;
		}*/
...
/* marked for Sailfish OS
	if (!ExecuteFastboot(_T("erase:device-info")))
	{
		//m_str_errorCode = DLErr_Erase_Deviceinfo_Fail;
		//return false;
	}
*/
...
*****************************************************************************/
bool CDownload8930_Sailfish::GetOemMainVersion(CString& str_parameter)
{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::FastbootFlashCmd(const std::string& str_parameter)
{
	...
	else if (str_parameter == _T("Sailfish001")) 
	{
		str_imageFilePath = m_str_sailfish001;
	}
	else if (str_parameter == _T("Sailfish002")) 
	{
		str_imageFilePath = m_str_sailfish002;
	}
	else if (str_parameter == _T("Sailfish003")) 
	{
		str_imageFilePath = m_str_sailfish003;
	}
	else if (str_parameter == _T("Sailfish004")) 
	{
		str_imageFilePath = m_str_sailfish004;
	}
	else if (str_parameter == _T("Sailfish005")) 
	{
		str_imageFilePath = m_str_sailfish005;
	}
	else if (str_parameter == _T("Sailfish006")) 
	{
		str_imageFilePath = m_str_sailfish006;
	}
	else if (str_parameter == _T("Sailfish007")) 
	{
		str_imageFilePath = m_str_sailfish007;
	}
	else if (str_parameter == _T("Sailfish008")) 
	{
		str_imageFilePath = m_str_sailfish008;
	}
	else if (str_parameter == _T("Sailfish009")) 
	{
		str_imageFilePath = m_str_sailfish009;
	}
	else if (str_parameter == _T("Sailfish010")) 
	{
		str_imageFilePath = m_str_sailfish010;
	}
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::FastbootFormatCmd(const std::string& str_parameter)
{
	/* Check input parameter */
	if (str_parameter == _T("")) 
	{
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::ADBFlash(const char* sz_parmeter, const void* p_data, unsigned long long l_fileLen)
{
	/* Check input parameter */
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
void* CDownload8930_Sailfish::LoadFile(const char* sz_fileName, unsigned long long* i_fileSize)
{
	/* 线程同步处理 */
...
* 1.0          Alex.Chen     2011/01/19          First version             
*****************************************************************************/
ADBDevice* CDownload8930_Sailfish::GetADBDevice(const char* sz_comPort, int nRetryCount)
{
	/* Get ADB by the COM port */
	if (m_p_adbDevice == NULL)
	{
		m_p_adbDevice = new ADBDevice(adb_class_id, 0, 0, DEVICE_INTERFACE_ID, sz_comPort, nRetryCount);
		if(m_p_adbDevice) 
		{
...
*****************************************************************************/
bool CDownload8930_Sailfish::GetADB(int nRetryCount)
{
	...
	/* Find ADB */
	if (!GetADBDevice(sz_comPort, nRetryCount)) 
	{
...
*****************************************************************************/
bool CDownload8930_Sailfish::SetFTDMode(void)
{
...
*****************************************************************************/
bool CDownload8930_Sailfish::Reboot(void)
{
...
* 1.0          Alex.Chen     2011/09/19             First version             
*****************************************************************************/
bool CDownload8930_Sailfish::EraseSBL1(void)
{
...
bool CDownload8930_Sailfish::EraseUserFat(void)
{
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::WriteCheckSum(CString str_imageFilePathName)
{
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::ReadCheckSumFile(void)
{
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::ReadFA(char* sz_FAData)
{
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::WriteFA(char* sz_FAData)
{
...
*****************************************************************************/
bool CDownload8930_Sailfish::ReadFASector(int i_sectorNum, char *sz_sectorData, int i_sectorSize)
{
	bool b_speedUp = false;
...
* 1.0          Alex.Chen        2011/11/23          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::WriteFASector(int i_sectorNum, char *sz_sectorData, int i_sectorSize)
{
	AddMsg("Write FA.", DownloadStep::None, 100);
...
* 1.0          Alex.Chen        2011/09/27          First version             
*****************************************************************************/
bool CDownload8930_Sailfish::BackupNVPartition(void)
{
...
bool CDownload8930_Sailfish::SpiltString(CString str_sourceString, CString str_delimiter, CStringArray& stra_strArg)
{
	/* Check Input */
...
bool CDownload8930_Sailfish::ADBFlashAll(const char* sz_parmeter, const char* sz_fileName)
{
	/* Check input parameter */
		...
		if (m_p_adbDevice->Write(sz_command, strlen(sz_command), 60000)) 
		{
			...
			if (m_p_adbDevice->ReadResponse(60000) == RESPONSE::DATA) 
			{
				...
				if (m_p_adbDevice->ReadResponse(120000) == RESPONSE::OKAY) 
				{
					/* flash command --- ""flash:%s""  example:"flash boot" */
					char sz_command[64] = {0};
					if(NULL != strstr(sz_parmeter, "Sailfish"))
					{
						sprintf(sz_command, "flash:%s", "sailfish");
					}
					else
					{
						sprintf(sz_command, "flash:%s", sz_parmeter);
...
bool CDownload8930_Sailfish::ExecuteFastboot(char* sz_command)
{
	/* Check input parameter */
	if ((sz_command == NULL)) 
	{
	...
	if (m_p_adbDevice->Write(sz_command, strlen(sz_command), 6000)) 
	{
		...
		while (1)
		{
			...
			if (ret == RESPONSE::OKAY || ret == RESPONSE::FAIL || ret == RESPONSE::TIMEOUT) 
			{
				break;
			}
		}
...
bool CDownload8930_Sailfish::ExecuteFastbootOut(char* sz_command, CString& out)
{
	/* Check input parameter */
	if ((sz_command == NULL)) 
	{
