...
#include "..\DownloadDLL\NV_ItemHolder.h"
#include "..\Common\CommonUtil\ErrorDefine.h"
...
{
public:
	CNVItem(const TSTRING& strDevice);
	CNVItem(int nCOMPort);
	virtual ~CNVItem(void);

	TSTRING GetBTAddr();
	virtual bool WriteBTaddr(const string& strBTAddr);
	TSTRING GetBTAddr_C7();
	bool GetNv_T47(char *szNV30002, char *szNV30003, char *szNV30004);
	bool GetNv_Detroit(char *szNV30015, char *szNV30016, char *szNV30017);
	bool GetNv_CheckSum(char *szNV30015, char *szNV30016, char *szNV30017);
	virtual bool WriteBTaddr_C7(const string& strBTAddr);
	TSTRING GetWLANAddr();
	virtual bool WriteWLANaddr(const string& strWLANAddr);
 	TSTRING GetWLANAddr_C7();
	virtual bool WriteWLANaddr_C7(const string& strWLANAddr);
    TSTRING GetIMEI();
	virtual bool WriteIMEI(const string& strIMEI);
	TSTRING GetMEID();
	virtual bool WriteMEID(const string& strMEID);

	//--------------------- Austin func.---------------------
	bool AustinWriteServiceTag(const string& strServiceTag);
	TSTRING AustinGetServiceTag();
	bool AustinWriteDRMKey(const string& strDRMKey);
	TSTRING AustinGetDRMKey();
	bool AustinWriteBTAddr(const string& strBTAddr);
	TSTRING AustinGetBTAddr();
	bool AustinWriteWLANAddr(const string& strWLANAddr);
	TSTRING AustinGetWLANAddr();
	...
	bool RestoreBRT(const std::string& strInBRTFileName);
	...
	//SUGNumber is also transciver number
	bool ReadSUGNumber(char* szSUGNumber, int& nBufferSize,int nVer);
	bool WriteSUGNumber(char* szSUGNumber,int nVer);
	...
	bool ResetProcessInfo (char* szProcessIndex);
	...
	bool C7FreezeOTP(int nRequestType, int& nStatus);//LION 2012 1.1
    bool ClearActivationData(int nTimeout = 3000);
	bool ClearActivationDataForLavernock(int nTimeout = 3000);
	...
	bool CheckSIMCardExistence_ICE(int &nType, int nTimeout = 3000);
	...
	bool ReadQFuseState (unsigned long* preg1, unsigned long* preg2, unsigned long* preg3, unsigned long* preg4, unsigned long* preg5, bool &bSuccess);
    bool SetOnline();
    bool SetOffline();
    bool ResetHandset();
    bool SetFTM();
    ...
    bool ProximitySensorTest();
};



class CNVItem_CLASS_API CCFCDiagCommand : public CNVItem, public CRefObject, public CLastError
{
public:
	CCFCDiagCommand(const TSTRING& strDevice);
	...
	bool Restart_UsePNP(int nRestartWaitingTime , int nNVTimeOut = g_nDEFAULT_CMD_TIME_OUT);
#endif
	...
	bool WriteWLANAddr(char* szAddress, int nTimeOut=g_nDEFAULT_CMD_TIME_OUT);
	...
	bool GetLavernockVersion(int& nVarientVersion, int nTimeOut=g_nDEFAULT_CMD_TIME_OUT);

	// MMI setting
	bool TurnOnOffEM(bool bOpen, int nTimeOut=g_nDEFAULT_CMD_TIME_OUT);
	bool SetRTC(const COleDateTime& time,int nTimeOut=g_nDEFAULT_CMD_TIME_OUT);
	bool SetRTC_For71M(int nTimeOut=g_nDEFAULT_CMD_TIME_OUT);
	...
	bool RestoreBRT(const std::string& strOutputBRTFileName){ return CNVItem::RestoreBRT(strOutputBRTFileName); }
public:
