...
bool CBatteryCapacity::BatteryCapacityDetectEmpty()
{
	...
	SetPICSData(_T("BattaryCheck"), m_strMeasured);
	/* test result */
	if (b_res)
	{
		m_strMessage = MakeStr("PASS, BatteryCapacity = %s%%", m_strMeasured.c_str());
		m_strResult = "PASS";
		TraceLog(MSG_INFO, str_msg);
	}
	else
	{
		m_strMessage = MakeStr("FAIL, BatteryCapacity = %s%%", m_strMeasured.c_str());
		m_strResult = "FAIL";
		TraceLog(MSG_INFO, str_msg);
	}
	FactoryLog();
