...
QISDA_EFS_API bool EFS_ReadFile(const char*szPort,const char *sFileSrc, const char *sFileDst)
{
	...
	if (! PathFileExists(sFileDst))
	{
		char chTemp[MAX_PATH];
		strcpy (chTemp, sFileDst);
		PathRemoveFileSpec (chTemp);
		CreateDirectory (chTemp, NULL);
	}
...
QISDA_EFS_API bool EFS_WriteFile(const char*szPort,const char *sFileSrc, const char *sFileDst)
{
	bool bRes = false;
	long iErrorCode = 0;

	// check sFileSrc	
	FILE* pfile = fopen (sFileSrc, _T("r"));
	if (! pfile)
	{
		return false;
	}
	...
	CEFs efs(::atoi(szPort));
	bRes = efs.EfsWriteFile(sFileSrc,sFileDst,iErrorCode);
	return bRes;
}
...
QISDA_EFS_API bool EFS_DeleteFilesInFolder(const char*szPort, char *szFolderPath)
{
	bool bRes = false;	
	CEFs efs(::atoi(szPort));
	...
	bool bListFile = efs.EfsDirectoryList(szFolderPath, &DirList, nSize,entryType,iErrorCode);
	if(bListFile)
	{
		for(std::vector<Efs2DirItem>::iterator Iter = DirList.begin(); !bRes && Iter != DirList.end(); ++Iter)
		{
			if(! efs.EfsDeleteFile(Iter->name, iErrorCode))
				break;
		}
	...
	if(bListFile)
		efs.EFsRemoveFolder(szFolderPath, iErrorCode);
