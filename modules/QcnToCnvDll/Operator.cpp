...
int COperator:: GetNVItem(int mNVItemID)
{
	...
	case DU:
	case QCN:
		for(int i = 0; i < (int) m_NVItemArray.size(); i++)
		{
...
{
	return AddToNVArrar(hDuckFile, intData, -1, NVItemArray);
}

bool COperator::AddToNVArrar(FILE* hDuckFile, int intData, int iIndex, std::vector<CNVItemData>& NVItemArray)
{
	CNVItemData nvItem;
	GetNVBlock(hDuckFile,intData,nvItem);

	nvItem.SetIndex(iIndex);
		...
		{	 
			...
			{//It's NV file or Array
				...
				{//It's NV array
					...
					AddToNVArrar(hDuckFile, intData, _ttoi(strNVIndex), NVItemArray);
				}
				else
				{//It's NV file
...
int COperator::SaveCnvFileToQcnFileFunction(const CString& strCNV,const CString& strQCN)
{
	...
	{
		int nItemCount = m_NVItemArrayReverse.GetSize();
