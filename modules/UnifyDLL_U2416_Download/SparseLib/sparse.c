...
#include <fcntl.h>//lion

SPARSEDLLAPI struct sparse_file **load_sparse_files_dll(const char *fname, int max_size)//lion
{
    int fd;
...
struct sparse_file **load_sparse_files(const char *fname, int max_size)//lion
{
    int fd;
    struct sparse_file *s;
    int files;
...
SPARSEDLLAPI int sparse_file_callback_dll(struct sparse_file *s, bool sparse, bool crc,
...
SPARSEDLLAPI int64_t sparse_file_len_dll(struct sparse_file *s, bool sparse, bool crc)
{
	int ret;
	...
	//fprintf(stderr, "in sparse_file_len count \n"); // lion 
	...
	//fprintf(stderr, "sparse_file_len1 count = '%lld'  \n", count);lion
	return count;
}
