...
typedef unsigned __int64 off64_t; //lion
		...
		uint32_t *crc32)
{
	...
	if (crc32) {
		...
		//lseek64(fd, len, SEEK_CUR);lion
		_lseeki64(fd, len, SEEK_CUR);
	}
...
static int sparse_file_read_sparse(struct sparse_file *s, int fd, bool crc)
{
	...
	if (sparse_header.file_hdr_sz > SPARSE_HEADER_LEN) {
		 ...
		 * we expected.
		 */
		//lseek64(fd, sparse_header.file_hdr_sz - SPARSE_HEADER_LEN, SEEK_CUR);lion
		_lseeki64(fd, sparse_header.file_hdr_sz - SPARSE_HEADER_LEN, SEEK_CUR);
	}
	...
	for (i = 0; i < sparse_header.total_chunks; i++) {
		...
		if (sparse_header.chunk_hdr_sz > CHUNK_HEADER_LEN) {
			 ...
			 * we expected.
			 */
			//lseek64(fd, sparse_header.chunk_hdr_sz - CHUNK_HEADER_LEN, SEEK_CUR);lion
			_lseeki64(fd, sparse_header.chunk_hdr_sz - CHUNK_HEADER_LEN, SEEK_CUR);
		}

		//offset = lseek64(fd, 0, SEEK_CUR);lion
...
struct sparse_file *sparse_file_import(int fd, bool verbose, bool crc)
{
	...
	//ret = lseek64(fd, 0, SEEK_SET);lion
	ret = _lseeki64(fd, 0, SEEK_SET);
...
struct sparse_file *sparse_file_import_auto(int fd, bool crc)
{
	...
	if (s) {
		//fprintf(stderr, "sparse_file_import_auto return s = '%lu' \n", s);//lion
		return s;
	}

	//len = lseek64(fd, 0, SEEK_END);lion
