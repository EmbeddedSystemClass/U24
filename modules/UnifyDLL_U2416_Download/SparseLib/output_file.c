...
//typedef unsigned long long off64_t;//lion
...
static int file_skip(struct output_file *out, int64_t cnt)
{
	...
	//ret = lseek64(outn->fd, cnt, SEEK_CUR);_lseeki64 
...
static struct output_file_ops file_ops = { //lion
	file_open,
	file_skip,
	file_pad,
	file_write,
		...
		int chunks, int crc)
{
	...
	if (!outc) {
		...
		fprintf(stderr, "malloc struct outc  \n");
		return NULL;
	}
...
//#else
	...
	//pos = lseek64(fd, offset, SEEK_SET);lion
		...
		const char *file, int64_t offset)
{
	...
	int file_fd = _open(file, O_RDONLY | O_BINARY);
	if (file_fd < 0) {
		return -errno;
	}
