...
CMD5Checksum::~CMD5Checksum(void)
{
}
...
bool CMD5Checksum::CheckDirMD5( const CString& strXMLPath, CString strDir, CString& strOutMsg )
{
	bool bRes = false;
	...
	m_map_md5.clear();
	strOutMsg = _T("");
	if (obj_myxml.Load(strXMLPath) == ERROR_SUCCESS)
	{
		XMLNodeList md5InfoNodeList = NULL;
		md5InfoNodeList = obj_myxml.GetChildNodes(_T("//MD5Check"));
		if (md5InfoNodeList != NULL)
		{
			for (UINT i = 0; i < obj_myxml.GetNodeListSize(md5InfoNodeList); ++i)
			{
				XMLNode testerDeviceItem = obj_myxml.GetNodeListItem(md5InfoNodeList, i);
				if (testerDeviceItem != NULL)
				{
					CString str_filePath = obj_myxml.GetNodeAttrValue(testerDeviceItem, _T("FilePath"));
					CString str_MD5 = obj_myxml.GetNodeAttrValue(testerDeviceItem, _T("MD5"));
					if (str_filePath.IsEmpty() || str_MD5.IsEmpty())
					{
						continue;
					}

					// Insert <Name, ClassName> to map
					m_map_md5[str_filePath] = str_MD5;
					bRes = true;
				}
				else
				{
					strOutMsg = _T("MD5Check nodelist invalide!");
				}
		...
		{
			strOutMsg = _T("MD5Check node missed!");
		}
	}
	else
	{
		strOutMsg = _T("MD5 file damaged!");
	}

	if (!bRes && (strOutMsg == _T("")))
	{
		strOutMsg = _T("No file need MD5 check in check list!");
		return false;
	}
	...
	for (Itr = m_map_md5.begin(); Itr != m_map_md5.end(); Itr++)
	{
		strCurrentPath.Format(_T("%s\\%s"),strDir,Itr->first);
		CString strOutmd5 = GetMD5(strCurrentPath);
		if (strOutmd5.Compare(Itr->second) != 0)
		{
			strOutMsg.Format(_T("MD5 for \'%s\' is not invalide!"),Itr->first);
			return false;
		}
	}
	return bRes;
