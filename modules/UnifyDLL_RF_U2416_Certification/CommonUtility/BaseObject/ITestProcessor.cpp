...
bool ITestProcessor::LoadTestProcessorParams()
{
	...
	XMLNode xml_LogUpload = m_TestItemXML.SearchNode(_T("//Configuration//TestProcessor//LogUpload"));
	if (xml_LogUpload != NULL)
	{
		CString cstr_xml_LogUpload = m_TestItemXML.GetNodeText(xml_LogUpload);
		if (! cstr_xml_LogUpload.IsEmpty())
		{
			if (0 == cstr_xml_LogUpload.CompareNoCase(_T("TRUE")))
				m_b_LogUpload = true;
			else
				m_b_LogUpload = false;
		}
	...
	XMLNode m_UploadServerIP = m_TestItemXML.SearchNode(_T("//Configuration//TestProcessor//UploadServerIP"));
...
void ITestProcessor::SetPICSDate(std::string strKey, std::string strValue)
{
	sPICSData firedData;
...
bool ITestProcessor::LogUpload(char *str_modelname)
{
	...
	{
		m_strResult = _T("FAIL");
		m_strMessage = _T("Fail to connect server uploading.");
		FactoryLog();
		return false;
	}
	...
	if(b_ok)
	{
		...
		//ShellExecuteEx(&ShExecInfo);
		//WaitForSingleObject(ShExecInfo.hProcess, INFINITE); 
		m_strMessage = _T("Check and Upload Pass.");
		FactoryLog();
		return true;
	}
	else
	{
		m_strResult = _T("FAIL");
		m_strMessage = _T("Copy and Check upload file Fail.");
		FactoryLog();
		return false;
	}
...
bool ITestProcessor::DFSDeleteFile(char *str_LocalPath, char *str_TracePath)
{
	...
	{	
		bool b_DelFile = true;
		//.csv檔不刪
		if(strstr(str_TracePath, ".csv") != NULL)
		{
			b_DelFile = false;
		}
		...
		if(strstr(szDir, TodayKeepLog) != NULL)
		{
			b_DelFile = false;
		}
