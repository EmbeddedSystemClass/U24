...
bool CWireless_BT_TxTest::Run()
{
	...
	if (bRes)
	{
		...
		gCpkRecord.assign ("BT_TX_VALUE", m_strMeasured); //write BT_TX_VALUE to CPK
	}
	else
	{
...
bool CWireless_BT_TxTest::MainFunction()
{
	...
	if (! m_pIPhone->BTStartTxPower(m_i_BTChannel,m_i_BTPower,m_i_BTType))
	{
...
bool CWireless_BT_TxTest::InitData(std::map<std::string, std::string>& paramMap)
{
	...
	if (paramMap.find("BTChannel") == paramMap.end())
	{
		TraceLog(MSG_ERROR, "Fail to find parameter BTChannel for CWireless_BT_TxTest");
		return false;
	}
	m_i_BTChannel = atoi(paramMap["BTChannel"].c_str());

	if (paramMap.find("BTPower") == paramMap.end())
	{
		TraceLog(MSG_ERROR, "Fail to find parameter BTPower for CWireless_BT_TxTest");
		return false;
	}
	m_i_BTPower = atoi(paramMap["BTPower"].c_str());

	if (paramMap.find("BTType") == paramMap.end())
	{
		TraceLog(MSG_ERROR, "Fail to find parameter BTType for CWireless_BT_TxTest");
		return false;
	}
	m_i_BTType = atoi(paramMap["BTType"].c_str());
