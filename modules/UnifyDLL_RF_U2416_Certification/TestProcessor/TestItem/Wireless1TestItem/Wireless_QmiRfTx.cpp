...
bool CWireless_QmiRfTxTest::Run()
{
	...
	if (bRes)
	{
		...
		gCpkRecord.assign ("QMI_TX_VALUE", m_strMeasured); //write QMI_TX_VALUE to CPK
	}
	else
	{
		FactoryLog(false, "QmiRfTx", FunErr_WirelessWCDMA_Test_OutPower_OutRange, szBand, m_strChannel, szLower, szUpper, m_strMeasured, "dBm", m_strMsg);
		SetPICSDate(m_strPICSName, m_strMeasured);
		gCpkRecord.assign ("QMI_TX_VALUE", m_strMeasured); //write QMI_TX_VALUE to CPK
	}
...
bool CWireless_QmiRfTxTest::MainFunction()
{
	...
	if (!m_pIPhone->Detroit_QmiRfTxTest()){
		m_strMsg = "Fail to set Detroit_QmiRfTxTest";
		TraceLog(MSG_ERROR, m_strMsg);
		return false;
	}
