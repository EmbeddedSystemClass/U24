...
bool CBTWLAN_WiFi_TxTest::InitData(std::map<std::string, std::string>& paramMap)
{
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter DataRate for CBTWLAN_WiFi_TxTest");
		return false;
	}
	m_iDataRate = atoi(paramMap["DataRate"].c_str());
/*
	if (paramMap.find("Loss") == paramMap.end())
	{
		TraceLog(MSG_ERROR, "Fail to find parameter Loss for CBTWLAN_WiFi_TxTest");
		return false;
	}
	m_dLoss = atof(paramMap["Loss"].c_str());
*/
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter DiagramFull for CBTWLAN_WiFi_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter RetryTimes for CBTWLAN_WiFi_TxTest");
		return false;
	}
...
bool CBTWLAN_WiFi_TxTest::Run(void)
{
	...
	if (bRes)
	{
		FactoryLog(true, "WiFi_TxTest_FSP", "Pass", szBand, m_strChannel, szLower, szUpper, m_strMeasured, "dBm", "WiFi_TxTest_FSP PASS");
		SetPICSDate(m_strPICSName, "PASS");
		SetPICSDate(m_strPICSName_Value, m_strMeasured);
	}
	else
	{
		FactoryLog(false, "WiFi_TxTest_FSP", FunErr_WLAN_Test_OutPower_OutRange, szBand, m_strChannel, szLower, szUpper, m_strMeasured, "dBm", m_strMsg);
...
*/
	...
	if (! m_pITesterDeviceFSP->SetInputLoss(GetCableLoss("TX", "BTWLAN_WIFI", "WifiFixFreq")))
	{
