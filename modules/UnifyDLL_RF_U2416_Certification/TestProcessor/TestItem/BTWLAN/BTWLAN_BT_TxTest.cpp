...
bool CBTWLAN_BT_TxTest::InitData(std::map<std::string, std::string>& paramMap)
{
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter Channel for CBTWLAN_BT_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter Power for CBTWLAN_BT_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter Type for CBTWLAN_BT_TxTest");
		return false;
	}
...
*/
	if (! m_pIPhone->BTStartTxPower(m_iChannel,m_iPower,m_iType))
	{
