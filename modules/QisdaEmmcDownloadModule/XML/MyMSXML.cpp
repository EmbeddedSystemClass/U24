...
INT CMyMSXML::Load(LPCTSTR filePath)
{
	...
	OleInitialize(NULL);
	if (m_DomDoc == NULL) {
		HRESULT hr = m_DomDoc.CreateInstance(WinXML::CLSID_DOMDocument, NULL, CLSCTX_ALL);//CLSCTX_INPROC_SERVER| CLSCTX_LOCAL_SERVER);
	...
	if (!bResult) {
		...
		WinXML::IXMLDOMParseErrorPtr pParseError = m_DomDoc->GetparseError();
...
INT CMyMSXML::GetNodeType(XMLNode node)
{
	...
	WinXML::DOMNodeType nodeType;
 
	node->get_nodeType(&nodeType);
 
	switch(nodeType){
		case WinXML::NODE_ELEMENT:
			return XMLNodeType_Element;
 
		case WinXML::NODE_TEXT:
			return XMLNodeType_Text;
 
		case WinXML::NODE_INVALID:
			return XMLNodeType_Invalid;
		case WinXML::NODE_ATTRIBUTE:
			return XMLNodeType_Attribute;
		case WinXML::NODE_CDATA_SECTION:
		case WinXML::NODE_ENTITY_REFERENCE:
		case WinXML::NODE_ENTITY:
		case WinXML::NODE_PROCESSING_INSTRUCTION:
		case WinXML::NODE_COMMENT:
		case WinXML::NODE_DOCUMENT:
		case WinXML::NODE_DOCUMENT_TYPE:
		case WinXML::NODE_DOCUMENT_FRAGMENT:
		case WinXML::NODE_NOTATION:
...
CString CMyMSXML::GetNodeValue(XMLNodeType attribute, XMLNode node)
{
	...
	WideCharToMultiByte(CP_ACP, 0, str.m_str, -1, data, (int)szData, NULL, NULL);
...
BOOL CMyMSXML::CreateNode(LPCTSTR path, LPCTSTR nodeText)
{
	...
	// create attributes
	WinXML::IXMLDOMNamedNodeMapPtr pAttrList = newElem->Getattributes();
	if (NULL == pAttrList)
		return FALSE;
	...
	for (UINT i = 0; i < attrName.size(); i++){
		...
		WinXML::IXMLDOMAttributePtr pNewAttr = m_DomDoc->createAttribute(bstrAttrName.m_str);
...
BOOL CMyMSXML::DeleteNode(LPCTSTR path)
{
	...
	if (WinXML::NODE_DOCUMENT == nodeType ||	WinXML::NODE_DOCUMENT_FRAGMENT == nodeType)
...
void CMyMSXML::GetNodeAttrValue(LPCTSTR nodeName, LPCTSTR attrName, LPTSTR attrValue, size_t inLength)
{
	...
	WinXML::IXMLDOMNamedNodeMapPtr attr = NULL;
...
BOOL CMyMSXML::HasChild(XMLNode node)
{
	...
	for (int i = 0; i < nodeList->Getlength(); i++) {
		...
		node->get_nodeType(&nodeType);
		if (nodeType == WinXML::NODE_ELEMENT)
			return TRUE;
	}
