...
namespace QSYNC_LIB 
{
	bool SearchDirectory(std::vector<TSTRING>& vDirName,
						const TSTRING &strRootFolder) //only search in 1 level
	{
		bool bRes = false;
		vDirName.clear();
		WIN32_FIND_DATA FindFileData;
		TSTRING strPattern(strRootFolder);
		strPattern += _T("*.*");
		HANDLE hFind = FindFirstFile(strPattern.c_str(), &FindFileData);
		if(INVALID_HANDLE_VALUE != hFind)
		{
			do{
				if(FindFileData.cFileName[0] != '.')
				{
					...
					{
						vDirName.push_back(FindFileData.cFileName);
	...
	bool SearchFiles(std::vector<TSTRING>& vFileName, 
					...
					const TSTRING& refstrExtension,/*if=empty, means no filter*/
					bool bSkipExtension/*true=remove .extension*/)
	{
		bool bRes = false;
		vFileName.clear();
		WIN32_FIND_DATA FindFileData;
		...
		if(INVALID_HANDLE_VALUE != hFind)
		{
			do{
				if(FindFileData.cFileName[0] != '.')
				{
					if(!(FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
					{
						if(refstrExtension.empty())
						{
							vFileName.push_back(FindFileData.cFileName);
						}
						else
						{
							TSTRING strExtension = FindFileData.cFileName;
							strExtension = strExtension.substr(strExtension.rfind(".") + 1);
							
							CString strExtensionL = strExtension.c_str(); 
							CString strRefExtensionL = refstrExtension.c_str();
							
							if(strExtensionL.MakeLower() == strRefExtensionL.MakeLower())	
							{
								// Save filename
...
#define MD5STEP(f,w,x,y,z,in,s) \
	...
	*/
	...
	//Just used on string file, like txt, ini, xml....
	...
	bool GetMD5Content(const std::string& strContent, std::string& strDigest)
	{
		...
		if(nDigestSize > 0)
		{
			...
			memset(szBuffer,0,nDigestSize+2);
	...
	{
		bool bRes = false;
		FILE *stream = ::fopen(strFilePath.c_str(), "w");
		if(stream)
		{
			fprintf(stream, strContent.c_str());
			fclose(stream);
			bRes = true;
		}
		return bRes;
