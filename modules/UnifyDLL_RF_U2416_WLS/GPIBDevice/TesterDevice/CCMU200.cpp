...
bool CCMU200::RF_SetAnalyzerFreq(double dFreqMHz)
{
	...
	sprintf_s(szCommand, 255, "%d;RFANalyzer:BANDwidth 1 MHZ; *OPC?", g_iMode);
	strCommand = szCommand;
	if (! GPIB_OPComplete(strCommand))
		return false;
...
bool CCMU200::RF_SetModulationMode(std::string strMode)
{
	...
	if (
		...
		stricmp("DSB",const_cast<char*>(strMode.c_str()))!=0 &&
...
bool CCMU200::RF_NPowerMeasurementNoOFF(int iStatistics, std::string& strMeasured)
{
    bool bFetch = false;
    int iRetry = 50;
...
bool CCMU200::RF_NPowerMeasurementMAX(int iStatistics, std::string& strMeasured)
{
    ...
    sprintf_s(szCommand, 255, "%d;INITiate:SPECtrum;*OPC?", g_iMode );
    strCommand = szCommand;

    if (! GPIB_OPComplete(strCommand))
        return false;

    sprintf_s(szCommand, 255, "%d;FETCh:SPECtrum:STATus?" , g_iMode);
    strCommand = szCommand;
    do
    {
        if (! GPIB_Querry(strCommand, strResponse))
	...
	std::string tmp;	
	sprintf_s(szCommand, 255, "%d;FETCh:ARRay:SPECtrum:AVERage?", g_iMode );
	strCommand = szCommand;
	GPIB_Querry(strCommand, tmp);
	sprintf_s(szCommand, 255, "%d;ABORt:SPECtrum", g_iMode );
	GPIB_Write(szCommand);

	strMeasured = tmp;
	//sprintf_s ( szCommand, 255, "%d;INITiate:SPECtrum", _RF_NSIG );

	//sprintf_s ( szCommand, 255, "%d;FETCh:SPECtrum:STATus?", _RF_NSIG );

	//sprintf_s ( szCommand, 255, "%d;FETCh:ARRay:SPECtrum:AVERage?", _RF_NSIG );
	//m_pITestBox->GPIB_Query(szCommand, tmp);
	//sprintf_s ( szCommand, 255, "%d;ABORt:SPECtrum", _RF_NSIG );
	//m_pITestBox->GPIB_Write(szCommand);

	//STDStringTokenizer ( tmp, ",", vPowerList );
	//dPower = atof (vPowerList [(int)vPowerList.size() / 2 -1 ].c_str() );
...
bool CCMU200::RF_SetLevelMode(std::string strLevelMode)
{
	char szCommand[256];
	memset(szCommand, 0, 256);
	std::string strCommand;

	sprintf_s(szCommand, 255, "%d;LEVel:MODE %s; *OPC?", g_iMode, strLevelMode.c_str());
	strCommand = szCommand;
	if (! GPIB_OPComplete(strCommand))
		return false;
...
bool CCMU200::RF_SetLevelMaximum(int iLevelMax)
{
	char szCommand[256];
	memset(szCommand, 0, 256);
	std::string strCommand;

	sprintf_s(szCommand, 255, "%d;LEVel:MAXimum %d; *OPC?", g_iMode, iLevelMax);
	strCommand = szCommand;
	if (! GPIB_OPComplete(strCommand))
		return false;
...
bool CCMU200::SetResolutionBandwidth(double dRBWMHz)
{
	char szCommand[256];
	memset(szCommand, 0, 256);
	std::string strCommand;

	sprintf_s(szCommand, 255, "%d;NPOWer:BWIDth %f MHZ; *OPC?", g_iMode, dRBWMHz);
	strCommand = szCommand;
	if (! GPIB_OPComplete(strCommand))
		return false;
	return true;
