...
bool CNFC_Antenna_Tuner_IC::Run()
{
	...
	if (bRes)
	{
		FactoryLog(true, "AntennaTunerIC", "Pass", "", "", "", "", m_strMeasured, "", "Antenna Tuner IC PASS");
	}
	else
	{
		FactoryLog(false, "AntennaTunerIC", FunErr_NFC_Antenna_IC_ID_Fail, "", "", "", "", m_strMeasured, "", m_strMsg);
	}
...
bool CNFC_Antenna_Tuner_IC::MainFunction()
{
	...
	{
		m_strMsg = "Fail to read Antenna Tuner IC ID";
		TraceLog(MSG_ERROR, m_strMsg);
		return false;
	}
	...
	{
		m_strMsg = "Fail to compare Antenna tuner IC ID";
		TraceLog(MSG_ERROR, m_strMsg);
		bPass = false;
	}
...
bool CNFC_Antenna_Tuner_IC::InitData(std::map<std::string, std::string>& paramMap)
{
	...
	if (paramMap.find("Tuner_IC") == paramMap.end())
	{
		TraceLog(MSG_ERROR, "Fail to find parameter Tuner_IC for CNFC_Antenna_Tuner_IC");
		return false;
	}
	m_strAntennaTunerIC = paramMap["Tuner_IC"];
