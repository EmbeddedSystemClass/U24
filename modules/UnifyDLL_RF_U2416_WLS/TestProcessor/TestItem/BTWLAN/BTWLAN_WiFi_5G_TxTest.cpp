#include "StdAfx.h"
#include "BTWLAN_WiFi_5G_TxTest.h"
...
RF_IMPLEMENT_DYNCREATE(CBTWLAN_WiFi_5G_TxTest)

CBTWLAN_WiFi_5G_TxTest::CBTWLAN_WiFi_5G_TxTest(void)
	:m_WriteCPKLog(false)
	,m_bKLossMode(false)
{
}
...
bool CBTWLAN_WiFi_5G_TxTest::InitData(std::map<std::string, std::string>& paramMap)
{
	std::vector<std::string> vTmp;

	if (paramMap.find("PICSName") == paramMap.end())
	{
		TraceLog(MSG_ERROR, "Fail to find parameter PICSName for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter Frequency for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter Channel for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter Power for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter RBW for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter VBW for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter Span for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter TraceMode for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter Detector for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter RetryTimes for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter ChannelBW for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter SweepTime for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter AttenuationManual for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter RefLevel for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
	...
	{
		TraceLog(MSG_ERROR, "Fail to find parameter Range for CBTWLAN_WiFi_5G_TxTest");
		return false;
	}
...
bool CBTWLAN_WiFi_5G_TxTest::Run(void)
{
	m_strMeasured = "";
	...
	TraceLog (MSG_INFO, "BT_WiFi_5G_TxTest_CPK_HEADER: " + gCpkRecord.GetLogHeader());
	TraceLog (MSG_INFO, "BT_WiFi_5G_TxTest_CPK_RESULT: " + gCpkRecord.GetLogString());
	...
	if (m_WriteCPKLog)
	{
		...
		gCpkRecord.assign ( "Wifi_TxPower" , m_strMeasured ); 
...
bool CBTWLAN_WiFi_5G_TxTest::MainFunction(void)
{
	char szTmp[256];
	...
	for (int i = 0; i < m_iRetryTimes; ++i)
	{
		...
		if (! m_pITesterDeviceFSP->RF_NPowerMeasurementNoOFF(0, strResponse))
		{
		...
		else
		{
			...
			{
				double dLoss = 0.0;
				dLoss = GetRefResult("TX", "BTWLAN_WIFI", "WifiAdjPower_5G") - dPower;
				
				memset(szTmp, 0, 256);
				sprintf_s(szTmp, 255, "%.1f", dLoss);

				UpdateCableLoss("TX", "BTWLAN_WIFI", "WifiAdjPower_5G", szTmp);
...
bool CBTWLAN_WiFi_5G_TxTest::ParseCPKItems()
{
	...
	XMLNode processObjectSet = m_TestItemXML.SearchNode(_T("//Configuration//ProcessObjectSet//CommonTestObjects//Wifi5GTxCPKcolumn"));
