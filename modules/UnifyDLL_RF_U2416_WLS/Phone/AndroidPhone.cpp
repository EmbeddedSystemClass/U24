...
bool CAndroidPhone::InitData(std::map<std::string, std::string>& paramMap)
{
	...
	{
		m_strDeviceName = m_strCOMport = std::string("0");
	}

	// Keyword
	if (paramMap.find("Keyword") != paramMap.end())
	{
		m_strKeyword = paramMap[std::string("Keyword")];
	}
...
bool CAndroidPhone::MyExecAdbCmd(const char* chCmd, char **chReturn, bool bWithShell)
{
	...
	try
	{
		...
		GetModuleFileName(GetModuleHandle(_T("RFTool.dll")), szExePath, MAX_PATH);
#else
		GetModuleFileName(GetModuleHandle(_T("FT.dll")), szExePath, MAX_PATH);
#endif
...
bool CAndroidPhone::SetFAData(char* szValue)
{
	...
	char pInBuffer[FTD_BUF_SIZE] = {0};
	char pOutBuffer[FTD_BUF_SIZE]={0};
	memset(pInBuffer, 0, FTD_BUF_SIZE);
...
bool CAndroidPhone::GetFASector( int i_sectorNum, char *sz_sectorData, int i_sectorSize )
{
	...
	char sz_outBuffer[FTD_BUF_SIZE]={0};//must be FTD_BUF_SIZE,lower will set FTD_BUF_SIZE bytes to this cach
...
bool CAndroidPhone::BTReset()
{
	char szInput[FTD_BUF_SIZE] = {0};
	pDetroit_BTReset Detroit_BTReset = (pDetroit_BTReset)GetProcAddress(m_hNeptuneCtrl, "Detroit_BTReset");
	if (! Detroit_BTReset)
		return false;
	...
	int ret = Detroit_BTReset (FTD_PORT, FTD_TIMEOUT, szInput ,chError);
	return ret == FTD_OK;

	//if (Detroit_BTReset (FTD_PORT, FTD_TIMEOUT, szInput ,chError) != FTD_OK)
	//	return false;
...
bool CAndroidPhone::BTStartTxPower(char *szInput)
{
#ifdef _USE_ADBSHELL_
...
bool CAndroidPhone::BTStartTxLtePower(char *szBTInput)
{
#ifdef _USE_ADBSHELL_
	return MyExecAdbCmd("bttest tx_continous_mode 1 7 4");
#else
	...
	int ret = FTD_BTTXLtePower(FTD_PORT, FTD_TIMEOUT, szBTInput, szError);
	return ret == FTD_OK;
#endif
}
...
bool CAndroidPhone::WiFiStartTxPower(int iRate, int iChannel, int iPower)
{
	...
	char szError[FTD_BUF_SIZE], szIn[FTD_BUF_SIZE];
	//int port = atoi(m_strCOMport.c_str());
	...
	else
	{
		...
		for(int i = 0;i < 1;i++)
		{
			...
			*/
			...
			//}
			//Set TX
			isOk = (0 != QLIB_FTM_WLAN_GEN6_ENABLE_CHAINS(m_hQMSLPhone, 6));
			if (!isOk) { 
				continue;
			}
			//Set close loop
			isOk = (0 != QLIB_FTM_WLAN_GEN6_CLOSE_TPC_LOOP(m_hQMSLPhone, 1));
			if (!isOk) {
				continue;
			}
			//Set channel
			isOk = (0 != QLIB_FTM_WLAN_GEN6_SET_CHANNEL(m_hQMSLPhone, iChannel));
			if (!isOk) { 
				continue;
			}
			//Set rate
			isOk =(0 !=  QLIB_FTM_WLAN_GEN6_SET_TX_FRAME(m_hQMSLPhone, 0, 2400, 10, true, cmdRateMap, cmdPreamble));
			if (!isOk) { 
				continue;
			}
/*			//Set Packet Gain
			...
			isOk = (0 != QLIB_FTM_WLAN_GEN6_SET_CLOSED_LOOP_POWER(m_hQMSLPhone, iPower));
			if (!isOk) { 
				continue;
			}
			/* marked for CDMA
			...
			//TX start
			isOk = (0 != QLIB_FTM_WLAN_GEN6_TX_PKT_START_STOP(m_hQMSLPhone, 1));
			if (!isOk) { 
				continue;
			}
	...
	else
	{
		...
		for(int i = 0;i < 1;i++)
		{
			...
			*/
			...
			//Set rate
			isOk = !!QLIB_FTM_WLAN_GEN6_SET_TX_FRAME_V2(m_hQMSLPhone, 0, 756, 2, 165, 10, true, cmdRateMap, cmdPreamble);
			if (!isOk) { 
				continue;
			}
...
bool CAndroidPhone::WifiPowerOnCw (int iChannel, int iGain, long iTone, unsigned long iAmpl)
{
	...
	{
		isOk = (0 != QLIB_FTM_WLAN_GEN6_STOP_RF_WAVEFORM(m_hQMSLPhone));
		if (isOk)
			return isOk && (0 != QLIB_FTM_WLAN_GEN6_STOP_WAVEFORM(m_hQMSLPhone));
		else
			return false;
	}
	...
	else	// Start Cw Power
	{
		...
		if (isOk)
			isOk = (0 != QLIB_FTM_WLAN_GEN6_SET_CHANNEL(m_hQMSLPhone, iChannel));
		if (isOk)
			isOk = (0 != QLIB_FTM_WLAN_GEN6_TX_PKT_START_STOP(m_hQMSLPhone, 0));
		if (isOk)
			isOk = (0 != QLIB_FTM_WLAN_GEN6_SET_TX_WAVEFORM_GAIN_V2(m_hQMSLPhone, iGain));
		if (isOk)
			isOk = (0 != QLIB_FTM_WLAN_GEN6_GEN_RF_CW(m_hQMSLPhone, iTone, iAmpl));
		if (isOk)
			isOk = (0 != QLIB_FTM_WLAN_GEN6_START_RF_WAVEFORM(m_hQMSLPhone));
...
bool CAndroidPhone::GPS_GetCN0value(double& dCN0, int& nFreq, unsigned char iTestMode, unsigned short iCaptureSize_kSamples, unsigned char iNumberOfIntegration)
{
//#ifdef _NOT_USE_FTD_
	return (0 != QLIB_DIAG_GEN8_GET_EMBEDDED_CALCULATED_CTON(this->m_hQMSLPhone,
													   iTestMode,
...
bool CAndroidPhone::WCDMA_Rssi (unsigned short iChannel, unsigned short* iRX_AGC, unsigned short* iRX_Power_dBm, unsigned short* iFTM_Error_Code)
{
	...
	isOk = (0 != QLIB_FTM_SET_MODE(m_hQMSLPhone, PHONE_MODE_WCDMA_IMT));

	if (isOk)
		isOk = (0 != QLIB_FTM_SET_CHAN(m_hQMSLPhone, iChannel));

	if (isOk)
		isOk = (0 != QLIB_FTM_GET_RX_LEVEL_DBM(m_hQMSLPhone, iRX_AGC, iRX_Power_dBm, iFTM_Error_Code));
...
bool CAndroidPhone::FSG_Enable(char* sz_inData, char* sz_outData)
{
	...
	/* Get DLL function */
	pSapporo_FSG_Enable FTD_FSG_Enable = (pSapporo_FSG_Enable)::GetProcAddress(m_hNeptuneCtrl, "Sapporo_FAC_FSGProtect");
	if (!FTD_FSG_Enable)
	{
		return false;
	}
...
// NFC
bool CAndroidPhone::NFCHCITypeAReader(char* sz_inData, char* sz_outData)
{
	int nRetCode;

	/* Get DLL function */
	pFTD_NFCHCITypeAReader FTD_NFCHCITypeAReader = (pFTD_NFCHCITypeAReader)::GetProcAddress(m_hNeptuneCtrl, "FTD_NFCHCITypeAReader");
	if (!FTD_NFCHCITypeAReader)
	{
		return false;
	}
	...
	if(FTD_NFCHCITypeAReader)
	{
		nRetCode = FTD_NFCHCITypeAReader(FTD_PORT, FTD_TIMEOUT, sz_inData, sz_outData);
...
bool CAndroidPhone::QMSL_FTM_SET_MODE_ID(short eModeId)
{
	if (m_hQMSLPhone != NULL)
	{
		if (QLIB_FTM_SET_MODE_ID(m_hQMSLPhone, eModeId))
			return true;
	}
...
bool CAndroidPhone::QMSL_FTM_SET_PA_STATE(unsigned short unState, unsigned short* iFTM_Error_Code)
{
	if (m_hQMSLPhone != NULL)
	{
		
		if (QLIB_FTM_SET_PA_STATE(m_hQMSLPhone, (unsigned char)unState , iFTM_Error_Code ))
			return true;
	}
...
bool CAndroidPhone::QMSL_ChangeFTM_BootMode(int iMode)
{
    ...
    if (m_hQMSLPhone != NULL)
    {
		...
		{
			unsigned char ucExecutionStatus;
			char szFilePath[8];
			memset(szFilePath, 0, 8);
			sprintf(szFilePath, "/");
			if (QLIB_EFS2_SyncWithWait(m_hQMSLPhone, (unsigned char*)szFilePath, (unsigned long)10000, &ucExecutionStatus))
				return true;
		}
    }
...
{
	char input_parameter[FTD_BUF_SIZE] = {0};
...
bool CAndroidPhone::Detroit_PWRchargerOn(int nPort, int nTimeout, char* pszInput, char* pszOutput)
{
	int nRetCode;
	char in_buf[FTD_BUF_SIZE] = {0};
	char out_buf[FTD_BUF_SIZE] = {0};

	PFGenCmdInOut pPWRchargerOn = (PFGenCmdInOut)::GetProcAddress(m_hNeptuneCtrl, _T("Detroit_PWRchargerOn"));
	if (!pPWRchargerOn)
	{
		return false;
	}
	strcpy(in_buf, pszInput);
	nRetCode = pPWRchargerOn(nPort, nTimeout, in_buf, out_buf);
	if (nRetCode == 1)  //HS_OK
	{
...
bool CAndroidPhone::Detroit_PWRAverageGaugeCurrent(int nPort, int nTimeout, char* pszInput, char* pszOutput)
{
	int nRetCode;
	char in_buf[FTD_BUF_SIZE] = {0};
	char out_buf[FTD_BUF_SIZE] = {0};

	PFGenCmdInOut pPWRAverageGaugeCurrent = (PFGenCmdInOut)::GetProcAddress(m_hNeptuneCtrl, _T("Detroit_PWRAverageGaugeCurrent"));
	if (!pPWRAverageGaugeCurrent)
	{
		return false;
	}
	strcpy(in_buf, pszInput);
	nRetCode = pPWRAverageGaugeCurrent(nPort, nTimeout, in_buf, out_buf);
	if (nRetCode == 1)  //HS_OK
	{
		strcpy(pszOutput, out_buf);
	}
...
bool CAndroidPhone::Detroit_PWRWriteRsenseData(int nPort, int nTimeout, char* pszInput, char* pszOutput)
{
	int nRetCode;
	char in_buf[FTD_BUF_SIZE] = {0};
	char out_buf[FTD_BUF_SIZE] = {0};

	PFGenCmdInOut pPWRWriteRsenseData = (PFGenCmdInOut)::GetProcAddress(m_hNeptuneCtrl, _T("Detroit_PWRWriteRsenseData"));
	if (!pPWRWriteRsenseData)
	{
		return false;
	}
	strcpy(in_buf, pszInput);
	nRetCode = pPWRWriteRsenseData(nPort, nTimeout, in_buf, out_buf);
	if (nRetCode == 1)  //HS_OK
	{
		strcpy(pszOutput, out_buf);
	}
...
bool CAndroidPhone::FTD_PWRgaugeVoltage(int nPort, int nTimeout, char* pszInput, char* pszOutput)
{
	int nRetCode;
	char in_buf[FTD_BUF_SIZE] = {0};
	char out_buf[FTD_BUF_SIZE] = {0};

	PFGenCmdInOut pPWRGaugeVoltage = (PFGenCmdInOut)::GetProcAddress(m_hNeptuneCtrl, _T("FTD_PWRgaugeVoltage"));
	if (!pPWRGaugeVoltage)
	{
		return false;
	}
	strcpy(in_buf, pszInput);
	nRetCode = pPWRGaugeVoltage(nPort, nTimeout, in_buf, out_buf);
	if (nRetCode == 1)  //HS_OK
	{
		strcpy(pszOutput, out_buf);
	}
...
bool CAndroidPhone::FTD_FAC_FSGProtect(int nPort, int nTimeout, char* pszInput, char* pszOutput)
{
	...
	PFGenCmdInOut pFAC_FSGProtect = (PFGenCmdInOut)::GetProcAddress(m_hNeptuneCtrl, _T("Sapporo_FAC_FSGProtect"));
	if (!pFAC_FSGProtect)
	{
		return false;
	}
	strcpy(in_buf, pszInput);
	nRetCode = pFAC_FSGProtect(nPort, nTimeout, in_buf, out_buf);
	if (nRetCode == 1)  //HS_OK
	{
		strcpy(pszOutput, out_buf);
	}
...
// LTE
//////////////////////////////////////////////////////////////////////////
bool CAndroidPhone::QMSL_FTM_LTE_SET_TX_BANDWIDTH(unsigned char irxChanBW)
{
	if (m_hQMSLPhone != NULL)
	{
		if (QLIB_FTM_LTE_SET_TX_BANDWIDTH(m_hQMSLPhone, irxChanBW))
			return true;
	}
...
bool CAndroidPhone::QMSL_FTM_LTE_SET_RX_BANDWIDTH(unsigned char irxChanBW)
{
	if (m_hQMSLPhone != NULL)
	{
		if (QLIB_FTM_LTE_SET_RX_BANDWIDTH(m_hQMSLPhone, irxChanBW))
			return true;
	}
...
bool CAndroidPhone::QMSL_FTM_LTE_SET_TX_WAVEFORM(unsigned char iTxWaveform, unsigned char inumRBsPUSCH, unsigned char inumRBsPUCCH,unsigned char iPUSCHStartRBIndex)
{
	if (m_hQMSLPhone != NULL)
	{
		if (QLIB_FTM_LTE_SET_TX_WAVEFORM(m_hQMSLPhone, iTxWaveform, inumRBsPUSCH, inumRBsPUCCH, iPUSCHStartRBIndex))
			return true;
	}
...
bool CAndroidPhone::QMSL_FTM_LTE_STOP_TX_WAVEFORM()
{
	if (m_hQMSLPhone != NULL)
	{
		if (QLIB_FTM_LTE_STOP_TX_WAVEFORM(m_hQMSLPhone))
			return true;
	}
...
bool CAndroidPhone::QMSL_FTM_SET_TX_GAIN_INDEX(unsigned short iTxGainIndex)
{
	if (m_hQMSLPhone != NULL)
	{
		if (QLIB_FTM_SET_TX_GAIN_INDEX(m_hQMSLPhone, iTxGainIndex))
			return true;
	}
...
bool CAndroidPhone::QMSL_FTM_LTE_SET_NET_SIG_VALUE(unsigned char iNS, unsigned short* iFTM_Error_Code)
{
	if (m_hQMSLPhone != NULL)
	{
		if (QLIB_FTM_LTE_SET_NET_SIG_VALUE(m_hQMSLPhone, iNS, iFTM_Error_Code))
			return true;
	}
...
bool CAndroidPhone::QMSL_FTM_SET_TX_POWER_DBM(unsigned char iEnable, short iPowerLevel, unsigned short* iFTM_Error_Code)
{
	if (m_hQMSLPhone != NULL)
	{
		if (QLIB_FTM_SET_TX_POWER_DBM(m_hQMSLPhone, iEnable, iPowerLevel, iFTM_Error_Code))
			return true;
	}
...
bool CAndroidPhone::QMSL_FTM_LTE_SET_TX_MODULATION_TYPE(unsigned long iModulationType)
{
	if (m_hQMSLPhone != NULL)
	{
		if (QLIB_FTM_LTE_SET_TX_MODULATION_TYPE(m_hQMSLPhone, iModulationType))
			return true;
	}
...
bool CAndroidPhone::LTE_B3_RxRssi (unsigned short iChannel, unsigned short* iRX_AGC, unsigned short* iRX_Power_dBm, unsigned short* iFTM_Error_Code)
{
	bool isOk = true;

	//isOk = QLIB_FTM_SET_MODE(m_hQMSLPhone, PHONE_MODE_WCDMA_IMT);
	//isOk = QMSL_FTM_SET_MODE_ID(FTM_MODE_ID_LTE); // FTM_MODE_ID_LTE = 29,       //!<' LTE FTM Calibration
...
bool CAndroidPhone::LTE_B7_RxRssi (unsigned short iChannel, unsigned short* iRX_AGC, unsigned short* iRX_Power_dBm, unsigned short* iFTM_Error_Code)
{
	bool isOk = true;
...
bool CAndroidPhone::LTE_B20_RxRssi (unsigned short iChannel, unsigned short* iRX_AGC, unsigned short* iRX_Power_dBm, unsigned short* iFTM_Error_Code)
{
	bool isOk = true;
...
bool CAndroidPhone::LTE_Band_RxRssi ( unsigned short* iRX_AGC, unsigned short* iRX_Power_dBm, unsigned short* iFTM_Error_Code)
{
	bool isOk = true;
...
//////////////////////////////////////////////////////////////////////////
bool CAndroidPhone::QMSL_FTM_GSM_GET_MEAS_PWR (unsigned short* iMeasured_Dbm16_Value, unsigned long*  iRaw_RSSI, unsigned short* iFTM_Error_Code)
{
	if (m_hQMSLPhone != NULL)
	{
		if (QLIB_FTM_GSM_GET_MEAS_PWR(m_hQMSLPhone, iMeasured_Dbm16_Value, iRaw_RSSI, iFTM_Error_Code))
			return true;
	}
...
bool CAndroidPhone::QMSL_FTM_GSM_SETUP_RX_BURST_FOR_EXPECTED_PWR (unsigned short iExpected_RX_Power_dbm16, unsigned short iNumber_of_Averages,
         unsigned short* iFTM_Error_Code)
{
	if (m_hQMSLPhone != NULL)
	{
		if (QLIB_FTM_GSM_SETUP_RX_BURST_FOR_EXPECTED_PWR(m_hQMSLPhone, iExpected_RX_Power_dbm16, iNumber_of_Averages, iFTM_Error_Code))
			return true;
	}
...
bool CAndroidPhone::QMSL_FTM_RFFE_READWRITE_CMD(unsigned char* szTunerIC)
{
	if (m_hQMSLPhone != NULL)
	{
		if (QLIB_FTM_RFFE_READWRITE_CMD (m_hQMSLPhone,0,1,1,6,0x1d,szTunerIC,0))
			return true;
	}
...
bool CAndroidPhone::QMSL_SendSync(short iRequestSize, BYTE* piRequestBytes, BYTE* piResponseBytes, short* piResponseSize)
{
	if (m_hQMSLPhone != NULL)
	{
		for(int retry = 0; retry < 3; retry++)
		{
			if (!QLIB_SendSync(m_hQMSLPhone, iRequestSize, piRequestBytes, piResponseSize, piResponseBytes, 3000))
			{
				if(!IsConnected_QMSL())											
				{																
					Initial_QMSL();												
...
bool CAndroidPhone::QMSL_FTM_DO_TUNER_SELF_CAL()
{
	if (m_hQMSLPhone != NULL)
	{
		unsigned short iResultCode;
		if(QLIB_FTM_DO_TUNER_SELF_CAL(m_hQMSLPhone, &iResultCode))
		{
			if(iResultCode == 0)
				return true;
			else
...
bool CAndroidPhone::QMSL_EFS2_SyncWithWait(unsigned char* ucStatus, int iTimeout)
{
	if (m_hQMSLPhone != NULL)
	{
		char szFilePath[8] = {0};
		...
		for(int retry = 0; retry < 3; retry++)
		{
			if (!QLIB_EFS2_SyncWithWait(m_hQMSLPhone, (unsigned char*)szFilePath, (unsigned long)iTimeout, ucStatus))
			{
				if(!IsConnected_QMSL())											
				{																
					Initial_QMSL();												
...
bool CAndroidPhone::QMSL_FTM_LTE_SET_LNA_STATE_DBM ( short iExpected_RX_Power_dBm )
{
	if (m_hQMSLPhone != NULL)
	{
		unsigned short _iFTM_Error_Code;
		char cSelected_LNA_Range[FTD_BUF_SIZE] = {0};

		if(QLIB_FTM_LTE_SET_LNA_STATE_DBM( m_hQMSLPhone , iExpected_RX_Power_dBm , (unsigned char*)cSelected_LNA_Range , &_iFTM_Error_Code ))
		{
			if( _iFTM_Error_Code == 0 )
				return true;
			else
				return false;
		}
...
bool CAndroidPhone::QMSL_FTM_GET_RX_AGC(unsigned short* iRX_AGC)
{
	if (m_hQMSLPhone != NULL)
	{
		if ( QLIB_FTM_GET_RX_AGC(m_hQMSLPhone, iRX_AGC))
			return true;
	}
	return false;
}