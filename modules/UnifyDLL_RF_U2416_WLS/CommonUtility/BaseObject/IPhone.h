...
public:
	...
	virtual bool MyExecAdbCmd(const char* chCmd, char **chReturn, bool bWithShell) = 0;
	...
	virtual bool Detroit_PWRchargerOn(int nPort, int nTimeout, char* p_sz_Input, char* p_sz_Output) = 0;
	virtual bool Detroit_PWRWriteRsenseData(int nPort, int nTimeout, char* p_sz_Input, char* p_sz_Output) = 0;// cmd: 13
	...
	virtual bool NFCHCITypeAReader(char *szInput, char *szOutput) = 0;
	...
	virtual bool BTStartTxPower(char *szInput) = 0;
	...
	virtual bool BTStartTxLtePower(char *szBTInput) = 0;
	virtual bool BTReset()=0;
	...
	virtual bool GPS_GetCN0value(double& dCN0, int& nFreq, unsigned char iTestMode, unsigned short iCaptureSize_kSamples, unsigned char iNumberOfIntegrations) = 0;
	...
	virtual bool QMSL_SendSync(short iRequestSize, BYTE* piRequestBytes, BYTE* piResponseBytes, short* piResponseSize) = 0;


	virtual bool ReadNV_QMSL(unsigned short unIndex, unsigned char* pData, int iSize) = 0;
	virtual bool QMSL_FTM_SET_MODE_ID(short eModeId) = 0;
	...
	virtual bool QMSL_FTM_SET_PA_STATE(unsigned short unState, unsigned short* iFTM_Error_Code) = 0;
	...
	// LTE
	virtual bool QMSL_FTM_LTE_SET_TX_BANDWIDTH(unsigned char irxChanBW) = 0;
	virtual bool QMSL_FTM_LTE_SET_RX_BANDWIDTH(unsigned char irxChanBW) = 0;
	virtual bool QMSL_FTM_LTE_SET_TX_WAVEFORM(unsigned char iTxWaveform, unsigned char inumRBsPUSCH, unsigned char inumRBsPUCCH,unsigned char iPUSCHStartRBIndex) = 0;
	virtual bool QMSL_FTM_LTE_STOP_TX_WAVEFORM() = 0;
	virtual bool QMSL_FTM_SET_TX_GAIN_INDEX(unsigned short iTxGainIndex) = 0;
	virtual bool QMSL_FTM_LTE_SET_NET_SIG_VALUE(unsigned char iNS, unsigned short* iFTM_Error_Code) = 0;
	virtual bool QMSL_FTM_SET_TX_POWER_DBM(unsigned char iEnable, short iPowerLevel, unsigned short* iFTM_Error_Code) = 0;
	virtual bool QMSL_FTM_LTE_SET_TX_MODULATION_TYPE(unsigned long iModulationType) = 0;
	virtual bool LTE_B3_RxRssi (unsigned short iChannel, unsigned short* iRX_AGC, unsigned short* iRX_Power_dBm, unsigned short* iFTM_Error_Code) = 0;

	virtual bool LTE_B7_RxRssi (unsigned short iChannel, unsigned short* iRX_AGC, unsigned short* iRX_Power_dBm, unsigned short* iFTM_Error_Code) = 0;
	virtual bool LTE_B20_RxRssi (unsigned short iChannel, unsigned short* iRX_AGC, unsigned short* iRX_Power_dBm, unsigned short* iFTM_Error_Code) = 0;

	virtual bool LTE_Band_RxRssi ( unsigned short* iRX_AGC, unsigned short* iRX_Power_dBm, unsigned short* iFTM_Error_Code) = 0;

	virtual bool QMSL_FTM_GSM_GET_MEAS_PWR (unsigned short* iMeasured_Dbm16_Value, unsigned long*  iRaw_RSSI, unsigned short* iFTM_Error_Code) = 0;
	virtual bool QMSL_FTM_GSM_SETUP_RX_BURST_FOR_EXPECTED_PWR (unsigned short iExpected_RX_Power_dbm16, unsigned short iNumber_of_Averages, unsigned short* iFTM_Error_Code) = 0;
	virtual bool QMSL_FTM_RFFE_READWRITE_CMD (unsigned char* szTunerIC) = 0;
	virtual bool QMSL_FTM_DO_TUNER_SELF_CAL() = 0;
	virtual bool QMSL_EFS2_SyncWithWait(unsigned char* ucStatus, int iTimeout = 5000) = 0;

	virtual bool QMSL_FTM_LTE_SET_LNA_STATE_DBM ( short iExpected_RX_Power_dBm ) = 0;

	virtual bool QMSL_FTM_GET_RX_AGC(unsigned short* iRX_AGC) = 0;
