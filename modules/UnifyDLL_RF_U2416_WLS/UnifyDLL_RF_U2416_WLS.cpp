// UnifyDLL_RF_U2416_WLS.cpp : Defines the initialization routines for the DLL.
//

#include "stdafx.h"
#include "UnifyDLL_RF_U2416_WLS.h"
...
#else
UnifyDLL_WLS_API bool NewInterface(IFacTestToolInterface** ppIToolInterface)
#endif
{
	bool bRes = false;
	...
	if (! (*ppIToolInterface))
	{
		*ppIToolInterface = new CUnifyDLL_WLS;
		bRes = true;
	}

	CUnifyDLL_WLS* pToolInstance = dynamic_cast<CUnifyDLL_WLS*>(*ppIToolInterface);
...
#endif
{
	if (pIToolInterface)
	{
		...
		delete (CUnifyDLL_WLS*)pIToolInterface;
#endif
...
bool CUnifyDLL_WLS::LoadSettingFile()
{
...
bool CUnifyDLL_WLS::CheckCableLossXMLExist()
{
	bool bRes = false;
...
bool CUnifyDLL_WLS::CheckTestItemXMLExist()
{
	bool bRes = false;
...
bool CUnifyDLL_WLS::CreateTestProcessor()
{
	bool bRes = true;
...
bool CUnifyDLL_WLS::LoadToolInfo()
{
	if (m_pITool)
	{
...
bool CUnifyDLL_WLS::LoadTestItem()
{
	if (m_pITool)
	{
...
#ifdef _ToolInterface
bool CUnifyDLL_WLS::OnAPPLaunch()
#else
...
#ifdef _ToolInterface
bool CUnifyDLL_WLS::OnAPPEnd()
#else
bool CUnifyDLL_WLS::End( int i_slot )
#endif
{
	if (m_pITool)
	{
...
#ifndef _ToolInterface
bool CUnifyDLL_WLS::SetParameterValue(char* sz_keyword, char* sz_value)
{
...
bool CUnifyDLL_WLS::GetParameterValue(char* sz_keyword, char* sz_value, int i_size)
{
	if (m_pITool)
	{
...
#else
bool CUnifyDLL_WLS::SetParameterValue(const wchar_t* sz_keyword, const wchar_t* sz_value)
{
	CString cstrLogName;
...
bool CUnifyDLL_WLS::GetParameterValue(const wchar_t* sz_keyword, wchar_t* sz_value, int i_size)
{
	CString str_key = CW2CT(sz_keyword);
	CString str_value = CW2CT(sz_value);
...
bool CUnifyDLL_WLS::SetFAData(int i_slot, char* sz_value)
{
	return m_pITool->SetFAData(i_slot, sz_value);
}

bool CUnifyDLL_WLS::GetFAData(int i_slot, char* sz_value, int i_size)
{
...
int CUnifyDLL_WLS::GetItemCount()
{
	return m_pITool->GetItemCount();
}

bool CUnifyDLL_WLS::GetItemByIndex(int iIndex, char* szName, int iSize)
{
...
// --------------------- Test sequence ---------------------
#ifdef _ToolInterface
bool CUnifyDLL_WLS::SetCableloss(const wchar_t *szType, const wchar_t *szBand, const wchar_t *szChannel, const wchar_t *szLoss)
{
	return true; //this->m_pITool->SetCableLoss(CW2A(szType), CW2A(szBand), CW2A(szChannel), CW2A(szLoss));
}

bool CUnifyDLL_WLS::SetTestPart(IToolInterface::ToolTestParts PartID, const wchar_t *szName, const wchar_t *szItem, const wchar_t *szContent, const wchar_t *szAttribute)
{
	bool bRet = true;
...
bool CUnifyDLL_WLS::SetTestItem(const wchar_t *szItem)
{
	return true; //this->m_pITool->SetTestItem(CW2A(szItem));
}

bool CUnifyDLL_WLS::InitTestItem(const wchar_t *szItem)
{
	return true;
}
#endif
...
#ifdef _ToolInterface
bool CUnifyDLL_WLS::OnRunInit(int i_slot)
#else
bool CUnifyDLL_WLS::PreRun(int i_slot)
#endif
{
	m_str_picasso.Empty();
//	SetLogFileName();
...
#ifdef _ToolInterface
bool CUnifyDLL_WLS::RunTestItem(int iItem, int i_slot)
#else
bool CUnifyDLL_WLS::Run(int i_slot)
#endif
{
	return  m_pITool->Run();
}
...
#ifdef _ToolInterface
bool CUnifyDLL_WLS::OnRunFinish(int i_slot)
#else
bool CUnifyDLL_WLS::PostRun(int i_slot)
#endif
{
...
#ifndef _ToolInterface
bool CUnifyDLL_WLS::Stop(int i_slot)
{
...
bool CUnifyDLL_WLS::ResopnseToJig(bool b_wait)
{
...
bool CUnifyDLL_WLS::ResopnseToShowDlg(char* sz_showReslut)
{
...
void CUnifyDLL_WLS::Event(const std::string& strEvent, long nParam)
{
...
void CUnifyDLL_WLS::FireResult( CString str_errorcode, CString str_errrormsg )
{
	st_UIResult sFired;
	sFired.i_slot = 1;
...
//                          INotifyBase_Char
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool CUnifyDLL_WLS::Register(INotify_Char* pNotify,const char* strEvent)
{
	return CNotifyBase_Char::Register(pNotify, strEvent);
}
bool CUnifyDLL_WLS::Register(EventFunc_Char* pEventFunc,const char* strEvent)
{
	return CNotifyBase_Char::Register(pEventFunc, strEvent);
}

bool CUnifyDLL_WLS::Unregister(INotify_Char* pNotify,const char* strEvent)
{
	return CNotifyBase_Char::Unregister(pNotify, strEvent);
}
bool CUnifyDLL_WLS::Unregister(EventFunc_Char* pEventFunc,const char* strEvent)
{
	return CNotifyBase_Char::Unregister(pEventFunc, strEvent);
}

void CUnifyDLL_WLS::DefineNotify(const char* strNewEvent)
{
	return CNotifyBase_Char::DefineNotify(strNewEvent);
}
void CUnifyDLL_WLS::Fire(const char* strEvent,long nParam)
{
	return CNotifyBase_Char::Fire(strEvent, nParam);
}

void CUnifyDLL_WLS::DefineUINotify()
{
	...
	this->DefineNotify(UI_SHOWIMGDIALOG);
}

bool CUnifyDLL_WLS::CheckMD5(CString& strOutMsg)
{
...
void CUnifyDLL_WLS::WriteLog(CString str_log, const int i_severity)
{
	m_GLog.Log(i_severity, str_log);
}

void CUnifyDLL_WLS::SetLogFileName()
{
...
void CUnifyDLL_WLS::SetLogFilePath()
{
	/* Initial log path */
	CString str_path = _T("");
...
bool CUnifyDLL_WLS::MD5Verify(CString str_dirPath, CString str_XMLFilePathName, CString str_logFile)
{
	/* Check Input */
...
bool CUnifyDLL_WLS::CheckVerifyResult(CString str_logFile)
{
	/* Check Input */
	if(str_logFile == _T(""))
	{
