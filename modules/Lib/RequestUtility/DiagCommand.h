...
typedef enum
{
	...
	DIAG_ERROR_GETHWVERSION_FAIL                                                      =35,
	DIAG_ERROR_GETSWVERSION_FAIL                                                      =36,
	DIAG_ERROR_SWHWVERSION_NOTMATCH                                             =37,
	DIAG_ERROR_GETERROR_FAIL								=38
} DIAG_ERROR;
...
PACKET   diagpkt_subsidy_lock_req/resp; Qisda-defined structure
...
PACKET   diagpkt_subsidy_lock_ex_req/resp; Qualcomm-defined structure
...
typedef struct {
	...
	DWORD aps_as_transport; // 0x00000001
	DWORD  subcommand; // 0x00000042
	BYTE slot_id; // 0x01
	BYTE sim_app_id_len; // 0
	BYTE sim_app_id[16];
} diagpkt_subsidy_lock_ex_req;

typedef struct {
	BYTE  cmd_code;			// DIAG_SUBSYS_CMD_VER_2_F (128)
	BYTE  subsys_id;		// DIAG_SUBSYS_GSDI  (33)
	WORD  subsys_cmd_code;	// GSDI_GET_FEATURE_INDICATION_CMD (0x48)
	DWORD command_status;
	WORD delayed_response_id;
	WORD delayed_response_count;
	DWORD aps_as_transport; // 0x00000001
	DWORD subcommand; // 0x00000042
	DWORD status; // 0 means success, otherwises, fail
	BYTE umts_locks_present;
	BYTE cdma_locks_present;
	BYTE sim_nw_feature;
	BYTE sim_ns_feature;
	BYTE sim_sp_feature;
	BYTE sim_cp_feature;
	BYTE sim_sim_feature;
	BYTE ruim_nw1_feature;
	BYTE ruim_nw2_feature;
	BYTE ruim_sp_feature;
	BYTE ruim_cp_feature;
	BYTE ruim_ruim_feature;
	BYTE ruim_hrpd_feature;
} diagpkt_subsidy_lock_ex_resp;
#pragma pack(pop)
/*===========================================================================

PACKET   diagpkt_subsidy_unlock_req/resp; Qisda-defined structure
...
PACKET   diagpkt_subsidy_unlock_req/resp; Qualcomm-defined structure
...
typedef struct {
	...
	WORD subsys_cmd_code;	// 0x48
	DWORD aps_as_transport;	// 0x00000001
	DWORD subcommand;		// 0x00000041
	BYTE slot_id;			// 0x01
	BYTE sim_app_id_len;	// 0
	BYTE sim_app_id[16];	// 0
	DWORD perso_feature;	// lock type
	char perso_feature_key[9]; // unlock code
} diagpkt_subsidy_unlock_ex_req;
...
typedef struct {
	...
	DWORD status; // 0 means success; otherwise, fail
	DWORD perso_feature; // lock type
	DWORD num_retries; // the remaining number of retry
} diagpkt_subsidy_unlock_ex_resp;
#pragma pack(pop)
...
}GSIDDIAG_SIM_SECURE_STORE_REQ_576;
#pragma pack(pop)
...
typedef struct
{
	...
	BYTE sim_app_id[16];
	DWORD simsecure_data_len;
	BYTE simsecure_data[1024];
}GSIDDIAG_SIM_SECURE_STORE_REQ_1024;
#pragma pack(pop)
...
typedef struct
{
	...
	BYTE sim_app_id[16];
	DWORD simsecure_data_len;
	BYTE simsecure_data[2048];
}GSIDDIAG_SIM_SECURE_STORE_REQ_2048;
#pragma pack(pop)
...
typedef struct
{
	...
	DWORD simsecure_data_len;
	BYTE simsecure_data[2048];
}GSIDDIAG_SIM_SECURE_STORE_REQ_NEW;
#pragma pack(pop)
/*===========================================================================
...
typedef struct 
{
	...
	WORD delayed_response_count;
	DWORD status;
}GSIDDIAG_SIM_SECURE_STORE_RESP_NEW;
#pragma pack(pop)
...
===========================================================================*/
typedef enum{
	READ_NV_AND_WRITE_OTP_SUCCESS = 0,
	READ_NV_FAIL = -1,
	WRITE_OTP_FAIL = -2,
	WRITE_OTP_FAIL_VALID_OTP_HAD_EXISTED = -3,
	WRITE_OTP_FAIL_INVALID_OTP_HAD_EXISTED = -4,
	READ_NV_AND_WRITE_OTP_MAX = 0x7FFFFFFF
}Austin_OTP_Freeze_Status;
#pragma pack(push, 1)
typedef struct {
	diagpkt_subsys_hdr_type  header ;
	BYTE  imei[9];
	BYTE  serial_no[16];
	BYTE  drm_key[400];
	BYTE  bt_mac_addr[12];
	BYTE  wlan_mac_addr[12];
	Austin_OTP_Freeze_Status status;
} austin_otp_freeze_rsp_type;
...
typedef struct {
	...
	BYTE  simlockkey[128];
	Austin_OTP_Freeze_Status status;
} C7_otp_freeze_rsp_type;

#pragma pack(pop)

#define DRM_KEY_NV_SIZE 40
