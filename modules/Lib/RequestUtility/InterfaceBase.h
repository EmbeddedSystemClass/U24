...
protected:
	...
	public:
		...
		int GetComport() 
		{
			...
			if(m_ptIRequestManager)
				nComPort = GetCOMPort(m_ptIRequestManager->GetDevice().c_str());

			return nComPort;
		}
	private:
		int GetCOMPort(LPCTSTR lpszDevice)
		{
			int nPortNumber = -1;
			...
			strCOMNum.MakeUpper();		
			int nIndex = strCOMNum.Find(_T("COM"));
			if(nIndex >= 0 )
			{
				strCOMNum = strCOMNum.Mid(nIndex+3);
				nPortNumber = _ttoi(strCOMNum);
			}
			return nPortNumber;
		}
	private:
	...
	public:
		...
		{
			if(m_ptIRequestManager->GetDevice().size() > 5)
			{
				HANDLE hPortEvent = ::CreateEvent(NULL,TRUE,FALSE,GetDeviceID().c_str());
		...
		void AddRestartTimeLog(const CString& strMsg)
		{
			...
			strLog += ": ";
			strLog += strMsg;
			LogPolicy::Log(strLog.GetBuffer(), strFileName);
		...
		TSTRING GetDeviceID()
		{
			TSTRING strDeviceID;
			if(m_ptIRequestManager->GetDevice().size() > 5)
				strDeviceID = m_ptIRequestManager->GetDevice().substr(4);
			return strDeviceID;
		}
		bool ReOepnComport(int nWaitingTime = 10000, bool bUsePNP = false, int nRetryTimes = 20, int nDelayTime = 5000)
		{
			...
			*/
			...
			HANDLE hPortEvent = ::CreateEvent(NULL,TRUE,TRUE,GetDeviceID().c_str());
			int nIndex = 0;
			...
			if(m_ptIRequestManager)
			{
				if(m_ptIRequestManager->Close(m_bKeepProtocolType))
				{
					if (bUsePNP)
					{
						...
						if (nPortInTime - nPortOutTime < 10000)
							Sleep_ByMessageLoop(10000);
...
protected:
	CInterfaceBase(const TSTRING& strDevice, const std::string& strPlatform = "Qualcomm6245")
		:m_strPlatform(strPlatform)
		,m_ptReceiveBuffer(new CBuffer)
		,m_ptSendBuffer(new CBuffer)
	{
		CreateLogFileName(strDevice);

		using namespace PLATFORM_TOOL;
		if(GetIRequestManager(m_ptIRequestManager,strDevice,m_strPlatform)){
			if (( m_strPlatform.find("Qualcomm8x26") == std::string::npos )&&
				( m_strPlatform.find("Qualcomm8x10") == std::string::npos )&&
				( m_strPlatform.find("Qualcomm8916") == std::string::npos ))
			{
				m_ptIRequestManager->Open(); /*for NOT 8x26 & 8x10 platform*/
			}
		}
		else AddLog(_T("m_ptIRequestManager->Open() fail"));
	}
	CInterfaceBase(int nCOMPort, const std::string& strPlatform = "Qualcomm6245")
		...
		,m_ptSendBuffer(new CBuffer)
	{	
		TSTRING strDevice = COMDeviceName(nCOMPort);
		CreateLogFileName(strDevice);
	...
	bool Execute(PLATFORM_TOOL::CRequestPtr& ptRequest)
	{
		...
		if(bRes)
		{
			...
			if(ptRequest->IsHadReturnValue() == false)
...
public:
	...
	TSTRING COMDeviceName( int nPort)
	{
		TCHAR szComport[50] = {0};
