...
public:
	...
	virtual int GeneralCmdInOut(int nPort, int Timeout, char* CtrlBuffer, char* InBuffer, size_t i_InLength, char* OutBuffer) = 0;
	
	//110420 gilpin add for receive serial command such as touch panel pixel and key value 
    virtual int ReceiveCmdOut(int nPort, int Timeout, char* CtrlBuffer, char* OutBuffer) = 0;
