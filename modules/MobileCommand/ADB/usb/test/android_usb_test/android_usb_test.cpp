...
const wchar_t test_interface_name[] = L"\\\\?\\usb#vid_413c&pid_b007#5&1B8ED9A6&0&8#{f72fe0d4-cbcb-407d-8814-9ed673d0dd6b}";
...
bool RunInterfaceEnumTest(bool exclude_not_present,
  ...
  do {
    ...
    if (ERROR_INSUFFICIENT_BUFFER == GetLastError()) {
	  ...
	  if (res) 
	  {
		  if (exclude_not_present && active_only/* && (0 == wcsicmp(info->device_name, test_interface_name))*/) 
		  {
			  device_active = true;
		  }
	  } 
...
bool RunTimeoutsTest() {
  ...
  char write_buf[40960];
  ULONG written, read;
  ULONG small_block = 0x13;
  //000001: Bulk or Interrupt Transfer (UP), 17.09.2009 17:49:03.5528815 +2304.3891505
  //Pipe Handle: 0x89c458bc (Endpoint Address: 0x1)
  //Send 0x3c bytes to the device:
  // 53 53 53 53 53 53 53 53 53 53 53 53 53 53 53 53   SSSSSSSSSSSSSSSS
  // 53 53 53 53 53 53 53 53 53 53 53 53 53 53 53 53   SSSSSSSSSSSSSSSS
  // 53 53 53 53 53 53 53 53 53 53 53 53 53 53 53 53   SSSSSSSSSSSSSSSS
  // 53 53 53 53 53 53 53 53 53 53 53 53               SSSSSSSSSSSS
  //000002: Bulk or Interrupt Transfer (UP), 17.09.2009 17:49:03.5528815 +0.0
  //Pipe Handle: 0x89c4589c (Endpoint Address: 0x81)
  //Get 0x13 bytes from the device:
  // 46 41 49 4C 69 6E 76 61 6C 69 64 20 63 6F 6D 6D   FAILinvalid comm
  // 61 6E 64                                          and
  ...
  for (int n = 0; n < 8; n++) {
    ...
    bool rd_res = AdbReadEndpointSync(adb_read_endpoint, read_buf, small_block, &read, 1000);
