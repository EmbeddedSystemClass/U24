...
int CmdCtrl::open(int nPort, int nTimeout)
{
	...
	while (m_pCommDrv->open(nPort, 115200, NOPARITY, ONESTOPBIT, 8) < 0) {
		nCount++;
		TRACE(_T("m_pCommDrv->open:%d(%d)\n"), nPort, nCount);
		if (nPort >= QTH_PORT_TCP) {
			if (nCount > 10) { // only retry 10 times
				return HS_TIMEOUT;
			}
			Sleep(0);
		}
...
int CmdCtrl::GeneralCmdOut(int nPort, int nTimeout, char* CtrlBuffer, char* OutBuffer)
{
	...
	unsigned char cGenPayload[12];
	memset(cGenPayload, 0, sizeof(cGenPayload));
	memcpy(cGenPayload, CtrlBuffer, 4*sizeof(unsigned char));
...
int CmdCtrl::GeneralCmdInOut(int nPort, int nTimeout, char* CtrlBuffer, char* InBuffer, size_t nInLength, char* OutBuffer)
{
	if (m_pCommDrv == NULL) {
		return HS_NOT_INITIALIZE;
	}
	...
	nRetCode = ExactGeneralCmdInOut(nPort, nTimeout, CtrlBuffer, InBuffer, nInLength, OutBuffer);
	if (nRetCode != HS_OK) {
...
//110420 gilpin add for receive serial command such as touch panel pixel and key value 
int CmdCtrl::RecvCommand(int PayloadSize, unsigned char* Command, unsigned char* Payload, unsigned char* Output, int nPort, int nTimeout)
{
	//TRACER_FUNCTION_IN();
	if (m_pCommDrv == NULL) {
		return HS_NOT_INITIALIZE;
	...
	memcpy(cGenPayload + 4, Payload, PayloadSize);
	...
	while (1) {
		...
		ReceiveResult = QthGenPacketRecv(DT_PKT_RESULT, cGenPayload, pPackData, &NewChecksum, nTimeout);
		int nRetCode = PacketResultCheck(ReceiveResult, pPackData);
		if (nRetCode == HS_OK) {
			memcpy(Output, (pPackData + 1), PAYLOAD_SIZE*sizeof(unsigned char)); // FIXME buffer size
			return HS_OK;
		}
		else if (nRetCode != HS_PROGRESS) {
			TRACE(_T("%s PacketResultCheck(%d) fail\r\n"), __FUNCTION__, nRetCode);
			TRACER_STREAM(__FUNCTION__ << " PacketResultCheck port:" << nPort << " code:" << nRetCode << "\r\n");
			return HS_FAIL;
		}
	}
	return HS_FAIL;
...
int CmdCtrl::ExactReceiveCmdOut(int nPort, int nTimeout, char* CtrlCmd, char* OutBuffer)
{
	...
	unsigned char cGenPayload[12];
	memset(cGenPayload, 0, sizeof(cGenPayload));
	memcpy(cGenPayload, CtrlCmd, 4);
	int PayloadSize = 0;
	if (nPort >= QTH_PORT_USB) {
		m_bIfUSBCmd = true;
	}
	int nRetCode = RecvCommand(PayloadSize, cGenPayload, cGenPayload + 4, (unsigned char *)OutBuffer, nPort, nTimeout);
	if (m_bIfUSBCmd) {
		m_bIfUSBCmd = false;
	}
	...
	if (nRetCode != HS_OK) {
		...
		TRACER_STREAM(__FUNCTION__ << " RecvCommand port:" << nPort << " code:" << nRetCode << "\r\n");
		return nRetCode;
	}
...
int CmdCtrl::QthGenCommand(int nSize, unsigned char* command, unsigned char* payload, unsigned char* buffer, int nPort, int nTimeout)
{
	TRACER_FUNCTION_IN();
	if (m_pCommDrv == NULL) {
		return HS_NOT_INITIALIZE;
	...
	unsigned char cGenPayload[PAYLOAD_SIZE]; // FIXME buffer size
	memcpy(cGenPayload, command, 4);
	memcpy(cGenPayload+4, payload, nSize); //FF:如果有參數的話copy 端看傳進來的Payloadsize是多少
	...
	SendResult = QthGenPacketSend(DT_PKT_REQUEST, nSize, cGenPayload, &OldChecksum, nPort);
	if (SendResult < HS_OK) {
		TRACER_STREAM("QthGenPacketSend:" << ReceiveResult << "\r\n");
	...
	unsigned char pPackData[PAYLOAD_SIZE];
	memset(pPackData, 0, sizeof(pPackData));
	ReceiveResult = QthGenPacketRecv(DT_PKT_ACK, cGenPayload, pPackData, &NewChecksum, nTimeout);
	if (ReceiveResult != HS_OK) {
		TRACER_STREAM("QthGenPacketRecv ack:" << ReceiveResult << "\r\n");
	...
	if (OldChecksum != NewChecksum) {
		...
		TRACE(_T("%s checksum fail(%x != %x)\r\n"), __FUNCTION__, OldChecksum, NewChecksum);
	...
	while (1) {
		...
		if (nRetCode == HS_OK) {
			memcpy(buffer, (pPackData + 1), PAYLOAD_SIZE*sizeof(unsigned char)); // FIXME buffer size
			return nRetCode;
		}
		else if (nRetCode != HS_PROGRESS) {
			memcpy(buffer, (pPackData + 1), PAYLOAD_SIZE*sizeof(unsigned char)); // FIXME buffer size
...
int CmdCtrl::ExactGeneralCmd(int nPort, int nTimeout, char* CtrlCmd)
{
	...
	int nPayloadSize = 0;
	int nRetCode = QthGenCommand(nPayloadSize, cGenPayload, cGenPayload + 4, pPackData, nPort, nTimeout);
	if (m_bIfUSBCmd) {
		m_bIfUSBCmd = false;
	}
...
int CmdCtrl::ExactGeneralCmdOut(int nPort, int nTimeout, char* CtrlCmd, char* OutBuffer)
{
	...
	unsigned char cGenPayload[12];
	memset(cGenPayload, 0, sizeof(cGenPayload));
	memcpy(cGenPayload, CtrlCmd, 4*sizeof(unsigned char));
	int PayloadSize = 0;
	if (nPort >= QTH_PORT_USB) {
		m_bIfUSBCmd = true;
	}
	int nRetCode = QthGenCommand(PayloadSize, cGenPayload, cGenPayload + 4, (unsigned char *)OutBuffer, nPort, nTimeout);
	if (m_bIfUSBCmd) {
		m_bIfUSBCmd = false;
	}

	if (nRetCode != HS_OK) {
		TRACER_STREAM("!" << __FUNCTION__ << ":" << nRetCode << "\r\n");
...
// TODO: loger timeout for new atool rssi command to 12 sec
// TODO: move timeout setting to HWTool
int CmdCtrl::QthGenPacketRecv(int type, unsigned char * PayloadCommand, unsigned char * pReceiveData, unsigned char * NChecksum, int nTimeout)
{
	//TRACER_FUNCTION_IN();
	...
	while (1) {
		...
		if (m_pCommDrv->read(data, 1, nTimeout) != 1) {
			TRACER_STREAM("QthGenPacketRecv while B, timeout:" << nTimeout << "\r\n");
			return HS_READ_DATA_FAIL;
		}
		if (data[0] != 'B') {
			continue;
		...
		if (data[0] != 'E') { //FF: try to read E

			/*2012.5.10. Lion Fix Double 'B' Bug -> B B E N Q*/
			int isSecondB = 0;
			if (data[0] == 'B') { // B[B]E N Q
				isSecondB = 1;
				if (m_pCommDrv->read(data, 1, 300) != 1) {
					TRACER_STREAM("QthGenPacketRecv while E\r\n");
					return HS_READ_DATA_FAIL;
		...
		if (data[0] != 'Q') { //FF: try to read Q
			continue;
		}
		temp[3] = data[0];
		TRACE("[Q]"); //BEN[Q]
		TRACE(", get command");//get command type
		...
		if (data[0] == type || progress_flag) {
			...
			if (m_pCommDrv->read(data, 4, 300) != 4) {
			...
			TRACE("%s payload size: %d\n", __FUNCTION__, nPayloadSize);
			int nRead = m_pCommDrv->read(temp + 13, nPayloadSize + 1, 5000);
			if (nRead != nPayloadSize + 1) {				
				TRACER_STREAM("!" << __FUNCTION__ << ",nRead:" << nRead << "\r\n");
				TRACE("%s packet read size fail(%d!=%d)\n", __FUNCTION__, nRead, nPayloadSize);
				return HS_READ_DATA_FAIL;
			}
...
int CmdCtrl::ExactUSBGenDownloadCmd(int nPort, int nTimeout, char* DLFileType, char* FileName)
{
	...
	if (DLFileType[0] == 1 && DLFileType[1] == 2) {
		...
		memset(cGenPayload, 0, sizeof(cGenPayload));
		memcpy(cGenPayload, DLFileType, 4*sizeof(unsigned char));//1200
		...
		while (1) {
			...
			memset(pPackData, 0, sizeof(pPackData));
			ReceiveResult = QthGenPacketRecv(P21_DATADL_ACK, cGenPayload, pPackData, &NewChecksum, nTimeout);
			if (ReceiveResult != 1) {
				/////////////////////////////////////////////////////////////////////
...
| Returns     : int, 1 represent success
+----------------------------------------------------------------------------*/
int CmdCtrl::UARTPutHugeData(BaseDrv *pCom, int PortNum, unsigned char * pPackData, int Length, int nTimeout)
{
...
int CmdCtrl::ExactGeneralCmdIn(int nPort, int nTimeout,
							   char* CtrlCmd, char* InBuffer, size_t nInSize)
{
	if (m_pCommDrv == NULL) {
		return HS_NOT_INITIALIZE;
	}
	...
	int nPayloadSize = static_cast<int>(nInSize);//size = 4
	unsigned char cGenPayload[12];
	memset(cGenPayload, 0, sizeof(cGenPayload));
	memcpy(cGenPayload, CtrlCmd, 4*sizeof(char));
	...
	int nRetCode = QthGenCommand(nPayloadSize, cGenPayload, (unsigned char *)InBuffer, pPackData, nPort, nTimeout);
	if (m_bIfUSBCmd) {
		m_bIfUSBCmd = false;
	}
	if (nRetCode != HS_OK) {
...
int CmdCtrl::ExactUSBGenUploadCmd(int nPort, int nTimeout, char* CtrlBuffer, char* WorkBuffer)
{
	...
	SendResult = QthGenPacketSend(DT_PKT_REQUEST, PayloadSize, cGenPayload, &OldChecksum, nPort);
	ReceiveResult = QthGenPacketRecv(DT_PKT_ACK, cGenPayload, (unsigned char *)pPackData, &NewChecksum, nTimeout);
	if (ReceiveResult != HS_OK) {
		TRACE(_T("%s QthGenPacketRecv(%d) fail\r\n"), __FUNCTION__, ReceiveResult);
		return ReceiveResult;
	}
	...
	for (;;) {
		Sleep(20);
		memset(pPackData, 0, sizeof(pPackData));
		ReceiveResult = QthGenPacketRecv(DT_PKT_RESULT, cGenPayload, (unsigned char *)pPackData, &NewChecksum, nTimeout);
		nRetCode = PacketResultCheck(ReceiveResult, pPackData);
		if (nRetCode == HS_OK) {
			int ij = -10;
			int sum = 0;
	...
	while (index < iBlockNum) {
		...
		if (retransfer > 0) {
			for (int i = 0; i < retransfer; i++) {
				ReceiveResult = QthGenPacketRecv(P21_DATAUL_ACK, cGenPayload, (unsigned char *)pPackData, &NewChecksum, nTimeout);
				if (ReceiveResult == 1) {
					retransfer--;
				}
				else {
...
int CmdCtrl::ExactGeneralCmdInOut(int nPort, int nTimeout, char* CtrlBuffer, char* InBuffer, size_t i_InLength, char* OutBuffer)
{
	...
	int nRetCode = QthGenCommand(PayloadSize, cGenPayload, (unsigned char *)InBuffer, pPackData, nPort, nTimeout);
	if (m_bIfUSBCmd) {
		m_bIfUSBCmd = false;
	}
	//萬一傳回值比較大怎麼辦...??一定會錯
	...
	if (nRetCode != HS_OK) {
		TRACER_STREAM("QthGenCommand(" << nRetCode << ") fail\r\n");
...
int CmdCtrl::ExactBDTLSectorDownloadCmd(int nPort, int nTimeout, char* CmdNumber, char* FilePath, char* parameter)
{
	...
	memset(cGenPayload, 0, sizeof(cGenPayload));
	memcpy(cGenPayload, CmdNumber, 4*sizeof(unsigned char));
...
int CmdCtrl::ExactBDTLSectorUploadCmd(int nPort, int nTimeout, char* CmdNumber, char* FilePath, char* parameter)
{
	...
	memcpy(cGenPayload, CmdNumber, 4*sizeof(char));
	PayloadSize = 4;
	PayloadSize = (int)strlen(parameter);
	strncpy((char*)cGenPayload + 4, parameter, strlen(parameter));
	SendResult = QthGenPacketSend(DT_PKT_REQUEST, PayloadSize, cGenPayload, &OldChecksum, nPort);
...
int CmdCtrl::PacketResultCheck(int iPacketResult, unsigned char * pPackData)
{
	if (iPacketResult == HS_PROGRESS) {
		...
		TRACER_STREAM(" =>recv packet fail:" << (char*)pPackData << "\r\n");
	...
	if (pPackData[0] == '1') {
		...
		TRACER_STREAM(" =>recv packet fail(0):" << (char*)pPackData << "\r\n");
		return HS_FAIL;
	}
	else {
		TRACER_STREAM(" =>recv packet fail(other):" << (char*)pPackData << "\r\n");
		return HS_FAIL;
	}
}
