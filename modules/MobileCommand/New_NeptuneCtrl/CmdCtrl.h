...
public:
	...
	virtual int GeneralCmdInOut(int nPort, int Timeout, char* CtrlBuffer, char* InBuffer, size_t nInLength, char* OutBuffer);
	...
	int ExactGeneralCmdIn(int nPort, int Timeout, char* CtrlCmd, char* InBuffer, size_t nInSize);
	//110420 gilpin add for receive serial command such as touch panel pixel and key value 
...
private:
	...
	int UARTPutHugeData(BaseDrv *pCom, int PortNum, unsigned char* pPackData, int Length, int nTimeout);

	int QthGenPacketSend(int Type, int PayLoadSize, unsigned char* pPackData, unsigned char* OChecksum, int nPort);
	int QthGenPacketRecv(int type, unsigned char* PayloadCommand, unsigned char* pReceiveData, unsigned char* NChecksum, int nTimeout);
	int QthGenCommand(int nSize, unsigned char* command, unsigned char* payload, unsigned char* buffer, int nPort, int nTimeout);
	//110420 gilpin add for receive serial command such as touch panel pixel and key value 
	int RecvCommand(int PayloadSize, unsigned char* Command, unsigned char* Payload, unsigned char* Output, int nPort, int nTimeout);
};
