...
int IGPIBDevice::g_iGPIBCardAddress = 0;//Primary Address for Tester Device
...
std::string IGPIBDevice::m_strDeviceName;
int IGPIBDevice::m_iNI488Address = 0;
int IGPIBDevice::m_iPAD = 0;
int IGPIBDevice::m_iSAD = 0;
int IGPIBDevice::m_iDD = 0;
...
bool IGPIBDevice::DetectGPIBDevice(int iGPIBInterfaceID, int iGPIBCardAddress)
{
	...
	for (int i = 0; i < iListnerCount; ++i)
	{
		...
		int iDD = ibdev(g_iGPIBInterfaceID, iPAD, iSAD, T10s, 1, 0);
		if (ibsta & ERR)
			return false;
...
bool IGPIBDevice::InitTestDeviceData(std::string strDeviceName, int iNI488Address, int iPAD, int iSAD)
{
...
bool IGPIBDevice::InitPowerSupplyData(std::string strDeviceName, int iNI488Address, int iPAD, int iSAD)
{
	m_strDeviceName = strDeviceName;
	m_iNI488Address = iNI488Address;
	m_iPowerSupplyPAD = iPAD;
	m_iPowerSupplySAD = iSAD;

	m_iPowerSupplyDD = ibdev(g_iGPIBInterfaceID, m_iPowerSupplyPAD, m_iPowerSupplySAD, T1s, 1, 0);
	if (ibsta & ERR)
		return false;
...
bool IGPIBDevice::TestDeviceIsAvaliable()
{
...
bool IGPIBDevice::PowerSupplyIsAvaliable()
{
	if (m_strDeviceName.empty() || m_iNI488Address == -1 || m_iPowerSupplyPAD == -1 || m_iPowerSupplySAD == -1 || m_iPowerSupplyDD == -1)
		return false;
	else
		return true;
}
...
bool IGPIBDevice::GPIB_PS_Querry(std::string strCommand, std::string& strResponse)
{
	char szCmd[4096];
	memset(szCmd, 0, 4096);
	char szRead[524288];
...
bool IGPIBDevice::GPIB_PS_Write(std::string strCommand)
{
	char szCmd[4096];
	memset(szCmd, 0, 4096);
