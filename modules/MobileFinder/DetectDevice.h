...
#include "..\..\lib\loki-0.1.5\include\loki\Singleton.h"
#include "..\Common\CommonUtil\SerialPort.h"
...
#define MAX_VALUE_LENGTH 16383
	...
	public:
			...
			,m_hDeviceConnectionStatusEvent(FALSE,TRUE,_T("MobileDeviceConnectionEvent"))
		{
			...
			DefineNotify(DEVICE_ADB_CONNECT);
			DefineNotify(DEVICE_ADB_DISCONNECT);
			//dave
			DefineNotify(DEVICE_STORAGE_CONNECT);
			DefineNotify(DEVICE_STORAGE_DISCONNECT);
			//dave
			CreateGhostWnd();
		...
		bool RemoveNotify(const TSTRING& strSymboic,const TSTRING& strEvent);
		...
		TSTRING GetUSBDeviceInfo(const USHORT idProduct,const USHORT idVendor,const TSTRING& strPara,const TSTRING& strClass,const TSTRING& strSunFolder,const TSTRING& strField,TSTRING& strSymbolic);
		...
		void AddUSBNotify(const TSTRING& strDeviceClass,const TSTRING& strEvent);
		//dave
		void AddStorageNotify(const TSTRING& strDeviceClass, const TSTRING& strEvent);
		//dave
		bool RemovePortNotify(const TSTRING& strPort,const TSTRING& strEvent);
		...
		const bool IsADBDLDevice(const TSTRING& strSymbolicName);
		bool IsReallyCOMPort(const TSTRING& strPortName);
		TSTRING GetQualcommPortName(HKEY hKey,const TSTRING& strSubKey);

		bool EnumerateDeviceInterfaces(GUID class_id,bool exclude_removed = true,bool active_only=  true,ULONG flags = DIGCF_DEVICEINTERFACE | DIGCF_PRESENT);
