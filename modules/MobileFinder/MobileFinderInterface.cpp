	...
	public:	
		virtual bool Register(INotify* pNotify)
		{
			...
			if(pNotify)
			{
				...
				bRes = bRes && m_DectectDevice.Register(pNotify,DEVICE_ADB_CONNECT);
				bRes = bRes && m_DectectDevice.Register(pNotify,DEVICE_ADB_DISCONNECT);
				//dave
				bRes = bRes && m_DectectDevice.Register(pNotify,DEVICE_STORAGE_CONNECT);
				bRes = bRes && m_DectectDevice.Register(pNotify,DEVICE_STORAGE_DISCONNECT);
				//dave
			}
			return bRes;
		}
		virtual bool Unregister(INotify* pNotify)
		{
			...
			if(pNotify)
			{
				...
				bRes = bRes && m_DectectDevice.Unregister(pNotify,DEVICE_ADB_CONNECT);
				bRes = bRes && m_DectectDevice.Unregister(pNotify,DEVICE_ADB_DISCONNECT);
				//dave
				bRes = bRes && m_DectectDevice.Unregister(pNotify,DEVICE_STORAGE_CONNECT);
				bRes = bRes && m_DectectDevice.Unregister(pNotify,DEVICE_STORAGE_DISCONNECT);
				//dave
			}
			return bRes;
		}
