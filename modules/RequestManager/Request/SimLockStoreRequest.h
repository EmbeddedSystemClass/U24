...
namespace PLATFORM_TOOL
{
	...
	class RM_CLASS_API CSimLockStoreRequest: public CDiagRequestBase
	{
		public:
			...
			CSimLockStoreRequest(const GSIDDIAG_SIM_SECURE_STORE_REQ_1024 &simReq, DWORD &cmdStatus, DWORD &status);
			CSimLockStoreRequest(const GSIDDIAG_SIM_SECURE_STORE_REQ_2048 &simReq, DWORD &cmdStatus, DWORD &status);
		...
		private:
			...
			GSIDDIAG_SIM_SECURE_STORE_REQ_1024 m_SimLockReq_1024;
			GSIDDIAG_SIM_SECURE_STORE_REQ_2048 m_SimLockReq_2048;
			DWORD &m_CmdStatus;
			DWORD &m_Status;		
	};
	...
	// Query Sim Lock Status; use Qualcomm-defined command structure
	class RM_CLASS_API CSimLockStatusRequestEx: public CDiagRequestBase
	{
		public:
			CSimLockStatusRequestEx(diagpkt_subsidy_lock_ex_resp &lockResp);
			virtual ~CSimLockStatusRequestEx(void);
			virtual void OnDataReceiver(CBufferPtr& ptBufferReceived);
			virtual bool IsCompletePacket(CBufferPtr& ptBufferReceived);

		private:
			diagpkt_subsidy_lock_ex_req  m_Req;
			diagpkt_subsidy_lock_ex_resp  &m_Resp;
	};
	...
	// Unlock Simlock; use Qualcomm-defined command structure
	class RM_CLASS_API CSimUnLockRequestEx: public CDiagRequestBase
	{
	public:
		CSimUnLockRequestEx(int nUnLockType, char *sCode, diagpkt_subsidy_unlock_ex_resp &unlockResp);
		virtual ~CSimUnLockRequestEx(void);
		virtual void OnDataReceiver(CBufferPtr& ptBufferReceived);
		virtual bool IsCompletePacket(CBufferPtr& ptBufferReceived);

	private:
		diagpkt_subsidy_unlock_ex_req m_Req;
		diagpkt_subsidy_unlock_ex_resp &m_Resp;
	};

	class RM_CLASS_API CSimTempUnLockRequest: public CDiagRequestBase
	{
	public:		
		CSimTempUnLockRequest(int nUnLockType, char *sCode, diagpkt_subsidy_unlock_resp &unlockResp);
		virtual ~CSimTempUnLockRequest(void);
		virtual void OnDataReceiver(CBufferPtr& ptBufferReceived);
		virtual bool IsCompletePacket(CBufferPtr& ptBufferReceived);
	...
	************************************************************************/
	...
	/************************************************************************
	// Class:    CWriteSimDataNewRequest
	// Purpose:  For writing sim data, new way from Boston
	...
	************************************************************************/
	class RM_CLASS_API CWriteSimDataNewRequest: public CDiagRequestBase
	{
	public:
		CWriteSimDataNewRequest(BYTE *pSimDataBuf, int nSimDataBufLen, bool& bExecuteResult, DWORD iSlotId);
		virtual ~CWriteSimDataNewRequest();
		virtual bool IsCompletePacket(CBufferPtr& ptBufferReceived);
		virtual void OnDataReceiver(CBufferPtr& ptBufferReceived);
	private:
		bool& m_bExecuteResult;
	...
	************************************************************************/
	...
	class RM_CLASS_API CQuerySIMLockNameRequest: public CDiagRequestBase
	{
	public:
		CQuerySIMLockNameRequest(char *szSIMLockName, bool& nStatus);
		virtual ~CQuerySIMLockNameRequest();
		virtual bool IsCompletePacket(CBufferPtr& ptBufferReceived);
		virtual void OnDataReceiver(CBufferPtr& ptBufferReceived);
	private:
		bool& m_bExecuteResult;
		char *m_szSIMLockName;
	};

	class RM_CLASS_API CQuerySIMLockNameNewRequest: public CDiagRequestBase
	{
	public:
		CQuerySIMLockNameNewRequest(char *szSIMLockName, bool& nStatus);
		virtual ~CQuerySIMLockNameNewRequest();
		virtual bool IsCompletePacket(CBufferPtr& ptBufferReceived);
		virtual void OnDataReceiver(CBufferPtr& ptBufferReceived);
	private:
		bool& m_bExecuteResult;
