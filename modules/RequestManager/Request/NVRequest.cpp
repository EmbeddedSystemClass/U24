...
namespace PLATFORM_TOOL
{
	...
	CNVRequest::CNVRequest(const WORD& NVItem, const WORD& nDIAGcmd, DIAG_NVITEM_PACKET_F_type& NVResp, const BYTE *WriteData, const size_t szWriteData)
		:CDiagRequestBase(_T("CNVRequest_V0"))
		,m_NVItem(NVItem)
		,m_ReadResp(NVResp)
		,m_bSucess(m_bSucess_ForTemp)
		,m_nVersion(0)
	{
		m_bSucess = false;
		memset(&m_ReadResp, 0, sizeof(m_ReadResp));

		m_ReadResp.nDIAGcmd = (uint8) nDIAGcmd;
		...
		Encode((BYTE*)&m_ReadResp, sizeof(m_ReadResp), ptBufferSend);
		SetSendData(ptBufferSend);
	}

	CNVRequest::CNVRequest(const WORD& NVItem, const WORD& nDIAGcmd, DIAG_NVITEM_PACKET_F_type& NVResp,bool& bSucess, const BYTE *WriteData, const size_t szWriteData)
		:CDiagRequestBase(_T("CNVRequest_V1"))
		,m_NVItem(NVItem)
		,m_ReadResp(NVResp)
		,m_bSucess(bSucess)
		,m_nVersion(1)
	{
		WORD nv_stat = m_ReadResp.nv_stat;
		m_bSucess = false;
		memset(&m_ReadResp, 0, sizeof(m_ReadResp));
	...
	void CNVRequest::OnDataReceiver(CBufferPtr& ptBufferReceived) 
	{
		...
		*/		
		...
		//::memcpy(&m_ReadResp,ptBufferReceived->GetData(),sizeof(DIAG_NVITEM_PACKET_F_type));
		Safememcopy((void*) &m_ReadResp, sizeof(DIAG_NVITEM_PACKET_F_type) , (void*) ptBufferReceived->GetData(), ptBufferReceived->GetLength());
		if(m_ReadResp.nDIAGcmd == 0x13)
			m_bSucess = false;
		else
			m_bSucess = true;
	...
	bool CNVRequest::IsCompletePacket(CBufferPtr& ptBufferReceived)
	{
		...
		if (bRes)
		{
			...
			*/
			...
			{
				if(szBuffer[1] == m_ReadResp.nDIAGcmd && m_nVersion == 1)
					bRes = true;//0x13 means not support this DIAG command
				else
					bRes = false;//Unknow need to check
			}
			else
	...
	//////////////////////////////////////////////////////////////////////////
	CClearActivationDataRequest::CClearActivationDataRequest(bool& bIsCmdSuccess,bool bNewVersion)
		:CDiagRequestBase(_T("CClearActivationDataRequest"))
		,m_bIsCmdSuccess(bIsCmdSuccess) 
		,m_bNewVersion(bNewVersion)
	{
		const BYTE pSendBuf_Old[] = {0x4b, 0x43, 0x03, 0x00};
		const BYTE pSendBuf_New[] = {0x4b, 0xF7, 0x03, 0x00};
		CBufferPtr ptBufferSend;
		if(m_bNewVersion)
			Encode(pSendBuf_New, 4, ptBufferSend);
		else
			Encode(pSendBuf_Old, 4, ptBufferSend);
		SetSendData(ptBufferSend);
	}
	...
	bool CClearActivationDataRequest::IsCompletePacket(CBufferPtr& ptBufferReceived) 
	{
		...
		,m_bIsFTM(bIsFTM)
	{
		...
		BYTE szBufSend[0x20] = {0x4b, 0x0b, 0x18, 0x00, 0x01, 0x00, 0x16, 0x00, 0x0c, 0x00};
		szBufSend[10] = m_parameters.chain & 0x00ff;
		szBufSend[11] = (m_parameters.chain & 0xff00) >> 8;

		szBufSend[12] = (BYTE)(m_parameters.LSB & 0x000000ff);
		szBufSend[13] = (BYTE)((m_parameters.LSB & 0x0000ff00) >> 8);
		szBufSend[14] = (BYTE)((m_parameters.LSB & 0x00ff0000) >> 16);
		szBufSend[15] = (BYTE)((m_parameters.LSB & 0xff000000) >> 24);

		szBufSend[16] = (BYTE)(m_parameters.MSB & 0x000000ff);
		szBufSend[17] = (BYTE)((m_parameters.MSB & 0x0000ff00) >> 8);
		szBufSend[18] = (BYTE)((m_parameters.MSB & 0x00ff0000) >> 16);
		szBufSend[19] = (BYTE)((m_parameters.MSB & 0xff000000) >> 24);

		szBufSend[20] = (BYTE)(m_parameters.RegConf2 & 0x000000ff);
		szBufSend[21] = (BYTE)((m_parameters.RegConf2 & 0x0000ff00) >> 8);
		szBufSend[22] = (BYTE)((m_parameters.RegConf2 & 0x00ff0000) >> 16);
		szBufSend[23] = (BYTE)((m_parameters.RegConf2 & 0xff000000) >> 24);

		szBufSend[24] = (BYTE)(m_parameters.RegConf3 & 0x000000ff);
		szBufSend[25] = (BYTE)((m_parameters.RegConf3 & 0x0000ff00) >> 8);
		szBufSend[26] = (BYTE)((m_parameters.RegConf3 & 0x00ff0000) >> 16);
		szBufSend[27] = (BYTE)((m_parameters.RegConf3 & 0xff000000) >> 24);

		szBufSend[28] = (BYTE)(m_parameters.RegConf4 & 0x000000ff);
		szBufSend[29] = (BYTE)((m_parameters.RegConf4 & 0x0000ff00) >> 8);
		szBufSend[30] = (BYTE)((m_parameters.RegConf4 & 0x00ff0000) >> 16);
		szBufSend[31] = (BYTE)((m_parameters.RegConf4 & 0xff000000) >> 24);
	...
	{		
		...
		Encode(bufSend,17,ptBufferSend);
		SetSendData(ptBufferSend);
	}
	...
	{
		CBufferPtr ptBufferSend;
		BYTE bufSend[] = {0x4b, 0x21, 0x17, 0x00};
	...
	CCheckSIMICE::CCheckSIMICE(int* nState) 
		: CDiagRequestBase(_T("CheckSIMICE"))
		,m_pnState(nState) 
	{
		CBufferPtr ptBufferSend;
		BYTE bufSend[] = {0x4b, 0x21, 0x13, 0x00};
		Encode(bufSend,4,ptBufferSend);
		SetSendData(ptBufferSend);
	}
	CCheckSIMICE::~CCheckSIMICE() {}
	void CCheckSIMICE::OnDataReceiver(CBufferPtr& ptBufferReceived) {		
		const char* retVal = ptBufferReceived->GetData();
		*m_pnState = retVal[4];
		CDiagRequestBase::OnDataReceiver(ptBufferReceived);
	}
	bool CCheckSIMICE::IsCompletePacket(CBufferPtr& ptBufferReceived) {
		return CDiagRequestBase::IsCompletePacket(ptBufferReceived);
	}
	int CCheckSIMICE::GetState() {
		return *m_pnState;
	}
	//////////////////////////////////////////////////////////////////////////
	//
	...
	{				
		...
		if(m_nFunction == 3)
		{
			...
			Encode((BYTE*)&req,sizeof(GET_NV_PARTITION_STATUS_REQ),ptBufferSend);
		}
		else
		{			
			diagpkt_subsys_hdr_type_v2 req = {0};
	...
	{		
		...
		if(m_nFunction == 3)
		{
			...
			Safememcopy((void*) &resp, sizeof(GET_NV_PARTITION_STATUS_RESP) , (void*) retVal, ptBufferReceived->GetLength());
			if(resp.subsys_hdr.command_code == 0x4B)
			{
				m_nBackupTimes = resp.cnv_backup_checked.BackupCount == 0xFFFF ? 0 :  resp.cnv_backup_checked.BackupCount;
		...
		else
		{
			...
			//::memcpy(&resp,retVal,sizeof(diagpkt_subsys_hdr_type_v2));
			Safememcopy((void*) &resp, sizeof(diagpkt_subsys_hdr_type_v2) , (void*) retVal, ptBufferReceived->GetLength());
	...
	{
		CBufferPtr ptBufferSend;
		BYTE bufSend[] = {0x4b, 0x24, 0x05, 0x00};
		Encode(bufSend,4,ptBufferSend);
		SetSendData(ptBufferSend);
	}
	...
	//
	//////////////////////////////////////////////////////////////////////////
	CReadQFuseState::CReadQFuseState(unsigned long* reg1, unsigned long* reg2, unsigned long* reg3, unsigned long* reg4, unsigned long* reg5, bool* bSuccess)
	...
	{		
		...
		else if(retVal[0] == 0x4b) 
		{
			...
			*m_pReg3 = *((DWORD*)&retVal[20]);
			*m_pReg4 = *((DWORD*)&retVal[24]);
			*m_pReg5 = *((DWORD*)&retVal[28]);
		}
		CDiagRequestBase::OnDataReceiver(ptBufferReceived);
	}
	...
	void CReadFTMNV::OnDataReceiver(CBufferPtr& ptBufferReceived) {
		CDiagRequestBase::OnDataReceiver(ptBufferReceived);
		BYTE szCheckValue[] = {0x26, 0xc5, 0x01};
		const char* retVal = ptBufferReceived->GetData();
		int length = ptBufferReceived->GetLength();
	...
	{
		const BYTE szSendCmd[6] = {0x4b, 0x24, 0x07, 0x00, 0x00, 0x00};
		CBufferPtr ptBufferSend;        
	...
	{ 
		...
		*/
        char szCmpBuf[] = {0x4b, 0x24, 0x07,0x00};
        const unsigned char* retVal = (const unsigned char*)ptBufferReceived->GetData();
...
//            m_nTemperature -= 273;
		...
		, m_nRespDataSize(nRespDataSize)
	{
		...
		CBufferPtr ptBufferSend;
		Encode((BYTE*)pReqData, sizeof(BYTE)*nReqDataSize, ptBufferSend);
		SetSendData(ptBufferSend);
	}
		...
		,m_pPRLData(pPRLData), m_nPRLDataSize(nPRLDataSize), m_Status(Status), m_bRead(bRead)
	{
		...
		if (bRead)
		{
			...
			rd_req.cmd_code = 73;
			Encode((BYTE*)&rd_req, sizeof(DIAG_PR_LIST_RD_REQ_TYPE), ptBufferSend);
		}
		else
		{
			...
			Encode((BYTE*)&wr_req, sizeof(DIAG_PR_LIST_WR_REQ_TYPE), ptBufferSend);
		}		
	...
	{	
		if (m_bRead)
		{
			...
			Safememcopy((void*)&rd_resp,sizeof(rd_resp),(void*)ptBufferReceived->GetData(),ptBufferReceived->GetLength());
		...
		else
		{
			...
			Safememcopy((void*)&wr_resp,sizeof(wr_resp),(void*)ptBufferReceived->GetData(),ptBufferReceived->GetLength());
	...
	void CIsEMURequest::OnDataReceiver(CBufferPtr& ptBufferReceived)
	{
		...
		//memcpy(&resp, ptBufferReceived->GetData(),sizeof(diagpkt_IsEMU));
		Safememcopy((void*)&resp,sizeof(resp),(void*)ptBufferReceived->GetData(),ptBufferReceived->GetLength());
	...
	void CWriteNVItemRequest::OnDataReceiver(CBufferPtr& ptBufferReceived)
	{
		...
		//memcpy(&resp, ptBufferReceived->GetData(),sizeof(DIAG_NVITEM_PACKET_F_type));
		Safememcopy((void*)&resp,sizeof(resp),(void*)ptBufferReceived->GetData(),ptBufferReceived->GetLength());
	...
	{
		/*
		Send 2009\07\15 15:18:49
		4B 0B 0B 00 00 00 00 00 00 00 00 00 02 07 0F D9 07 07 12 31 68 DC 7E
		Read 2009\07\15 15:18:49
		4B 0B 0B 00 00 10 00 00 00 00 00 00 2C 50 7E
		*/
		m_bExecuteResult = false;

		//For example 
		// 2009/07/15 03:19:17 (+8)
		//char req[] = {0x4B ,0x0B ,0x0B ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x02 ,0x07 ,0x0F ,0xD9 ,0x07 ,0x07 ,0x12 ,0x31};
	...
	void CSetPMICTimeRequest::OnDataReceiver(CBufferPtr& ptBufferReceived)
	{
		...
		//memcpy(&resp, ptBufferReceived->GetData(),sizeof(ftm_pmic_pkt_type));
		Safememcopy((void*)&resp,sizeof(resp),(void*)ptBufferReceived->GetData(),ptBufferReceived->GetLength());

		if(resp.unknown[1] == 0x10 && resp.cmd_code == 0x4B)
			m_bExecuteResult = true; 
		...
		,m_nSec(nSec)
	{
		...
		BYTE szCmpBuf[] = {0x4B, 0x0B , 0x0B , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x01 , 0x78 , 0x4C , 0x4C , 0xC8, 0xEE , 0x28 , 0x00 , 0x4C };
		CBufferPtr ptBufferSend;
		Encode((BYTE*)szCmpBuf, sizeof(szCmpBuf), ptBufferSend);
		SetSendData(ptBufferSend);
	}
		...
		, m_szReadValue1024(szReadValue1024), m_nLength(nLength)
	{
		...
		CBufferPtr ptBufferSend;
		Encode((BYTE*)&DIAGcmd, sizeof(DIAGcmd), ptBufferSend);
		SetSendData(ptBufferSend);
	}
	...
	void CReadSWVersionRequest::OnDataReceiver(CBufferPtr& ptBufferReceived)
	{
		...
		//memcpy(&resp, ptBufferReceived->GetData(),sizeof(toolsdiag_ext_build_id_rsp_type));
		Safememcopy((void*)&resp,sizeof(resp),(void*)ptBufferReceived->GetData(),ptBufferReceived->GetLength());
    ...
    //  [9/8/2009 Eddy.Guo]
    //////////////////////////////////////////////////////////////////////////
    CProximitySensorTestRequest::CProximitySensorTestRequest(bool& bIsCmdSuccess)
        :CDiagRequestBase(_T("CClearActivationDataRequest"))
        ,m_bIsCmdSuccess(bIsCmdSuccess) 
    {
        BYTE pSendBuf_Old[] = {75, 0, 14, 0};
        CBufferPtr ptBufferSend;
        Encode(pSendBuf_Old, 4, ptBufferSend);
        SetSendData(ptBufferSend);
    }

    CProximitySensorTestRequest::~CProximitySensorTestRequest(void) 
    {
    }

    bool CProximitySensorTestRequest::IsCompletePacket(CBufferPtr& ptBufferReceived) 
    {
        return CDiagRequestBase::IsCompletePacket(ptBufferReceived);
    }

    void CProximitySensorTestRequest::OnDataReceiver(CBufferPtr& ptBufferReceived) 
    {
        if(ptBufferReceived->GetLength() >= 5)
        {
            const char* data = ptBufferReceived->GetData();
            if(data[0] == 0x4B)
                m_bIsCmdSuccess = (data[4] == 0);
        }
        CDiagRequestBase::OnDataReceiver(ptBufferReceived);
    }
