...
size_t CPowerSupplyHdrStd::SetToolPS()
{
	...
	for(	std::map<CString, CString>::iterator iter = this->m_mapPS.begin();
			...
			iter++)
	{
		if(!this->m_pITI->SetTestPart(IToolInterface::POWERSUPPLY, L"", iter->first.GetString(), iter->second.GetString()))
		{
	...
	for(	std::vector<CString>::iterator iter = this->m_mapPSList.begin();
			...
			iter++)
	{
		if(!this->m_pITI->SetTestPart(IToolInterface::POWERSUPPLY, L"", _T("PowerSupplyList"), (*iter).GetString()))
		{
			ret = TOOL_SET_PS_FAIL;
			break;
		}
...
{
	XMLNode powerSupplyCount;

	if (this->m_Parametermap[ParameterKeyWord::STATIONNAME] == STATION_2G3GTEST ||
		this->m_Parametermap[ParameterKeyWord::STATIONNAME] == _T("LTE"))
	{
		powerSupplyCount = m_PSXML.SearchNode(_T("//Configuration//PowerSupply//PowerSupplyCount"));
	}
	else
	{
		powerSupplyCount = m_PSXML.SearchNode(_T("//Configuration//TestProcessor//PowerSupply//PowerSupplyCount"));
	}
...
{
	XMLNode powerSupplyList;

	if (this->m_Parametermap[ParameterKeyWord::STATIONNAME] == STATION_2G3GTEST ||
		this->m_Parametermap[ParameterKeyWord::STATIONNAME] == _T("LTE"))
	{
		powerSupplyList = m_PSXML.SearchNode(_T("//Configuration//PowerSupply//PowerSupplyList"));
	}
	else
	{
		powerSupplyList = m_PSXML.SearchNode(_T("//Configuration//TestProcessor//PowerSupply//PowerSupplyList"));
	}
...
size_t CPowerSupplyHdrStd::LoadPSItem()
{
	...
	if (this->m_Parametermap[ParameterKeyWord::STATIONNAME] == STATION_2G3GTEST ||
		this->m_Parametermap[ParameterKeyWord::STATIONNAME] == _T("LTE"))
	{
		powerSupply = m_PSXML.SearchNode(_T("//Configuration//PowerSupply"));
	}
	else
	{
		powerSupply = m_PSXML.SearchNode(_T("//Configuration//TestProcessor//PowerSupply"));
	}
