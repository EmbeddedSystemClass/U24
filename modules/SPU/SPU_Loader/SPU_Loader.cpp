...
//For SPU
LOADER_API bool WriteTestItem(TCHAR* szName,TCHAR* szFunction,TCHAR* szOutputFile)
{
	bool bRes = false;
	...
	TSTRING strMode;
	if(_T("append") == strFunction)
		strMode = _T("a+");
	else if(_T("create") == strFunction)
		strMode = _T("w+");

	if(strMode.empty() == false)
	{
		FILE* file = _tfopen(szOutputFile,strMode.c_str());
		if(file)
		{
			int nIP = 0;
			fclose(file);
			...
			file = _tfopen(szOutputFile,_T("a"));
			int nSeek = fseek(file,-1,SEEK_END);
			TCHAR szBuffer[1024] = {0};

			if(nSeek == -1)
			{
				_stprintf(szBuffer,_T("{Header\n}\n{Description\n}\n{Code\n"));
				fwrite(szBuffer,_tcslen(szBuffer)*sizeof(TCHAR),1,file);
			}
			...
			fwrite(szBuffer,_tcslen(szBuffer)*sizeof(TCHAR),1,file);
			_stprintf(szBuffer,_T("%04d runscript %s\n"),nIP,szName);
			fwrite(szBuffer,_tcslen(szBuffer)*sizeof(TCHAR),1,file);
			_stprintf(szBuffer,_T("%04d comapretrue %04d\n"),nIP+1,nIP+4);
			fwrite(szBuffer,_tcslen(szBuffer)*sizeof(TCHAR),1,file);
			_stprintf(szBuffer,_T("%04d print \"%s fail\"\n"),nIP+2,szName);
			fwrite(szBuffer,_tcslen(szBuffer)*sizeof(TCHAR),1,file);
			_stprintf(szBuffer,_T("%04d comparefalse 9999\n"),nIP+3,szName);
			fwrite(szBuffer,_tcslen(szBuffer)*sizeof(TCHAR),1,file);
			_stprintf(szBuffer,_T("%04d print \"%s success\"\n"),nIP+4,szName);
			fwrite(szBuffer,_tcslen(szBuffer)*sizeof(TCHAR),1,file);
			fclose(file);
		}
	}
