...
#include "..\..\Modules\RequestManager\Request\SimLockStoreRequest.h"
#include "..\..\Modules\Lib\RequestUtility\DiagCommand.h"
								 ...
								 BYTE* piSIM_SecureData, DWORD *pCmdStatus, DWORD *pStatus, int nTimeout)
{
	...
	else if (iSIM_SecureDataLen <= 576)
	...
	else if (iSIM_SecureDataLen <= 1024)
		return StoreSimData_1024(iSlotId, iSIM_AppIdLen, piSIM_AppId, iSIM_SecureDataLen, piSIM_SecureData, pCmdStatus, pStatus, nTimeout);
	else if (iSIM_SecureDataLen <= 2048)
		return StoreSimData_2048(iSlotId, iSIM_AppIdLen, piSIM_AppId, iSIM_SecureDataLen, piSIM_SecureData, pCmdStatus, pStatus, nTimeout);
	else
		return FALSE;
}
...
BOOL CSimLockStore::StoreSimData_1024(BYTE iSlotId, BYTE iSIM_AppIdLen, BYTE* piSIM_AppId, DWORD iSIM_SecureDataLen, 
									 BYTE* piSIM_SecureData, DWORD *pCmdStatus, DWORD *pStatus, int nTimeout)
{
	using namespace PLATFORM_TOOL;
	GSIDDIAG_SIM_SECURE_STORE_REQ_1024 cmdReq;

	memset(&cmdReq, 0, sizeof(GSIDDIAG_SIM_SECURE_STORE_REQ_1024));
	cmdReq.aps_as_transport = 1;
	cmdReq.subcommand = 87;
	cmdReq.slot_id = iSlotId;
	cmdReq.sim_app_in_len = iSIM_AppIdLen;
...
BOOL CSimLockStore::StoreSimData_2048(BYTE iSlotId, BYTE iSIM_AppIdLen, BYTE* piSIM_AppId, DWORD iSIM_SecureDataLen, 
									BYTE* piSIM_SecureData, DWORD *pCmdStatus, DWORD *pStatus, int nTimeout)
{
	using namespace PLATFORM_TOOL;
	GSIDDIAG_SIM_SECURE_STORE_REQ_2048 cmdReq;

	memset(&cmdReq, 0, sizeof(GSIDDIAG_SIM_SECURE_STORE_REQ_2048));
	cmdReq.aps_as_transport = 1;
	cmdReq.subcommand = 87;
	cmdReq.slot_id = iSlotId;
	cmdReq.sim_app_in_len = iSIM_AppIdLen;
...
bool CSimLockStore::WriteSimDataNew(DWORD iSlotId, BYTE *pSimDataBuf, int nSimDataBufLen, int nTimeout)
{
	using namespace PLATFORM_TOOL;
	...
	CRequestPtr ptRequest = new CWriteSimDataNewRequest(pSimDataBuf, nSimDataBufLen, bExecuteResult, iSlotId);
...
bool CSimLockStatus::ReadSIMLockName (char* strSIMLockName)
{
	using namespace PLATFORM_TOOL;
	bool bRes = false;

	bool bStatus=0;
	CRequestPtr ptRequest = new CQuerySIMLockNameRequest(strSIMLockName, bStatus);	
	if(Execute(ptRequest))
	{
		bRes = bStatus;
	}
...
CSimLockStatusEx::CSimLockStatusEx(int iCOMPort):CInterfaceBase(iCOMPort)
{
}


CSimLockStatusEx::~CSimLockStatusEx(void)
{
}

bool CSimLockStatusEx::QueryLockStatus(bool *nwStatus, bool *spStatus, bool *nsStatus, bool *cpStatus, bool *simStatus)
{
	using namespace PLATFORM_TOOL;

	diagpkt_subsidy_lock_ex_resp resp = {0};
	CRequestPtr ptRequest = new CSimLockStatusRequestEx(resp);

	if (!Execute(ptRequest))
		return false;

	if (resp.status != 0 || resp.command_status != 0)
		return false;

	if (nwStatus != NULL)		*nwStatus = (resp.sim_nw_feature != 0);
	if (spStatus != NULL)		*spStatus = (resp.sim_sp_feature != 0);
	if (nsStatus != NULL)		*nsStatus = (resp.sim_ns_feature != 0);
	if (cpStatus != NULL)		*cpStatus = (resp.sim_cp_feature != 0);
	if (simStatus != NULL)		*simStatus = (resp.sim_sim_feature != 0);
...
CSIM_API bool QueryLockStatusEx(int iPort, bool *nwStatus, bool *spStatus, bool *nsStatus, bool *cpStatus, bool *simStatus)
{
	CSimLockStatusEx lockStatus(iPort);
	return lockStatus.QueryLockStatus(nwStatus, spStatus, nsStatus, cpStatus, simStatus);
}
...
bool CSimUnLockStatus::TempUnlockStatus(int unlockType, char *sCode, DWORD *pStatus, int nTimeout)
{
	if (sCode == NULL)
	{
		if (pStatus != NULL)
	...
	diagpkt_subsidy_unlock_resp resp = {0};
	CRequestPtr ptRequest = new CSimTempUnLockRequest(unlockType, sCode, resp);
...
CSimUnLockStatusEx::CSimUnLockStatusEx(int iCOMPort):CInterfaceBase(iCOMPort)
{
}


CSimUnLockStatusEx::~CSimUnLockStatusEx(void)
{
}

bool CSimUnLockStatusEx::UnlockStatus(SIMLOCKTYPE unlockType, char *szCode, DWORD *pdwStatus, int nTimeout)
{
	if (szCode == NULL) {
		if (pdwStatus != NULL)
			*pdwStatus = 0;
	...
	diagpkt_subsidy_unlock_ex_resp resp = {0};
	CRequestPtr ptRequest = new CSimUnLockRequestEx(unlockType, szCode, resp);

	if (!Execute(ptRequest, nTimeout))
		return false;

	if (pdwStatus != NULL)
		*pdwStatus = resp.status;

	return true;
}

CSIM_API bool UnlockSIMLockEx(int iComPort, SIMLOCKTYPE unlockType, char *szCode, DWORD *pdwStatus)
{
	CSimUnLockStatusEx unlockStatus(iComPort);
	return unlockStatus.UnlockStatus(unlockType, szCode, pdwStatus);
}

// --------------------------------------------------------

CSIM_API bool CheckSimDataIsProgrammed(long nComPort)
{
...
CCFCSimLockRelated::CCFCSimLockRelated(int nComPort):CSimLockStatus(nComPort), CLastError(COMDeviceName(nComPort))
{}
